...
class My_SubirMatrizInscripcion(SubirMatrizInscripcion):
    ...
    def crear_persona_senescyt(self, proceso, persona):
        ...
        cedulas = Matricula.objects.filter( status=True, nivel__periodo_id=317, retiradomatricula=False, materiaasignada__materia__asignaturamalla__malla__carrera__coordinacion=9).values_list('inscripcion__persona__cedula', flat=True)
                        ...
                        for result in results:
                            ...
                            if result[10]:
                                ...
                                try:
                                    ...
                                    eInstitucionEducacionSuperior = InstitucionEducacionSuperior(nombre=result[3],
        ...
        for repe in repetidores:
            ...
            if repe.mismaterias():
                ...
                if mallaacutualcarrera and mimalla and (not mallaacutualcarrera.id == mimalla.id):
                    ...
                    mimalla = im.malla
                if not Materia.objects.filter(nivel__periodo=periodoactual, asignaturamalla__malla=mimalla, asignaturamalla__malla__carrera=inscripcion.carrera, nivel__sesion=inscripcion.sesion).exists():
                    return False
