...
def view(request):
    ...
    if not es_director_carr and not es_docente and not es_director_vincu and not es_analista_vincu:
        return HttpResponseRedirect("/?info=El Módulo está disponible para Directores de Carrera, Docentes y Director/Analista de Vinculación.")
    ...
    if request.method == 'POST':
        ...
        if action == 'actualizarformatoactividades':
            ...
            try:
                ...
                if actividad.validacion_inscritos(idinscripcion, periodo):
                    return JsonResponse(
                ...
                if id2 == 2:
                    ...
                    for id, estado, observacion in zip(idinscripciones, estados, observaciones):
                        if int(estado) != 1:
                            inscripcion_ae = InscripcionActividadConvalidacionPPV.objects.get(pk=int(id))
                            if int(estado) != 3 :
                                if actividad.validacion_inscritos_adicionar_manual(inscripcion_ae.inscripcion.id, periodo):
