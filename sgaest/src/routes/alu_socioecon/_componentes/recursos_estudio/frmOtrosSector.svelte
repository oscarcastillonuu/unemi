<script lang="ts">
	...
	onMount(async () => {
		...
		if (aData.eFichaSocioeconomica) {
			...
			if (eFichaSocioeconomica.otrossector) {
				inputOtros = eFichaSocioeconomica.otrossector ?? '';
			}
		}
		await delay(2000);
		load = false;
	...
	const saveDatosRecursosEstudios = async () => {
		...
		formData.append('otrossector', inputOtros);
		formData.append('field', 'otrossector');
	...
	<ModalBody>
		{#if !load}
			<form action="javascript:;" id="frmRecurso">
				<div class="row g-3">
					<div class="col-12">
						<label for="id_otrossector" class="form-label fw-bold"
						...
						<div class="valid-feedback" id="id_otrossector_validate">¡Se ve bien!</div>
					</div>
				</div>
			</form>
		{:else}
