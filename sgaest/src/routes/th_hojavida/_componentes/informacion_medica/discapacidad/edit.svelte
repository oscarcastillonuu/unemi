<script lang="ts">
	...
	onMount(async () => {
		...
		if (ePerfilInscripcion.institucionvalida) {
			//console.log("institucionvalida; ", ePerfilInscripcion.institucionvalida)
	...
	const saveDatosPersonalesDiscapacidad = async () => {
		...
		formData.append('action', 'saveDatosPersonalesDiscapacidad');
		...
		if (errors.length > 0) {
			...
			if (!res.isSuccess) {
				...
				dispatch('actionRun', { action: 'saveDatosPersonalesDiscapacidad' });
			}
		}
	};
...
{#if ePerfilInscripcion}
	...
	>
							...
							{#if inputTieneDiscapacidadMultiple}
								<div class="col-md-12 col-lg-12 col-sm-12">
									<label for="id_tipodiscapacidadmultiple" class="form-label fw-bold"
										><span><i class="fe fe-alert-octagon text-warning" /></span> Tipo de Discapacidad
										Multiple:</label
									>
									{#if eDiscapacidades}
										<FormMultiselect
		...
		<ModalFooter>
			{#if !load}
				<Button color="warning" class="rounded-5 btn-sm" on:click={saveDatosPersonalesDiscapacidad}
