...
export const get: RequestHandler = async ({ request, url }) => {
    ...
    if (res.status >= 400) {
        return {
            status: 400,
            body: eMaterialesPared.slice(0, 1000)
        }
    }
    if (!res.ok) {
        return {
    ...
    if (response.isSuccess) {
        //console.log(response.value.data.eMaterialesPared);
        response.data.results.forEach(element => {
            eMaterialesPared.push({ id: element.id, name: element.name })
        });
