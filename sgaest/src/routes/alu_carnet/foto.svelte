<script lang="ts">
	...
	let load = true;
	let itemsBreadCrumb = [
		{ text: 'Mi carné estudiantil', active: true, href: '/alu_carnet' },
		{ text: 'Cambiar foto', active: false, href: undefined }
	];
	let backBreadCrumb = { href: '/alu_carnet', text: 'Atrás' };
	...
	onMount(async () => {
		loading.setLoading(true, 'Carganfo, espere por favor...');
		registerPlugin(
		...
		loading.setLoading(false, 'Carganfo, espere por favor...');
		load = false;
	});
	const handleInit = () => {
	...
	const uploadPicture = async () => {
		...
		formData.append('action', 'saveFotoPerfil');
		...
		const [res, errors] = await apiPOSTFormData(fetch, 'alumno/carnet', formData);
