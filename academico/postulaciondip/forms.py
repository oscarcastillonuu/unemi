...
class ConvocatoriaForm(forms.Form):
    ...
    fechainicio = forms.DateField(label=u"Fecha Inicio", input_formats=['%d-%m-%Y'], initial=hoy, required=True, widget=DateTimeInput(format='%d-%m-%Y', attrs={'class': 'form-control', 'autocomplete':'off', 'col':'6'}))
    fechafin = forms.DateField(label=u"Fecha Fin", input_formats=['%d-%m-%Y'], initial=hoy, required=True, widget=DateTimeInput(format='%d-%m-%Y', attrs={'class': 'form-control', 'autocomplete':'off', 'col':'6'}))
