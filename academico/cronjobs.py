                    ...
                    if claseshorario:
                        ...
                        for clase in claseshorario:
                            if not Leccion.objects.values('id').filter(clase_id=clase.id, fecha=datetime.now()).exists():
                                leccionanterior = lg.lecciones.filter(clase__materia_id=clase.materia_id)[0]
                        ...
                        if puede_continuar:
                            try:
                                if LeccionGrupo.objects.values('id').filter(profesor_id=lg.profesor_id, turno_id=turno.id, fecha=datetime.now()).exists():
                                    lecciongrupo = LeccionGrupo.objects.filter(profesor_id=lg.profesor_id, turno_id=turno.id, fecha=datetime.now())[0]
                                ...
                                for clase in claseshorario:
                                    ...
                                    eLecciones = Leccion.objects.filter(clase_id=clase.id, fecha=datetime.now())
                                    leccionanterior = lg.lecciones.filter(clase__materia_id=clase.materia_id)[0]
                                    ...
                                    for asistencias in AsistenciaLeccion.objects.filter(leccion_id=leccionanterior.id):
                                        ...
                                        # REGISTRO MANUAL (DOCENTE)
                                            ...
                                            vAsistio = False
                                            vVirtual = False
                                        if AsistenciaLeccion.objects.filter(leccion_id=leccion.id, materiaasignada_id=asistencias.materiaasignada_id).exists():
                                            asistencialeccion =AsistenciaLeccion.objects.filter(leccion_id=leccion.id, materiaasignada_id=asistencias.materiaasignada_id)
                                            asistencialeccion.update(status=vStatus,
                                                                     asistio=vAsistio,
                                                                     virtual=vVirtual,
                                                                     usuario_creacion_id=lg.profesor.persona.usuario_id)
                                            vUsuario_creacion_id = lg.profesor.persona.usuario_id
                                        else:
                                    ...
                                    if TemaAsistencia.objects.values('id').filter(leccion_id=leccionanterior.id, status=True).exists():
                                            for tema in TemaAsistencia.objects.filter(leccion_id=leccionanterior.id, status=True):
                                                ...
                                                temaasistencia.save()
                                                if SubTemaAsistencia.objects.values('id').filter(tema_id=tema.id).exists():
...
workQueue = queue.Queue(len(LeccionGrupo.objects.filter(abierta=True, solicitada=False)))
threads = []
threadID = 1
