    ...
    if request.method == 'POST':
        ...
        if 'action' in request.GET:
            ...
            if 'id' in request.GET:
                data['tipobus'] = 2
                ids = request.GET['id']
                ponencias = PonenciasInvestigacion.objects.select_related().filter(pk=int(encrypt(ids)), status=True).order_by('-fechapublicacion')
                ...
                if tipobus == 2:
                    ponencias = PonenciasInvestigacion.objects.select_related().filter(nombre__icontains=search, status=True).order_by('-fechapublicacion')
                if tipobus == 3:
                    if ' ' in search:
                        s = search.split(" ")
                        ...
                        ponenciasdoc = PonenciasInvestigacion.objects.select_related().filter(pk__in=participantesponenciasdoc, status=True)
                        ...
                        ponenciasadm = PonenciasInvestigacion.objects.select_related().filter(pk__in=participantesponenciasadm, status=True)
                ...
                if tipobus == 4:
                    ...
                    ponencias = PonenciasInvestigacion.objects.select_related().filter(evento__icontains=search, status=True).order_by('-fechapublicacion')
                if tipobus == 5:
                    # Buscar por año de publicación
                    anio = int(search)
                    ponencias = PonenciasInvestigacion.objects.select_related().filter(fechapublicacion__year=anio, status=True).order_by('-fechapublicacion')
                ...
                ponencias = PonenciasInvestigacion.objects.select_related().filter(pk__in=participantesponencias, status=True).order_by('-fechapublicacion')
            else:
                data['tipobus'] = 2
                ponencias = PonenciasInvestigacion.objects.filter(status=True).order_by('-fechapublicacion')
