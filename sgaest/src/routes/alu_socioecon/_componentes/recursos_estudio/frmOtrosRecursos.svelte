<script lang="ts">
	...
	onMount(async () => {
		...
		if (aData.eFichaSocioeconomica) {
			...
			if (eFichaSocioeconomica.otrosrecursos) {
				inputOtros = eFichaSocioeconomica.otrosrecursos ?? '';
			}
		}
		await delay(2000);
		load = false;
	...
	const saveDatosRecursosEstudios = async () => {
		loading.setLoading(true, 'Guardando la información, espere por favor...');
		await resetForms('none');
		const $frmRecurso = document.getElementById('frmRecurso');
		const formData = new FormData($frmRecurso);
		formData.append('otrosrecursos', inputOtros);		
		formData.append('field', 'otrosrecursos');
		formData.append('action', 'saveDatosRecursosEstudios');
		...
		if (errors.length > 0) {
			...
			if (!res.isSuccess) {
				...
				dispatch('actionRun', { action: 'saveDatosRecursosEstudios' });
			}
		}
	};
</script>
...
>
	...
	<ModalBody>
		{#if !load}
			<form action="javascript:;" id="frmRecurso">
				<div class="row g-3">
					<div class="col-12">
						<label for="id_otrosrecursos" class="form-label fw-bold"
							><span><i class="fe fe-alert-octagon text-warning" /></span> Describa otros
						</label>
						<input
							type="text"
							...
							aria-label=""
							id="id_otrosrecursos"
							bind:value={inputOtros}
						/>

						<div class="valid-feedback" id="id_otrosrecursos_validate">¡Se ve bien!</div>
					</div>
				</div>
			</form>
		{:else}
	...
	<ModalFooter>
		...
		{#if !load}
			<Button color="primary" class="rounded-3 btn-sm" on:click={saveDatosRecursosEstudios}
				>Guardar</Button
			>
		{/if}
	</ModalFooter>
