<script lang="ts">
	...
	import {
		getEstadoCivil as loadEstadoCivil,
		getSexos as loadSexos,
		getPaises as loadNacionalidades
	...
	onMount(async () => {
		...
		if (ePersona.paisnacionalidad) {
			readSelectionNacionalidad = {
				id: ePersona.paisnacionalidad['pk'],
				name: ePersona.paisnacionalidad['nombre']
			};
		}
		await delay(2000);
		load = false;
	...
	const saveDatosPersonalesBasico = async () => {
		...
		if (readSelectionNacionalidad != null) {
			formData.append('paisnacionalidad', readSelectionNacionalidad.id);
		}
		...
		formData.append('action', 'saveDatosPersonalesBasico');
		...
		if (errors.length > 0) {
			...
			if (!res.isSuccess) {
				...
				dispatch('actionRun', { action: 'saveDatosPersonalesBasico' });
			}
		}
	};
...
{#if mOpenModal}
	{#if ePersona}
		<Modal isOpen={true} toggle={mToggle} size={mSize} class={mClass} backdrop="static">
				...
				{#if !load}
					<form action="javascript:;" id="frmDatosPersonales">
						<div class="row g-3">
							...
							<div class="col-md-4">
								<label for="id_paisnacionalidad" class="form-label fw-bold"
									><span><i class="fe fe-alert-octagon text-warning" /></span> País de nacionalidad:</label
								>
								<FormSelectSearch
									inputId="id_paisnacionalidad"
									name="paisnacionalidad"
									bind:value={readSelectionNacionalidad}
									on:actionChangeSelectSearch={changeNacionalidad}
									fetch={(query) => loadNacionalidades(query)}
								/>
								<div class="valid-feedback" id="id_paisnacionalidad_validate">¡Se ve bien!</div>
							</div>
							<div class="col-md-4">
							...
							<div class="col-md-6">
								<div class="form-check form-switch">
									...
									/>
									<label class="form-check-label fw-bold text-dark" for="id_eszurdo"
										>¿Es Zurdo?</label
									>
								</div>
						...
						<div class="row g-3">
							...
							<div class="col-md-4">
								...
								<div class="valid-feedback" id="id_libretamilitar_archivo_validate">
									¡Se ve bien!
								</div>
							</div>
						</div>
			...
			<ModalFooter>
				{#if !load}
					<Button color="warning" class="rounded-5 btn-sm" on:click={saveDatosPersonalesBasico}
