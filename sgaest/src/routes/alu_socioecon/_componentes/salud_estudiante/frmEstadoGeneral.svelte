<script lang="ts">
	...
	onMount(async () => {
		...
		if (aData.eFichaSocioeconomica) {
			...
			if (eFichaSocioeconomica.estadogeneral) {
				selectEstadoGeneral = eFichaSocioeconomica.estadogeneral ?? 0;
			}
		}
		await delay(2000);
		load = false;
	...
	const saveDatosSaludEstudiante = async () => {
		...
		formData.append('estadogeneral', selectEstadoGeneral.toString());
		formData.append('field', 'estadogeneral');
	...
	<ModalBody>
		{#if !load}
			<form action="javascript:;" id="frmEnfermedad">
				<div class="row g-3">
					<div class="col-12">
						<label for="id_estadogeneral" class="form-label fw-bold"
							><span><i class="fe fe-alert-octagon text-warning" /></span> Estado de salubridad de las
							condiciones de vida
						</label>
						<select
							...
							aria-label=""
							id="id_estadogeneral"
							bind:value={selectEstadoGeneral}
						>
							<option value={0} selected> ----------- </option>
							{#each eTipos as eTipo}
								{#if selectEstadoGeneral === eTipo.id}
						...
						<div class="valid-feedback" id="id_estadogeneral_validate">¡Se ve bien!</div>
					</div>
				</div>
			</form>
		{:else}
