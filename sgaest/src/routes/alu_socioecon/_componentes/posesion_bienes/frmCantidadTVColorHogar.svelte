<script lang="ts">
	...
	import { getCantidadesTVColorHogar as loadDataCantidades } from '$lib/utils/loadDataApi';
	...
	let selectCantidadTVColorHogar = 0;
	...
	onMount(async () => {
		...
		if (aData.eFichaSocioeconomica) {
			...
			if (eFichaSocioeconomica.canttvcolor) {
				selectCantidadTVColorHogar = eFichaSocioeconomica.canttvcolor['pk'] ?? 0;
			}
		}
		await delay(2000);
		load = false;
	...
	const saveDatosPosesionBienes = async () => {
		...
		formData.append('canttvcolor', selectCantidadTVColorHogar.toString());
		formData.append('field', 'canttvcolor');
	...
	<ModalBody>
		{#if !load}
			<form action="javascript:;" id="frmCantidad">
				<div class="row g-3">
					<div class="col-12">
						<label for="id_canttvcolor" class="form-label fw-bold"
							><span><i class="fe fe-alert-octagon text-warning" /></span> ¿Cuántos TV a color tienen
							en el hogar?
						</label>
						<select
							...
							aria-label=""
							id="id_canttvcolor"
							bind:value={selectCantidadTVColorHogar}
						>
							<option value={0} selected> ----------- </option>
							{#each eCantidadades as eCantidad}
								{#if selectCantidadTVColorHogar === eCantidad.id}
						...
						<div class="valid-feedback" id="id_canttvcolor_validate">¡Se ve bien!</div>
					</div>
				</div>
			</form>
		{:else}
