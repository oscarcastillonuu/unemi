    ...
    if request.method == 'POST':
        ...
        if action == 'inscribir':
            try:
                ...
                if InscripcionActividadConvalidacionPPV.objects.filter(status=True, inscripcion=inscripcion, actividad__periodo_id= periodo, estado__in=[1, 2, 4]):
        ...
        if 'action' in request.GET:
            ...
            if action == 'inscribir':
                ...
                try:
                    data['prcaticas_en_proceso'] = prcaticas_en_proceso = DetallePreInscripcionPracticasPP.objects.filter(status=True, inscripcion=inscripcion, estado__in=[1, 2, 4, 6]).values('id').exists()
                    data['actividad_en_proceso'] = actividad_en_proceso = InscripcionActividadConvalidacionPPV.objects.filter(status=True,estado__in=[1,2,4],inscripcion=inscripcion).exists()
