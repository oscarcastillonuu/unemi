...
def view(request):
    ...
    if request.method == 'POST':
        ...
        if 'action' in request.GET:
            ...
            perfil, ids, search, gruposelect, url_vars=request.GET.get('perfil',''), \
                                                       request.GET.get('id', ''), \
                                                       request.GET.get('s',''),\
                                                      request.GET.get('g',0),''
            ...
            listagrupo = []
            if gruposelect:
                data['gruposelect'] = gruposelect = int(request.GET['g'])
                url_vars += "&gruposelect={}".format(gruposelect)
                if gruposelect > 0:
            ...
            if search:
                ...
                if len(ss) == 1:
                    ...
                    administrativos = administrativos.filter(Q(persona__apellido1__icontains=ss[0]) & Q(persona__apellido2__icontains=ss[1]) & Q(persona__usuario__groups__in=listagrupo)).distinct()
            elif ids:
                data['ids'] = ids = request.GET['id']
                url_vars += "&ids={}".format(ids)
                administrativos = administrativos.filter(id=ids, persona__usuario__groups__in=listagrupo).distinct()
            if perfil:
                data['perfil'] = perfil = request.GET['perfil']
                ...
                if perfil == '1':
                    ...
                    administrativos = administrativos.filter(activo=False, persona__usuario__groups__in=listagrupo).distinct()
            if gruposelect>0:
                administrativos = administrativos.filter(persona__usuario__groups__in=listagrupo).distinct()
