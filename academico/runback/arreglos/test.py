...
with transaction.atomic():
    try:
        matriculas = []
        # totalrecord = RecordAcademico.objects.filter(status=True, asignaturamalla__malla__modalidad_id=3, aprobada=False).exclude(asignaturamalla__malla__carrera_id__in=[133, 61, 235, 225])
        totalrecord = RecordAcademico.objects.filter(status=True, asignaturamalla__malla__modalidad_id=3, aprobada=False, inscripcion__persona__cedula='0929484871')
        c = 0
        for hrecord in totalrecord:
            matricula = hrecord.inscripcion.matricula_set.filter(status=True, nivel__periodo_id=317).first()
            c += 1
            if matricula:
                if matricula.aranceldiferido == 1:
                    # noti = Notificacion(cuerpo=f'Diferido {matricula.inscripcion.persona.cedula}',
                    #                     titulo='Diferido',
                    ...
                    # noti.save()
                    # continue
                    materia = matricula.materiaasignada_set.filter(status=True, materia__asignatura_id=hrecord.asignatura_id).first()
                    totalmaterias = hrecord.historicorecordacademico_set.filter(status=True).count() + 1
                    if materia:
                        if not materia.matriculas == totalmaterias:
                            if not matricula.id in matriculas:
                                matriculas.append(matricula.id)
                            materia.matriculas = totalmaterias
                            materia.save()
        if c == totalrecord.count():
            from matricula.funciones import get_tipo_matricula
            for matricula in Matricula.objects.filter(status=True, id__in=matriculas):
                prubro = Rubro.objects.filter(matricula=matricula)
                if p in prubro:
                    if p.tiene_pagos():
                        break
                if not matricula.inscripcion.mi_coordinacion().id in [7, 10]:
                    ...
                    matricula.save()
                ePeriodoMatricula = PeriodoMatricula.objects.filter(status=True, activo=True, periodo=matricula.nivel.periodo).first()
                ...
                arancel = Rubro.objects.filter(matricula=matricula, tipo_id=RUBRO_ARANCEL, status=True).first()
                if not arancel:
                ...
                if valor_cuota_mensual == 0:
                    # raise NameError(u"No se puede procesar el registro.")
                    continue
                eRubroMatricula = Rubro.objects.filter(matricula=matricula, tipo_id=RUBRO_MATRICULA).first()
                ...
                if ePeriodoMatricula.valida_rubro_acta_compromiso:
                    isResult, message = matricula.generar_actacompromiso_matricula_pregrado(request)
                    if not isResult:
                        raise NameError(message)
                    url_acta_compromiso = message
                    matricula.aranceldiferido = 1
                    matricula.actacompromiso = url_acta_compromiso
                    matricula.save()
    except Exception as ex:
        print(ex)
        transaction.set_rollback(True)
