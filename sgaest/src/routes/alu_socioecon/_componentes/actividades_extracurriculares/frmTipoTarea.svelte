<script lang="ts">
	...
	onMount(async () => {
		...
		if (aData.eFichaSocioeconomica) {
			...
			if (eFichaSocioeconomica.tipotarea) {
				selectTipo = eFichaSocioeconomica.tipotarea ?? 0;
			}
			inputOtro = eFichaSocioeconomica.otrostarea ?? '';
		}
		await delay(2000);
		load = false;
	});
	...
	const saveDatosActividadExtracurricular = async () => {
		...
		formData.append('tipotarea', selectTipo.toString());
		formData.append('field', 'tipotarea');
		formData.append('otrostarea', inputOtro);
	...
	<ModalBody>
		{#if !load}
			<form action="javascript:;" id="frmActividad">
				<div class="row g-3">
					<div class="col-12">
						<label for="id_tipotarea" class="form-label fw-bold"
							><span><i class="fe fe-alert-octagon text-warning" /></span> Seleccione donde realiza las
							tareas</label
						>
						<select
						...
						<div class="valid-feedback" id="id_tipotarea_validate">¡Se ve bien!</div>
					</div>
					{#if selectTipo == 8}
						<div class="col-12">
							<label for="id_otrostarea" class="form-label fw-bold"
							...
							<div class="valid-feedback" id="id_otrostarea_validate">¡Se ve bien!</div>
						</div>
					{/if}
				</div>
			</form>
