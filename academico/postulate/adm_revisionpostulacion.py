    ...
    if request.method == 'POST':
        ...
        if 'action' in request.GET:
            ...
            if action == 'buscarpostulantesaprobados':
                try:
                    data['partida'] = partida = Partida.objects.get(pk=request.GET['partida'])
                    data['postulantes'] = PersonaAplicarPartida.objects.filter(partida=partida, status=True, estado=1).order_by('-nota_final_meritos')
                    template = get_template("postulate/adm_revisionpostulacion/postulantes.html")
                    return JsonResponse({"result": True, 'data': template.render(data)})
                except Exception as ex:
                    pass
            ...
            if action == 'excel_postulantes__all':
                try:
                    ...
                    for det in listado:
                        ...
                        ws.write(row_num, 20, det.get_estado_display(), style2)
                        ...
                        ws.write(row_num, 38, det.get_estado_display(), style2)
            ...
            if action == 'excel_partida':
                try:
                    ...
                    for det in listado:
                        ...
                        ws.write(row_num, 7, det.get_estado_display(), style2)
                        ...
                        ws.write(row_num, 26, 'SI' if det.calificada else 'NO', style2)
                        ws.write(row_num, 27, 'SI' if det.solapelacion else 'NO', style2)
                        ws.write(row_num, 28, det.traer_apelacion().get_estado_display() if det.traer_apelacion() else '', style2)
                        obs_revisor, revisado_por, fecha_revision = '', '', ''
                        if det.traer_apelacion():
                            if det.traer_apelacion().estado != 0:
                                obs_revisor = det.traer_apelacion().observacion_revisor
                                ...
                                fecha_revision = str(det.traer_apelacion().fecha_revision)
                        ws.write(row_num, 29, obs_revisor, style2)
                        ws.write(row_num, 30, revisado_por, style2)
                        ws.write(row_num, 31, fecha_revision, style2)
                        ws.write(row_num, 32, det.revisado_por.username if det.revisado_por else '', style2)
                        ws.write(row_num, 33, str(det.fecha_revision) if det.fecha_revision else '', style2)
                        ws.write(row_num, 34, det.desempate_revisado_por if det.desempate_revisado_por else '', style2)
                        ws.write(row_num, 35, str(det.desempate_fecha_revision) if det.desempate_fecha_revision else '', style2)
                        row_num += 1
                    wb.save(response)
                    return response
                except Exception as ex:
