...
from sagest.models import UnidadMedida, TipoProducto, TipoDocumento, Departamento, NATURALEZA_CUENTA, \
    ...
    PeriodoGastosPersonales, GastosPersonales, MOTIVO_SALIDA, GestionProductoServicioTH, SELECCION_REGISTRO_DECIMO, RequisitoPazSalvo, ESTADOS_DOCUMENTOS_PAZ_SALVO, ESTADO_PAZ_SALVO, ESTADOS_DOCUMENTOS_REQUISITOS
...
class DepartamentoForm(forms.Form):
    nombre = forms.CharField(max_length=200, label=u'Departamento', required=False)
    permisogeneral = forms.BooleanField(label=u'Revisa todos los permisos', required=False, widget=CheckboxInput())
    codigoindice = forms.CharField(max_length=200, label=u'Codigo de Indice', required=False,
                                   widget=forms.TextInput(attrs={'class': 'imp-25'}))
    tipoindice = forms.ChoiceField(choices=TIPO_INDICE, label=u'Naturaleza', required=False,
                                   widget=forms.Select(attrs={'class': 'imp-25'}))
...
class PeriodoPoaForm(forms.Form):
...
class AccionDocumentoDetalleForm(forms.Form):
    observacion_envia = forms.CharField(required=False, label=u'Observación', widget=forms.Textarea({'rows': '4'}))
...
class MotivoAccionPersonalForm(forms.Form):
...
class EvidenciaDocumentalForm(forms.Form):
...
class CambiarProductoForm(FormModeloBase):
    ...
    def cambiar(self, id):
        self.fields['producto'].queryset = ProductoServicioSeccion.objects.filter(seccion_id=id,activo=True, status=True)
...
class ConstatacionFisicaForm(FormModeloBase):
    ...
    estadoactual= forms.ModelChoiceField(queryset=EstadoProducto.objects.select_related().filter(status=True,), required=True, label=u'Estado actual', widget=forms.Select(attrs={'col':'12','placeholder': 'Seleccione un estado'}))
...
class ParRevisorArticuloForm(FormModeloBase):
    revista = forms.ModelChoiceField(label=u"Revista", queryset=RevistaInvestigacion.objects.filter(status=True, borrador=False).order_by('nombre'), widget=forms.Select(attrs={'col': '12'}))
