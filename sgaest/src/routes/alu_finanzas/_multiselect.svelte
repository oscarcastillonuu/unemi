  ...
  export let sumaValores = 0.00;
	...
	onMount(() => {
		slot.querySelectorAll('option').forEach((o) => {
			...
			options = [
				...options,
				{ value: o.value, name: o.textContent, valor: o.attributes['id_valor'].value }
			];
		});
		value &&
	...
	const handleChangeRubros = async () => {
		...
		for (let item of vrubrosid) {
			valores_id.push(item.attributes['data-id'].value);
		}
		const [res, errors] = await apiPOST(fetch, 'alumno/finanzas', {
			action: 'consultarvalores',
			id: JSON.stringify(valores_id)
		});
		if (errors.length > 0) {
...
<label for="eRubroComp" class="form-label fw-bold s-_249H3Or5s8C"> Valor $ {sumaValores}:</label>
<div class="multiselect col-md-12" class:readonly>
	<div class="tokens" class:showOptions on:click={handleTokenClick}>
		{#each Object.values(selected) as s}
			<div class="token" data-id={s.value} data-valor={s.valor}>
				<span>{s.name}</span>
				{#if !readonly}
	...
	<select bind:this={slot} type="multiple" class="hidden" on:change={handleChangeRubros}
		><slot /></select
	>
