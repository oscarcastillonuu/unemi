...
from postulaciondip.forms import CerrarActaForm, ArchivoInvitacionForm, FirmaElectronicaIndividualForm, PlanAccionForm, \
    ...
    ConfiguracionGeneralActaSeleccionDocenteForm, CertificacionPresupuestariaInformeContratacionForm,ConfiguracionConclusionesInformeContratacionForm,\
    ConfiguracionRecomendacionesInformeContratacionForm
...
from postulaciondip.models import ActaDocumentacion, HistorialActaSeleccionDocente, ActaSeleccionDocente, Convocatoria, \
    ...
    HistorialConvocatoria, HorarioPlanificacionConvocatoria, VotacionComiteAcademico, ConfiguracionRequisitosPersonalContratar,ConfiguracionGeneralActaSeleccionDocente, \
    InformeContratacion, DetalleInformeContratacion, ConfiguracionInforme,OrdenFirmaInformeContratacion, ResponsabilidadFirma, InformeContratacionIntegrantesFirma
...
def view(request):
    ...
    if request.method == 'POST':
        ...
        if action == 'generarconvocatoria':
            ...
            try:
                ...
                if convocatoria.tipo == 2:
                    ...
                    if form.is_valid():
                        ...
                        convocatoria.activo = form.cleaned_data['activo']
            ...
            try:
                ...
                if f.is_valid():
                    ...
                    if not eVotacionComiteAcademico.filter(inscripcion=f.cleaned_data['inscrito']).exists():
                        ...
                        eVotacionComiteAcademico.save(request)
                        log(u"eVotacionComiteAcademico", request, 'add')
                        ...
                        log(u"eVotacionComiteAcademico update", request, 'change')
                    return JsonResponse({"result": True})
                else:
                    return JsonResponse({"result": False, "mensaje": f"Error al validar los datos.", "form": [{k: v[0]} for k, v in f.errors.items()]})
            except Exception as ex:
            ...
            try:
                ActaParalelo.objects.filter(pk=int(encrypt(request.POST['id']))).update(status=False)
                ...
                if f.is_valid():
                    if not eConfiguracionInforme.exists():
                        eConfiguracionInforme = ConfiguracionInforme(
                            ...
                            de = f.cleaned_data.get('de'),
                            recomendaciones = f.cleaned_data.get('recomendaciones'),
                            valor_x_hora = f.cleaned_data.get('valor_x_hora'),
                        )
                        eConfiguracionInforme.save(request)
                    else:
                        eConfiguracionInforme.first().antecedentes = f.cleaned_data.get('antecedentes')
                        eConfiguracionInforme.first().motivacionjuridica = f.cleaned_data.get('motivacionjuridica')
                        eConfiguracionInforme.first().para = f.cleaned_data.get('para')
                        eConfiguracionInforme.first().de = f.cleaned_data.get('de')
                        eConfiguracionInforme.first().recomendaciones = f.cleaned_data.get('recomendaciones')
                        eConfiguracionInforme.first().valor_x_hora = f.cleaned_data.get('valor_x_hora')
                        eConfiguracionInforme.first().save(request)
        ...
        if 'action' in request.GET:
            ...
            if action == 'contratacion':
                ...
                try:
                    ...
                    if tab == 2:
                        if persona.usuario.is_superuser or puede_realizar_accion_afirmativo(request, 'postulaciondip.puede_entrar_como_user_a_seleccionposgrado'):
                ...
                try:
                    ...
                    data['title'] = u"Firmar informes de contrataciónpor honorarios profesionales"
                    url_vars = f"&action={action}"
                    search = None
                    search = request.GET.get('searchinput', '')
                ...
                try:
                    ...
                    if 'paralelo_id' in request.GET:
                        ...
                        data['listadoinscritos'] = eInscripcionConvocatorias
                        #banco elegible
                    ...
                    eActaSeleccionDocente = ActaSeleccionDocente.objects.filter(filtro).annotate(
                            estado_order=Case(
...
def valida_choque_horario_en_actas_generadas(request: object, **kwargs: object) -> object:
    ...
    try:
        ...
        eInscripcionConvocatoria = kwargs.pop('InscripcionConvocatoria', None)
        ePersonalAContratars =PersonalAContratar.objects.filter(status=True, inscripcion__postulante__persona = eInscripcionConvocatoria.postulante.persona).exclude(actaparalelo__convocatoria__id = eInscripcionConvocatoria.convocatoria.id)
