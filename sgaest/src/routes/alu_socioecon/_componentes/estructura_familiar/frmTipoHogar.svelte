<script lang="ts">
	...
	import { getTipoHogar as loadDataTiposHogar } from '$lib/utils/loadDataApi';
	...
	onMount(async () => {
		...
		eTiposHogar = await loadDataTiposHogar();
		await delay(1000);
		...
		if (aData.eFichaSocioeconomica) {
			...
			if (eFichaSocioeconomica.tipohogar) {
				selectTipoHogar = eFichaSocioeconomica.tipohogar['pk'] ?? 0;
			}
		}
		await delay(2000);
		load = false;
	...
	const saveDatosEstructuraFamiliar = async () => {
		...
		await resetForms('none');
		const $frmTipoHogar = document.getElementById('frmTipoHogar');
		const formData = new FormData($frmTipoHogar);
		formData.append('tipohogar', selectTipoHogar.toString());
		formData.append('field', 'tipohogar');
...
>
	...
	<ModalBody>
		{#if !load}
			<form action="javascript:;" id="frmTipoHogar">
				<div class="row g-3">
					<div class="col-12">
						<label for="id_tipohogar" class="form-label fw-bold"
							><span><i class="fe fe-alert-octagon text-warning" /></span> Tipo de Hogar:</label
						>
						<select
							class="form-select form-select-sm"
							aria-label=""
							id="id_tipohogar"
							bind:value={selectTipoHogar}
						>
							<option value={0} selected> ----------- </option>
							{#each eTiposHogar as eTipo}
								{#if selectTipoHogar === eTipo.id}
						...
						<div class="valid-feedback" id="id_tipohogar_validate">¡Se ve bien!</div>
					</div>
				</div>
			</form>
		{:else}
