            ...
            try:
                ...
                for incidencia in listado:
                    ...
                    if not time(8, 0) < hora < time(17, 0):
                        hora = time(9, 0)
                    fecha = datetime.strptime(incidencia['fecha'], '%d-%m-%Y').date()
        ...
        if 'action' in request.GET:
            ...
            if action == 'viewcomentarios':
                try:
                    ...
                    data['id'] = id = request.GET['id']
                    coment = ComentarioIncidenciaSCRUM.objects.filter(incidencia=id)
                    if coment:
            ...
            if action == 'viewactsecundarias':
                try:
                    ...
                    data['id'] = id = request.GET['id']
                    secont = IncidenciaSecundariasSCRUM.objects.filter(incidencia=id)
                    if secont:
            ...
            try:
                ...
                data['listpendientes'] = listpendientes = qsbase.filter(estado=1).order_by('orden')
                data['listenproceso'] = listenproceso = qsbase.filter(estado=2).order_by('orden')
                listfinalizadas = qsbase.filter(estado=3).order_by('-id')
