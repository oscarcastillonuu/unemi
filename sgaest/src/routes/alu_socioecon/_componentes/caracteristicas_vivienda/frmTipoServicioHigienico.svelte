<script lang="ts">
	...
	import { getTiposServiciosHigienicos as loadDataTipos } from '$lib/utils/loadDataApi';
	...
	onMount(async () => {
		...
		if (aData.eFichaSocioeconomica) {
			...
			if (eFichaSocioeconomica.tiposervhig) {
				selectTipo = eFichaSocioeconomica.tiposervhig['pk'] ?? 0;
			}
		}
		await delay(2000);
		load = false;
	...
	const saveDatosCaracteristicaVivienda = async () => {
		...
		const formData = new FormData($frmTipo);
		formData.append('tiposervhig', selectTipo.toString());
		formData.append('field', 'tiposervhig');
...
>
	...
	<ModalBody>
		{#if !load}
			<form action="javascript:;" id="frmTipo">
				<div class="row g-3">
					<div class="col-12">
						<label for="id_tiposervhig" class="form-label fw-bold"
							><span><i class="fe fe-alert-octagon text-warning" /></span> El tipo de servicio higiénico
							con que cuenta el hogar es:
						</label>
						<select
						...
						<div class="valid-feedback" id="id_tiposervhig_validate">¡Se ve bien!</div>
					</div>
				</div>
			</form>
		{:else}
