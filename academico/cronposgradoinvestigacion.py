...
from investigacion.models import ProyectoInvestigacion, ProyectoInvestigacionRecorrido, ProyectoInvestigacionHistorialArchivo, RubricaEvaluacionItem, EvaluacionProyecto, EvaluacionProyectoDetalle, GrupoInvestigacionIntegrante, PublicacionOrcid, PublicacionScopus, ObraRelevancia
from sagest.commonviews import obtener_estado_solicitud
from sagest.models import Pago, PagoLiquidacion, Rubro, TipoOtroRubro, CuentaContable, ComprobanteAlumno
...
def eliminar_notificaciones_vencidas():
    ...
    fechaactual = datetime.now().date()
    # fechaactual = datetime.strptime('2023' + '-' + '09' + '-' + '01', '%Y-%m-%d').date()
    # print(fechaactual)
    # Consultar las notificaciones vencidas
    c = 0
    notificaciones = Notificacion.objects.filter(status=True, fecha_hora_visible__lt=fechaactual).order_by('id')
