<script lang="ts">
	...
	import { getCantidadesBanioDuchas as loadDataCantidades } from '$lib/utils/loadDataApi';
	...
	onMount(async () => {
		...
		if (aData.eFichaSocioeconomica) {
			...
			if (eFichaSocioeconomica.cantbannoducha) {
				selectCantidad = eFichaSocioeconomica.cantbannoducha['pk'] ?? 0;
			}
		}
		await delay(2000);
		load = false;
	...
	const saveDatosCaracteristicaVivienda = async () => {
		loading.setLoading(true, 'Guardando la información, espere por favor...');
		await resetForms('none');
		const $frmCantidad = document.getElementById('frmCantidad');
		const formData = new FormData($frmCantidad);
		formData.append('cantbannoducha', selectCantidad.toString());
		formData.append('field', 'cantbannoducha');
		formData.append('action', 'saveDatosCaracteristicaVivienda');
		...
		if (errors.length > 0) {
			...
			if (!res.isSuccess) {
				...
				dispatch('actionRun', { action: 'saveDatosCaracteristicaVivienda' });
			}
		}
	};
</script>
...
>
	...
	<ModalBody>
		{#if !load}
			<form action="javascript:;" id="frmCantidad">
				<div class="row g-3">
					<div class="col-12">
						<label for="id_cantbannoducha" class="form-label fw-bold"
							><span><i class="fe fe-alert-octagon text-warning" /></span> ¿Cuántos cuartos de baño con
							ducha tiene el hogar?
						</label>
						<select
							class="form-select form-select-sm"
							aria-label=""
							id="id_cantbannoducha"
							bind:value={selectCantidad}
						>
							<option value={0} selected> ----------- </option>
							{#each eCantidadades as eCantidad}
								{#if selectCantidad === eCantidad.id}
						...
						<div class="valid-feedback" id="id_cantbannoducha_validate">¡Se ve bien!</div>
					</div>
				</div>
			</form>
		{:else}
	...
	<ModalFooter>
		...
		{#if !load}
			<Button color="primary" class="rounded-3 btn-sm" on:click={saveDatosCaracteristicaVivienda}
				>Guardar</Button
			>
		{/if}
	</ModalFooter>
