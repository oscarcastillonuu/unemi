...
class NoticiasVinculacion(ModeloBase):
    ...
    def puede_cambiar_tipo_original_publicacion(self):
        if self.principal:
            noticias = NoticiasVinculacion.objects.filter(status=True, principal=True, publicado=True)
...
class CardInformativo(ModeloBase):
    departamentoservicio = models.ForeignKey(DepartamentoServicio, blank=True, null=True, on_delete=models.CASCADE,
                                              verbose_name='Departamento de servicio')
    titulo = models.CharField(default='', max_length=1000, verbose_name=u'Titulo principal del cuerpo de alerta')
    subtitulo = models.CharField(default='', max_length=1000, verbose_name=u'Subtitulo principal del cuerpo de alerta')
    imagen = models.FileField(upload_to='gestionvinculacion/noticias', blank=True, null=True,
                                verbose_name=u'Portada de noticia')
    fondo = models.TextField(default='', null=True, blank=True, verbose_name=u'Fondo')

    cuerpoinformativa = models.TextField(default='', blank=True, null=True, verbose_name=u'Descripción de la Alerta', max_length=1000)
    ...
    def nombre_input(self):
        return remover_caracteres_especiales_unicode(self.departamentoservicio.nombre).lower().replace(' ', '_')

    class Meta:
        verbose_name = u"Card Informativo"
        verbose_name_plural = u"Cards Informativos"
