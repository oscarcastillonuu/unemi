<script lang="ts">
	...
	const actionRun = async (event) => {
		...
		if (action == 'saveSolicitudTutorSoporteMateria') {
	...
	const loadSolicitudes = async (filterText: string) => {
		return new Promise(async (resolve, reject) => {
			...
			const [res, errors] = await apiGET(fetch, 'alumno/solicitud_tutor/pregrado', {
				search: filterText
			});
			if (errors.length > 0) {
	...
	const openModal = (eSolicitud, title) => {
		...
		<div class="row row-cols-1 row-cols-sm-2 row-cols-md-2 mb-3 g-1">
			<div class="col">
				<button
					...
					on:click={() => openModal(undefined, 'Adicionar registro de consulta')}
					><span class="fe fe-plus" /> Adicionar consulta a docente de mis materias</button
				>
			</div>
			<div class="col">
												...
												<div class="dropdown dropstart">
													...
													<div class="dropdown-menu" aria-labelledby="Dropdown1" style="">
														<a
															class="dropdown-item"
															href="javascript:;"
															on:click={() => openModal(eSolicitud, 'Editar registro de solicitud')}
														>
