...
from decorators import inhouse_check, secure_module, last_access, get_client_ip
            ...
            if action == 'nuevaleccion':
                with transaction.atomic():
                    try:
                        ...
                        if clases.values("id").exists():
                            aula = clases[0].aula
                            if ePeriodoAcademia.valida_asistencia_in_home:
                ...
                with transaction.atomic():
                    try:
                        ...
                        for leccion in lecciongrupo.mis_leciones():
                            leccion_id = leccion.id
                            eDetalleSilaboSemanalTemas = DetalleSilaboSemanalTema.objects.filter(silabosemanal__silabo__materia=leccion.clase.materia, temaunidadresultadoprogramaanalitico=temaunidadresultadoprogramaanalitico, silabosemanal__numsemana=numsemana, silabosemanal__semana=semana, silabosemanal__status=True,silabosemanal__silabo__status=True, status=True)
                            if eDetalleSilaboSemanalTemas.exists():
                                if not TemaAsistencia.objects.filter(leccion_id=leccion_id, tema=eDetalleSilaboSemanalTemas[0]).exists():
                                    tema = TemaAsistencia(leccion_id=leccion_id, tema=eDetalleSilaboSemanalTemas[0], fecha=datetime.now().date())
                                    tema.save(request)
                                    log(u'Seleccionó tema de la planificacion: %s' % tema, request, "add")
                                    # if tema.tema.subtemaunidadresultadoprogramaanalitico_set.filter(status=True).exists():
                                    #     for subtema in tema.tema.subtemaunidadresultadoprogramaanalitico_set.filter(status=True):
                                    #         sub=SubTemaAsistencia(tema_id=tema.id, subtema_id=subtema.id, fecha=datetime.now().date())
                                    #         sub.save(request)
                                    #         log(u'Seleccionó subtema de la planificación: %s' % sub, request, "add")

                                    # return JsonResponse({"result": "ok"})
                                else:
                                    listatemas = TemaAsistencia.objects.filter(leccion_id=leccion_id, tema=eDetalleSilaboSemanalTemas[0])
                                    ...
                                    # tema = TemaAsistencia.objects.get(leccion_id=leccion_id,tema=eDetalleSilaboSemanalTemas[0])
                                    # if tema.subtemaasistencia_set.filter(status=True).exists():
                                    #     for s in tema.subtemaasistencia_set.all():
                                    #         lista.append([s.subtema.id])
                                    #     subtema = tema.subtemaasistencia_set.all()
                ...
                with transaction.atomic():
                    try:
                        ...
                        for indice, leccion in enumerate(lecciongrupo.mis_leciones()):
                            eDetalleSilaboSemanalTemas = DetalleSilaboSemanalTema.objects.filter(silabosemanal__silabo__materia=leccion.clase.materia, temaunidadresultadoprogramaanalitico=temaunidadresultadoprogramaanalitico, silabosemanal__numsemana=numsemana, silabosemanal__semana=semana,silabosemanal__status=True, status=True)
                            if eDetalleSilaboSemanalTemas.exists():
                                ...
                                if not TemaAsistencia.objects.filter(leccion_id=leccion.id, tema=eDetalleSilaboSemanalTemas[0]).exists():
                                    tema = TemaAsistencia(leccion_id=leccion.id, tema=eDetalleSilaboSemanalTemas[0], fecha=datetime.now().date())
                                    tema.save(request)
                                    log(u'Seleccionó tema de la planificacion: %s' % tema, request, "add")
                                    stema = tema.tema.id if indice==0 else stema
                                    sub = SubTemaAsistencia(tema=tema, subtema=eDetalleSilaboSemanalSubTemas[0], fecha=datetime.now().date())
                                    sub.save(request)
                                    log(u'Seleccionó subtema de la planificacion: %s' % sub, request, "add")
                                else:
                                    tema = TemaAsistencia.objects.get(leccion_id=leccion.id, tema=eDetalleSilaboSemanalTemas[0])
                                    if not tema.subtemaasistencia_set.filter(subtema=eDetalleSilaboSemanalSubTema).exists():
                                        sub = SubTemaAsistencia(tema_id=tema.id, subtema=eDetalleSilaboSemanalSubTema, fecha=datetime.now().date())
                                        sub.save(request)
                                        log(u'Seleccionó subtema de la planificacion: %s' % sub, request, "add")
                                    else:
                                        sub = SubTemaAsistencia.objects.get(tema=tema, subtema=eDetalleSilaboSemanalSubTema)
                ...
                with transaction.atomic():
                    try:
                        ...
                        if materia.modeloevaluativo_id==27 and materia.asignaturamalla.transversal:
        ...
        if 'action' in request.GET:
            ...
            if action == 'view':
                try:
                    ...
                    data['DEBUG'] = DEBUG
                    data['autorizado_clase_virtual'] = profesor.solicitudaperturaclasevirtual_set.filter(status=True, estadosolicitud=2, periodo=periodo).exists()
                    data['silabosemanal'] = ss = SilaboSemanal.objects.filter(silabo__materia=materia, fechainiciosemana__lte=now, fechafinciosemana__gte=now, status=True, examen=False).first()
                    if variable_valor('VERSION_REGISTRO_ASISTENCIA') == 2:
                        max_day = Clase.objects.filter(activo=True, profesor=profesor, materia=materia, inicio__lte=now.date(), fin__gte=now.date()).order_by('inicio').aggregate(max_valor=Max('dia'))['max_valor']
                        data['ultimodiaclases'] = ultimodiaclases = (now.weekday() + 1) == max_day
                        data['temaregistrado'] = __valida_temas_marcados(silabosemanal=ss, lecciones=lecciones, periodo=periodo, ultimodiaclases=ultimodiaclases)
                        # ---------------------------------------------------------- leccion clase
                        data['temas'] = temas = TemaAsistencia.objects.filter(tema__silabosemanal=ss, leccion__clase__materia_id=materia.id, leccion__clase_id=clase,status=True).distinct().order_by('tema__temaunidadresultadoprogramaanalitico__orden')
                        data['subtemas'] = subt = SubTemaAsistencia.objects.filter(Q(tema__in=temas) | Q(subtema__silabosemanal=ss)).distinct().order_by('subtema__subtemaunidadresultadoprogramaanalitico__orden')
                        vizualizar = False
                        if temas:
                            fecha = temas.order_by('-id')[0].fecha
                            vizualizar = fecha.month == datetime.now().month and fecha.day == datetime.now().day
                        if subt:
                            fechasub = subt.order_by('-id')[0].fecha
                            vizualizar = fechasub.month == datetime.now().month and fechasub.day == datetime.now().day
                        data['vizualizar'] = vizualizar
            ...
            #         pass
                ...
                try:
                    ...
                    for cuentamarcadas in results:
                        ...
                        if clase.materia.modeloevaluativo_id == 27 and clase.materia.asignaturamalla.transversal:
                            modalidad = Modalidad.objects.get(id=3)
                        datajson = {
                            'clases': clase_ids,
...
def __valida_temas_marcados(**kwargs):
    try:
        now = datetime.now()
        ss, lecciones, periodo, ultimodiaclases = kwargs.get('silabosemanal'), kwargs.get('lecciones'), kwargs.get('periodo'), kwargs.get('ultimodiaclases')
        puederegistarrasistencia = True

        if ss and (ss.fechainiciosemana <= now.date() <= ss.fechafinciosemana) and variable_valor('VALIDA_REGISTRO_ASISTENCIA'):
            clase = lecciones[0].clase
            profesor, materia = clase.profesor, clase.materia
            __adicionales = True
            if ultimodiaclases:
                filtro = Q(subtemaunidadresultadoprogramaanalitico__status=True, subtemaunidadresultadoprogramaanalitico__temaunidadresultadoprogramaanalitico__isnull=False, subtemaunidadresultadoprogramaanalitico__temaunidadresultadoprogramaanalitico__status=True)
                subtemassemanales = ss.detallesilabosemanalsubtema_set.select_related('subtemaunidadresultadoprogramaanalitico').values_list('id').filter(filtro).order_by('subtemaunidadresultadoprogramaanalitico__orden')

                temas = TemaAsistencia.objects.values_list('id', flat=True).filter(tema__silabosemanal=ss, leccion__clase__materia_id=materia.id, leccion__clase_id=clase, status=True).distinct().order_by('tema__temaunidadresultadoprogramaanalitico__orden')
                if selectsubtemaadicional := ss.subtemaadicionalessilabo_set.filter(status=True).values('id').order_by('id'):
                    __adicionales = SubTemaAdicionalAsistencia.objects.filter(tema_id__in=temas).values_list('subtema', flat=True).distinct().count() == selectsubtemaadicional.__len__()

                marcados = SubTemaAsistencia.objects.values_list('subtema', flat=True).filter(tema_id__in=temas).distinct().order_by('subtema__subtemaunidadresultadoprogramaanalitico__orden').count()
                puederegistarrasistencia = (subtemassemanales.__len__() == marcados) and __adicionales
        return puederegistarrasistencia
    except Exception as ex:
        return True
