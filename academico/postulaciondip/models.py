...
class ActaSeleccionDocente(ModeloBase):
    ...
    def existe_director_de_posgrado(self):
        return True if self.comite.get_integrantes().filter(Q(status=True), Q(cargo__nombre__contains='DIRECTOR DE POSGRADO') | Q(cargo__nombre__contains='VICERRECTOR DE INVESTIGACIÓN Y POSGRADO')).exists() else False
    ...
    def existe_profesor_a_fin_programa(self):
        return True if self.comite.get_integrantes().filter(status=True).filter(Q(cargo__nombre__icontains='PROFESOR A FIN AL PROGRAMA')).exists() else False

    def existe_director_de_la_escuela_de_posgrado(self):
        return True if  self.comite.get_integrantes().filter(status=True).filter(Q(cargo__nombre__icontains='DIRECTOR DE LA ESCUELA') |Q(Q(cargo__nombre__icontains='DIRECTORA DE LA ESCUELA'))).exists() else False

    def get_director_de_posgrado(self):
        return self.comite.get_integrantes().filter(Q(status=True), Q(cargo__nombre__contains='DIRECTOR DE POSGRADO') | Q(cargo__nombre__contains='VICERRECTOR DE INVESTIGACIÓN Y POSGRADO')).first()

    def get_coordinador_de_programa_de_posgrado(self):
        return self.comite.get_integrantes().filter(status=True).filter(Q(cargo__nombre__icontains='coordinador del programa') | Q(cargo__nombre__icontains='coordinadora del programa') | Q(cargo__nombre__icontains='coordinador/a del programa')).first()

    def get_director_de_la_escuela_de_posgrado(self):
        return self.comite.get_integrantes().filter(status=True).filter(Q(cargo__nombre__icontains='DIRECTOR DE LA ESCUELA') |Q(Q(cargo__nombre__icontains='DIRECTORA DE LA ESCUELA'))).first()

    def get_profesor_a_fin_programa(self):
        return self.comite.get_integrantes().filter(status=True).filter(Q(cargo__nombre__icontains='PROFESOR A FIN AL PROGRAMA')).first()
    ...
    def realizo_votos_miembro_comite(self,actaparalelo, persona):
        try:
            ...
            if miembro:
                return True if miembro.get_votos_realizados(actaparalelo).exists() else False
            else:
                return False
        except Exception as ex:
            return False
...
class VotacionComiteAcademico(ModeloBase):
    ...
    tipo = models.ForeignKey(TipoPersonal, verbose_name=u'Tipo', on_delete=models.CASCADE)
    ...
    def __str__(self):
        return f"{self.inscripcion} - {self.tipo}"
...
class InformeContratacion(ModeloBase):
    ...
    def generar_texto_cuerpo_memo_contratacion(self):
        mensaje = f'<p style="text-align: justify">De mi consideración:</p>'
        mensaje += f'<p style="text-align: justify"> En virtud al “Convenio Específico de Cooperación, Coordinación y Desarrollo de Programas de Postgrado y Educación Continua, eventos científicos, culturales y académicos entre la Universidad Estatal de Milagro y la Empresa Pública de Producción y Desarrollo Estratégico de la Universidad Estatal de Milagro”, con la finalidad de continuar con el proceso de contratación '
        mensaje += f"en calidad de "
        for tipo_profesor in self.get_tipo_profesores():
            mensaje += f"{tipo_profesor}, "
