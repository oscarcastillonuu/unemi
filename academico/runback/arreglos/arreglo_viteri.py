...
#
# becatotal = BecaSolicitud.objects.values('inscripcion_id').filter(periodo_id=224,status=True)
# cantidad_limite_becados = BecaPeriodo.objects.get(periodo_id=224,status=True)
# cuentas = CuentaBancariaPersona.objects.values('persona_id').filter(estadorevision=2)
# becaincluir = BecaSolicitud.objects.values('inscripcion_id').filter(status=True,becaaceptada=2, inscripcion__persona_id__in=(cuentas)).exclude(periodo_id=224)
# cantidad_estudiantes_becados = BecaSolicitud.objects.values('id').filter(status=True,periodo_id=224).exclude(
#                     becaaceptada=3).count()
# preinscriones = PreInscripcionBeca.objects.filter( inscripcion_id__in=becaincluir,periodo_id=224).order_by('orden')
...
#         if not BecaSolicitud.objects.filter(status=True,periodo_id=224, periodocalifica_id=153,inscripcion=prei.inscripcion).exists():
...
#                         titulo=f"Solicitud de Beca en Revisión - {prei.periodo.nombre}",
...
#                         destinatario=prei.inscripcion.persona,
#                         url="/alu_becas",
#                         content_type=None,
#                         object_id=None,
#                         prioridad=2,
#                         perfil=prei.inscripcion.perfil_usuario(),
...
def actualizar_productos():
    gestiones = GestionPlanificacionTH.objects.filter(cabecera__periodo_id=5,status=True)
    actualizados=0
    nuevos=0
    for gestion in gestiones:
...
def migrar_cargos_contratos():
    ...
    for contrato in contratos:
        i+=1
        puesto = DenominacionPuesto.objects.filter(descripcion=contrato.cargo.upper().strip()).first()
        if puesto:
