<script lang="ts">
	...
	onMount(async () => {
		...
		inputCiudadela = ePersona.ciudadela ?? '';
	...
	const saveDatosPersonalesDomicilio = async () => {
		...
		formData.append('ciudadela', inputCiudadela);
		...
		if (errors.length > 0) {
			...
			if (!res.isSuccess) {
				...
				dispatch('actionRun', { action: 'saveDatosPersonalesDomicilio' });
			}
		}
	};
...
{#if ePersona}
	...
	>
			...
			{#if !load}
				<form action="javascript:;" id="frmDatosDomicilio">
					...
					<div class="row g-3 mt-1">
						...
						<div class="col-lg-6 col-md-12 col-12 mb-1">
							<label for="id_ciudadela" class="form-label fw-bold"
								><span><i class="fe fe-alert-octagon text-warning" /></span> Ciudadela:</label
							>
							<input
								type="text"
								class="form-control form-control-sm"
								id="id_ciudadela"
								bind:value={inputCiudadela}
							/>
							<div class="valid-feedback" id="id_ciudadela_validate">¡Se ve bien!</div>
						</div>
		...
		<ModalFooter>
			{#if !load}
				<Button color="warning" class="rounded-5 btn-sm" on:click={saveDatosPersonalesDomicilio}
