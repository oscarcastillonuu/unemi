...
from sagest.models import CapConfiguracionIpec, PersonaDepartamentoFirmas, Rubro
from settings import SITE_STORAGE, MEDIA_URL
from sga.funciones import numero_a_letras, null_to_decimal, null_to_numeric
...
def cronogramatituex(id):
    ...
    if solicitud.perfil.inscripcion.carrera.id in PerfilAccesoUsuario.objects.filter(status=True, grupo__name__exact='DIRECTOR DE LA ESCUELA DE POSGRADO DE EDUCACIÓN').values_list('carrera_id', flat=True):
        data['cargo'] = cargo = 'DIRECTOR DE LA ESCUELA DE POSGRADO DE EDUCACIÓN'
        data['director'] = Persona.objects.filter(usuario__groups__name=cargo).order_by('id').first()
        ...
        data['cargo'] = cargo = 'DIRECTOR DE LA ESCUELA DE POSGRADO DE CIENCIAS DE LA SALUD'
        data['director'] = Persona.objects.filter(usuario__groups__name=cargo).order_by('id').first()
        ...
        data['cargo'] = cargo = 'DIRECTOR DE LA ESCUELA DE POSGRADO DE NEGOCIOS Y DERECHO'
        data['director'] = Persona.objects.filter(usuario__groups__name=cargo).order_by('id').first()
    else:
        data['cargo'] = ''
        data['director'] = ''
...
def actaconsolidadafirma(idgraduado):
    ...
    nombreniveles = ['PRIMERO', 'SEGUNDO', 'TERCERO', 'CUARTO', 'QUINTO', 'SEXTO', 'SEPTIMO', 'OCTAVO', 'NOVENO']
    data['miultimonivelnombre'] = nombreniveles[int(ultimonivelmalla.id) - 1]
