...
def view(request):
    ...
    if request.method == 'POST':
        ...
        if action == 'addaprobacion_rechazar':
            ...
            try:
                f = PermisoInstitucionalFechaForm(request.POST)
                newfile = None
        ...
        if action == 'addsubirinforme':
            ...
            try:
                ...
                soli.estado = int(request.POST['estado'])
                soli.save(request)
        ...
        if 'action' in request.GET:
            ...
            if action =='justifiacionmarcadas':
                try:
                    ...
                    estado_ex = []
                    url_vars = ''
                    if request.user.user_permissions.filter(codename='puede_aprobar_justificacion_marcada_director').exists():
                        data['es_director_th'] = True
                        estado = [2,3,4,5]
                        estado_ex = [1]
                    elif request.user.user_permissions.filter(codename='puede_cerrar_justificacion_marcada_analista').exists():
                        data['es_analista_th'] = True
                        estado = [3,4,5]
                        estado_ex = [1,2]
                    solicitud =SolicitudJustificacionMarcada.objects.filter(status=True,estado__in = estado).exclude(estado__in=estado_ex)
                    if 's' in request.GET:
