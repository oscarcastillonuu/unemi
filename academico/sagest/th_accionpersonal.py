...
def view(request):
    ...
    if request.method == 'POST':
            ...
            try:
                ...
                if form.is_valid():
                    if AccionPersonal.objects.filter(anio=form.cleaned_data['anio']).exists():
                        numeromaximo = AccionPersonal.objects.filter(anio=form.cleaned_data['anio']).order_by('-numero')[0]
        ...
        #este add es para la accion de personal de vacaciones
            ...
            try:
                form = request.POST['nombre']
                filtro = MotivoAccionPersonal(nombre=form)
                filtro.save(request)
                log(u'Registro motivo: %s' % filtro, request, "add")
            ...
            try:
                ...
                if form.is_valid():
                    ...
                    for d in datos:
                        id = d['id']
                        if not (MotivoAccionPersonalDetalle.objects.filter(motivo=motivo,regimenlaboral_id=id).exists()):
