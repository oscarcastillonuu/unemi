<script lang="ts">
	...
	onMount(async () => {
		...
		if (aData.eFichaSocioeconomica) {
			...
			if (eFichaSocioeconomica.tratamientomedico) {
				inputTratamientoMedico = eFichaSocioeconomica.tratamientomedico ?? '';
			}
		}
		await delay(2000);
		load = false;
	...
	const saveDatosSaludEstudiante = async () => {
		...
		formData.append('tratamientomedico', inputTratamientoMedico);
		formData.append('field', 'tratamientomedico');
	...
	<ModalBody>
		{#if !load}
			<form action="javascript:;" id="frmEnfermedad">
				<div class="row g-3">
					<div class="col-12">
						<label for="id_tratamientomedico" class="form-label fw-bold"
							><span><i class="fe fe-alert-octagon text-warning" /></span> Tratamiento médico
						</label>
						<input
							type="text"
							class="form-control form-control-sm"
							aria-label=""
							id="id_tratamientomedico"
							bind:value={inputTratamientoMedico}
						/>

						<div class="valid-feedback" id="id_tratamientomedico_validate">¡Se ve bien!</div>
					</div>
				</div>
			</form>
		{:else}
