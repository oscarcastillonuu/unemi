    ...
    if request.method == 'POST':
        ...
        if action == 'addproyecto':
            ...
            try:
                ...
                owner, lider = configuracion.profesor.persona, configuracion.proyecto.lider()
                if evidencia := EvidenciaActividadDetalleDistributivo.objects.filter(criterio__criteriodocenciaperiodo__criterio=[151, 150][owner == lider], criterio__distributivo__profesor=configuracion.profesor, proyectovinculacion=configuracion.proyecto, desde=configuracion.fecha_inicio, hasta=configuracion.fecha_fin, generado=True, status=True).first():
                    log(f'Eliminó la evidencia de la actividad {evidencia.criterio.pk} del mes {evidencia.hasta.month} del año {evidencia.hasta.year}', request, "del")
                    evidencia.delete()
                return JsonResponse({"error": False})
            except Exception as ex:
                transaction.set_rollback(True)
            ...
            try:
                ...
                if not DetalleDistributivo.objects.values('id').filter(criteriodocenciaperiodo__criterio__pk=pk_criterio, distributivo__profesor__persona=solicitud.profesor.persona, distributivo__periodo=periodo, status=True).exists():
                    try:
                        notificacion2('Carga de informe mensual para proyecto de vinculación', f"Estimad{'a' if solicitud.profesor.persona.es_mujer() else 'o'} {solicitud.profesor.persona.nombres.split()[0]}, no cuenta con el criterio {criterio} en este periodo.", solicitud.profesor.persona, None, '', solicitud.pk, 1, 'sga', ConfiguracionInformeVinculacion)
                    except Exception as ex:
                        ...
                ...
                if not esLider:
                    if tecnicoasociado := solicitud.proyecto.get_tecnicoasociado():
                        notificacion2('Carga de informe mensual para proyecto de vinculación', f"Estimad{'a' if tecnicoasociado.persona.es_mujer() else 'o'} {tecnicoasociado.persona.nombres.split()[0].title()}, {'la' if solicitud.profesor.persona.es_mujer() else 'el'} docente {solicitud.profesor.persona.__str__().title()} promotor de proyecto de vinculación solicitó la revisión de su evidencia de avance del proyecto {solicitud.proyecto.nombre.__str__().title()}.", tecnicoasociado.persona, None, f'/programasvinculacion?action=configurarinforme_adm&id={solicitud.proyecto.pk}', solicitud.pk, 1, 'sga', ConfiguracionInformeVinculacion)

                # Migrar al módulo 'Mi Cronograma'
                migrar_evidencia_proyecto_vinculacion(request, solicitud)

                log(u'Solicito revisión de Informe: %s' % solicitud, request, "edit")
                return JsonResponse({'result': 'ok', "error": False, 'pk': solicitud.pk})
            except Exception as ex:
                transaction.set_rollback(True)
                return JsonResponse({"error": True, "mensaje": u"Error al guardar los datos."})
            ...
            try:
                ...
                if f.is_valid():
                    idinforme = request.POST['id']
                    informe = ConfiguracionInformeVinculacion.objects.get(pk=idinforme)
                    informe.aprobacion = f.cleaned_data['aprobacion']
        ...
        #         return JsonResponse({"result": True, "mensaje": "Intentelo más tarde."}, safe=False)
            ...
            try:
                ...
                try: datau = JavaFirmaEc(archivo_a_firmar=documento_a_firmar, archivo_certificado=bytes_certificado, extension_certificado=extension_certificado, password_certificado=passfirma, page=numpaginafirma, reason=razon, lx=posx, ly=posy).sign_and_get_content_bytes()
                except Exception as x:
                    try: datau, datas = firmar(request, passfirma, certificado, documento_a_firmar, numpaginafirma, posx, posy, 150, 45)
                    except Exception as y:
                        ...

                if not datau: raise NameError('Por favor asegúrese de que la contraseña sea correcta y vuelva a intentarlo más tarde.')
                ...
                if soli.aprobacion == 3: _, msj = migrar_evidencia_proyecto_vinculacion(request, soli, 4)
                log(u'Firmo Documento: {}'.format(name_documento_a_firmar), request, "add")
                return JsonResponse({'result': 'ok', 'id': soli.pk, 'proyecto': soli.proyecto.pk, 'para_revision': para_revision})
            except Exception as ex:
        ...
        if 'action' in request.GET:
            ...
            if action == 'firmainformevinculacion':
                ...
                try:
                    ...
                    data['configuracion'] = conf = ConfiguracionInformeVinculacion.objects.get(id=request.GET['id'])
                    lider = ParticipantesMatrices.objects.filter(status=True, proyecto=conf.proyecto,tipoparticipante__pk=1, profesor=profesor, activo = True).exists()
                    ...
                    if detalle.exists() or not detalleinfo.exists():
                        ...
                        # return HttpResponseRedirect("/proyectovinculaciondocente?action=generar&id="+str(conf.pk)+"&info=Debe completar la información del informe.")
                    if lider:
                        data['acciones'] = DetalleInformeVinculacion.objects.filter(status=True, configuracion__id=(request.GET['id']))
                    else:
                        data['acciones'] = DetalleInformeVinculacion.objects.filter(status=True, usuario_creacion=request.user, configuracion__id=(request.GET['id']))
                    ...
                    filename = f"/informe_{abreviaturanombre.lower()}_{datetime.now().month}" + '.pdf'

                    lider = conf.proyecto.lider()
                    nRevisor, cRevisor = lider, 'LÍDER DEL PROYECTO'

                    nombre, cargo = '', 'TÉCNICO DE VINCULACIÓN'
                    if persona == conf.proyecto.lider():
                        if ta := conf.proyecto.get_tecnicoasociado():
                            nombre, cargo = ta.persona, ta.cargo
                            data['tecnicoasociado'] = ta
                        nRevisor, cRevisor = nombre, cargo

                    data['nRevisor'] = nRevisor
                    data['cRevisor'] = cRevisor
                    if request.GET.get('json', None):
                        if convert_html_to_pdf('proyectovinculaciondocente/generarinformepdf.html', {'data': data}, filename, folder_pdf):
                            conf.archivo = 'informeProyectoVinculacion' + filename
                            conf.save(request)
...
def migrar_evidencia_proyecto_vinculacion(request, solicitud, estadoaprobacion=1, evidencia=None):
    try:
        ...
        if not evidencia:
            ...
            if criterio := DetalleDistributivo.objects.filter(criteriodocenciaperiodo__criterio=c, distributivo__profesor__persona=owner, distributivo__periodo=periodo, status=True).first():
                if temp := solicitud.migracionevidenciaactividad_set.filter(evidencia__proyectovinculacion=solicitud.proyecto, status=True).first():
                    evidencia = temp.evidencia
                else:
                    evidencia = solicitud.proyecto.evidenciaactividaddetalledistributivo_set.filter(criterio=criterio, desde=solicitud.fecha_inicio, hasta=solicitud.fecha_fin, status=True, generado=True).first()

                if not evidencia:
                    evidencia = EvidenciaActividadDetalleDistributivo(proyectovinculacion=solicitud.proyecto, criterio=criterio, desde=solicitud.fecha_inicio, hasta=solicitud.fecha_fin, actividad=f'INFORMAR SOBRE LAS ACTIVIDADES REALIZADAS EN EL MES DE {nombre_mes(solicitud.fecha_fin.month).upper()}', generado=True)
                ...
                evidencia.save(request)
                migracion and migracion.save(request)
                ...
                historial = HistorialAprobacionEvidenciaActividad(evidencia=evidencia, observacion=u'%s desde el SGA' % ('Firmado' if estadoaprobacion == 4 else 'Generado'), estadoaprobacion=estadoaprobacion, aprobacionpersona=persona if estadoaprobacion == 4 else None, fechaaprobacion=hoy if estadoaprobacion == 4 else None)
                historial.save(request)

                if estadoaprobacion == 1:
                    mes = solicitud.fecha_fin.month
                    cuerpo = f"Se informa que {'la' if owner.es_mujer() else 'el'} docente <b>{owner.__str__().title()}</b> registró la evidencia del proyecto de vinculación denominado <b>{solicitud.proyecto.nombre.__str__().title()}</b> correspondiente al avance del mes de <b>{nombre_mes(mes).lower()}</b>."
                    if not es_lider:
                        if tecnico := TecnicoAsociadoProyectoVinculacion.objects.filter(proyecto=solicitud.proyecto, activo=True, status=True).first():
                            notificacion2('Carga de informe mensual para proyecto de vinculación', cuerpo, tecnico.persona, None, f'/proyectovinculaciondocente?action=configurarinforme&id={solicitud.proyecto.pk}', evidencia.pk, 1, 'sga', EvidenciaActividadDetalleDistributivo)
                        ...
                        for revisor in UserCriterioRevisor.objects.filter(criteriodocenciaperiodo__criterio__id=pk_criterio, criteriodocenciaperiodo__periodo=periodo, status=True, rol=1):
                            notificacion2('Carga de informe mensual para proyecto de vinculación', cuerpo, revisor.persona, None, f'/adm_revisioncriteriosactividades?action=solicitudes&s={owner.nombre_completo_inverso()}&mes={mes}', evidencia.pk, 1, 'sga', EvidenciaActividadDetalleDistributivo)
            else:
                return False, '%s docente %s no cuenta con el criterio: <br><b>%s</b> en el periodo seleccionado, por favor verifique que el periodo <b>%s</b> sea el correcto.' % ('La' if owner.es_mujer() else 'El', owner.nombre_completo().__str__().title(), c.__str__().title(), periodo.nombre)
        else:
