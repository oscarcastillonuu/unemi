<script lang="ts">
	...
	import { getCantidadesVehiculosHogar as loadDataCantidades } from '$lib/utils/loadDataApi';
	...
	let selectCantidadVehiculoHogar = 0;
	...
	onMount(async () => {
		...
		eCantidadades = await loadDataCantidades();
		await delay(1000);
		...
		if (aData.eFichaSocioeconomica) {
			...
			if (eFichaSocioeconomica.cantvehiculos) {
				selectCantidadVehiculoHogar = eFichaSocioeconomica.cantvehiculos['pk'] ?? 0;
			}
		}
		await delay(2000);
		load = false;
	...
	const saveDatosPosesionBienes = async () => {
		loading.setLoading(true, 'Guardando la información, espere por favor...');
		await resetForms('none');
		const $frmCantidad = document.getElementById('frmCantidad');
		const formData = new FormData($frmCantidad);
		formData.append('cantvehiculos', selectCantidadVehiculoHogar.toString());
		formData.append('field', 'cantvehiculos');
		formData.append('action', 'saveDatosPosesionBienes');
		...
		if (errors.length > 0) {
			...
			if (!res.isSuccess) {
				...
				dispatch('actionRun', { action: 'saveDatosPosesionBienes' });
			}
		}
	};
</script>
...
>
	...
	<ModalBody>
		{#if !load}
			<form action="javascript:;" id="frmCantidad">
				<div class="row g-3">
					<div class="col-12">
						<label for="id_cantvehiculos" class="form-label fw-bold"
							><span><i class="fe fe-alert-octagon text-warning" /></span> ¿Cuántos Vehículos de uso
							exclusivo tiene el hogar?
						</label>
						<select
							class="form-select form-select-sm"
							aria-label=""
							id="id_cantvehiculos"
							bind:value={selectCantidadVehiculoHogar}
						>
							<option value={0} selected> ----------- </option>
							{#each eCantidadades as eCantidad}
								{#if selectCantidadVehiculoHogar === eCantidad.id}
									<option value={eCantidad.id} selected>
										{eCantidad.name}
									</option>
								{:else}
									<option value={eCantidad.id}>
										{eCantidad.name}
									</option>
								{/if}
							{/each}
						</select>
						<div class="valid-feedback" id="id_cantvehiculos_validate">¡Se ve bien!</div>
					</div>
				</div>
			</form>
		{:else}
	...
	<ModalFooter>
		...
		{#if !load}
			<Button color="primary" class="rounded-3 btn-sm" on:click={saveDatosPosesionBienes}
				>Guardar</Button
			>
		{/if}
	</ModalFooter>
