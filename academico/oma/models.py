        ...
        if self.idcursomoodle and cursor:
            ...
            for estudiante in self.inscripcioncurso_set.filter(status=True):
                try:
                    contador += 1
                    bandera = 0
                    persona = estudiante.inscripcion.persona
...
class InscripcionCurso(ModeloBase):
    ...
    inscripcion = models.ForeignKey("sga.Inscripcion", on_delete=models.CASCADE, verbose_name=u"Inscripcion")
    ...
    def lista_emails_envio_oma(self):
        ...
        if self.inscripcion.persona.emailinst:
            lista.append(self.inscripcion.persona.emailinst)
        return lista
                ...
                if existente_ofimatica:
                    ...
                    if not existente:
                        historico = HistoricoRecordAcademico(inscripcion=self.inscripcioncurso.inscripcion,
                                                             ...
                                                             observaciones='VALIDADO CURSO %s - MIGRACIÓN' % self.inscripcioncurso.curso)
                        historico.save()
...
class EvaluacionGenerica(ModeloBase):
    asignaturainscripcion = models.ForeignKey(AsignaturaInscripcionCurso, on_delete=models.CASCADE,
