        ...
        if 'action' in request.GET:
            ...
            if action == 'requerimientos':
                try:
                    ...
                    url_vars, filtro, search, prioridad = f'&action={action}&idp={request.GET["idp"]}', \
                                                          Q(status=True, periodo_id=idp, gestion__departamento__in=midepartamento_), \
            ...
            if action == 'buscarpersonas':
                try:
                    ...
                    s = q.split(" ")
                    qspersona = Persona.objects.filter(status=True, distributivopersona__isnull=False, administrativo__isnull=False).exclude(id__in=idsexcluidas).order_by('apellido1')
                    if len(s) == 1:
