...
from investigacion.funciones import diff_month, periodo_vigente_distributivo_docente_investigacion
from investigacion.models import ProyectoInvestigacion, TipoRecursoPresupuesto, ProyectoInvestigacionInstitucion, \
    ...
    ProyectoInvestigacionInformeCambioProblema, ProyectoInvestigacionInformeEquipamiento, ProyectoInvestigacionInformeIndicador, ProyectoInvestigacionInformeInstitucion
...
from sga.funciones import MiPaginador, log, variable_valor, \
    ...
    validar_archivo, null_to_decimal, cuenta_email_disponible_para_envio, fechaletra_corta, contar_palabras, elimina_tildes
    ...
    if request.method == 'POST':
        ...
        if action == 'addpropuestaproyecto':
            ...
            try:
                ...
                if f.is_valid():
                    if not ProyectoInvestigacion.objects.filter(titulo=f.cleaned_data['titulo'], status=True).exclude(estado__valor=44).exclude(pk=int(encrypt(request.POST['id']))).exists():
                        ...
                        if f.cleaned_data['montounemi'] > f.cleaned_data['montomaximounemi']:
                            diferencia = Decimal(f.cleaned_data['montounemi']).quantize(Decimal('.01')) - Decimal(f.cleaned_data['montomaximounemi']).quantize(Decimal('.01'))
                            if diferencia > 1000:
                                return JsonResponse({"result": "bad", "titulo": "Atención!!!", "mensaje": "El valor del campo Monto UNEMI puede exceder hasta en 1000 al valor del campo Máximo a financiar por la Unemi", "showSwal": "True", "swalType": "warning"})
                        ...
                        # if f.cleaned_data['compraequipo']:
                        #     proyectoinvestigacion.compraequipo = f.cleaned_data['compraequipo']
                        #     proyectoinvestigacion.tipo = f.cleaned_data['compraequipo']
            ...
            try:
                ...
                if proyectoinvestigacion.estado.valor != 2:
            ...
            try:
                ...
                if f.is_valid():
                    ...
                    # personaexterna.nombres = f.cleaned_data['nombres']
                    # personaexterna.apellido1 = f.cleaned_data['apellido1']
                    # personaexterna.apellido2 = f.cleaned_data['apellido2']
            ...
            try:
                ...
                if f.is_valid():
                    ...
                    proyecto.resumenpropuesta = request.POST['resumenpropuesta']
                    proyecto.formulacionproblema = request.POST['formulacionproblema']
                    proyecto.objetivogeneral = request.POST['objetivogeneral']
                    proyecto.justificacion = request.POST['justificacion']
                    proyecto.estadoarte = request.POST['estadoarte']
                    proyecto.metodologia = request.POST['metodologia']
            ...
            try:
                ...
                return JsonResponse({"result": "ok", "totalitems": totales["totalitems"], "totalrecurso": totales["totaldetalle"], "totalpresupuesto": proyecto.presupuesto, "totalequipos": totalequipos, "montominimoequipos": montominimoequipos, "tipoporcentaje": regfin.tipoporcentaje, "titulo": "Proceso exitoso!!!", "mensaje": u"Registro eliminado con éxito", "showSwal": True})
            except Exception as ex:
                msg = ex.__str__()
                transaction.set_rollback(True)
                return JsonResponse({"result": "bad", "titulo": "Error", "mensaje": u"Error al eliminar el registro. [%s]" % msg, "showSwal": "True", "swalType": "error"})
            ...
            try:
                ...
                # Si el estado del proyecto NO ES APROBADO
                if proyecto.estado.valor != 18:
                    proyecto.registrado = False
                    proyecto.verificado = None
                    proyecto.documentogenerado = False
                    proyecto.archivodocumentofirmado = None
            ...
            try:
                ...
                historialarchivo = ProyectoInvestigacionHistorialArchivo(
                    proyecto=proyecto,
                    tipo=8 if proyecto.estado.valor == 1 else 9,
            ...
            try:
                ...
                historialarchivo = ProyectoInvestigacionHistorialArchivo(
                    proyecto=proyecto,
                    tipo=11 if proyecto.estado.valor == 1 else 12,
            ...
            try:
                ...
                if f.is_valid():
                    ...
                    lista_email_envio.append('investigacion@unemi.edu.ec')
                    ...
                    log(u'%s finalizó edición de la propuesta de proyecto: %s' % (persona, proyecto), request, "edit")
                    return JsonResponse({"result": "ok", "titulo": "Proceso exitoso!!!", "mensaje": u"Registro guardado con éxito", "showSwal": True})
                else:
                    msg = ",".join([k + ': ' + v[0] for k, v in f.errors.items()])
                    return JsonResponse({"result": "bad", "titulo": "Error", "mensaje": u"Error al guardar los datos. [%s]" % msg, "showSwal": "True", "swalType": "error"})
            ...
            try:
                ...
                nombre = "informeavanceproyecto" if informe.tipo == 1 else "informefinalproyecto"
                ...
                return JsonResponse({"result": "ok", "titulo": "Proceso exitoso!!!", "mensaje": u"Documento firmado con éxito", "showSwal": True, "documento": informe.archivo.url})
            except Exception as ex:
                msg = ex.__str__()
                transaction.set_rollback(True)
            ...
            try:
                ...
                if proyecto:
                    ...
                    conclusion = request.POST['conclusion'].strip()
                    continuacion = request.POST['continuacion'].strip()
                    lineabase = request.POST['lineabase'].strip()
        ...
        if 'action' in request.GET:
            ...
            if action == 'mostrarrecorrido':
                ...
                try:
                    data['title'] = u'Editar Detalle al Presupuesto'
                ...
                try:
                    ...
                    if convocatoria.esta_abierta():
                        ...
                        # Si el docente a tiempo completo y tiene título de cuarto nivel
                        if profesor.dedicacion.id == 1 and profesor.persona.tiene_titulo_cuarto_nivel():
                            # si el profesor no está como director en otro proyecto
                            if not persona.proyectoinvestigacionintegrante_set.filter(status=True, funcion=1, proyecto__convocatoria=convocatoria).exists():
                                data['mostrarboton'] = True
                            else:
                                data['mostrarboton'] = False
                                # data['mensaje'] = "Atención!!! Usted consta como director de otro proyecto por lo que no podrá agregar más propuestas"
                                # data['clasemsg'] = "alert alert-warning"
                        else:
                            data['mostrarboton'] = False
                            data['mensaje'] = "Atención!!! La opción Agregar Propuesta de Proyecto está habilitada únicamente para docentes a TIEMPO COMPLETO y con título de CUARTO NIVEL"
                            data['clasemsg'] = "alert alert-warning"
                    else:
                        data['mostrarboton'] = False
                        data['mensaje'] = "Atención!!! El periodo de registro de propuestas de proyectos está cerrado"
                ...
                try:
                    ...
                    if 'idi' in request.GET:
                        ...
                        data['informe'] = informe.archivogenerado.url
                        data['tipoinforme'] = 'Informe Final de Proyecto de Investigación'
