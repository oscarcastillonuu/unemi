...
from sga.forms import BecaTipoForm, BecaUtilizacionForm, BecaRequisitosForm, BecaAprobarArchivoForm, \
    ...
    SubirArchivoRequisitoBecaForm, ImportarArchivoBecariosForm, DocumentoBecaTipoConfiguracionForm, FiltroBecadoValidacionForm
...
from sga.models import Inscripcion, BecaTipo, \
    ...
    DocumentoBecaTipoConfiguracion, FUNCIONES_DOCUMENTOSBECAS_EJECUTAR
...
def view(request):
    ...
    if request.method == 'POST':
        ...
        if action == 'listapagadoscorreo':
            ...
            try:
                if BecaAsignacion.objects.filter(solicitud__inscripcion__persona__personadocumentopersonal__isnull=True,
                                                 status=True, solicitud__periodo=periodo,
            ...
            try:
                ...
                if becassolicitudpago:
                    ...
                    for solicitudpago in becassolicitudpago:
                        ...
                        hojadestino.write(fila, 0, str(solicitudpago.solicitudpago.numerosolicitud).zfill(5),
                                          fuentenormal)
                        ...
                        hojadestino.write(fila, 6, str(matricula.nivel.coordinacion()) if matricula else '',
                                          fuentenormal)
                        hojadestino.write(fila, 7, str(matricula.inscripcion.carrera) if matricula else '',
                                          fuentenormal)
                        hojadestino.write(fila, 8, str(matricula.inscripcion.modalidad) if matricula else '',
                                          fuentenormal)
            ...
            try:
                ...
                if becassolicitudpago:
                    ...
                    for solicitudpago in becassolicitudpago:
                        ...
                        hojadestino.write(fila, 16, solicitudpago.fechapago if solicitudpago.fechapago else "",
                                          fuentefecha)
        ...
        if 'action' in request.GET:
            ...
            if action == 'mostrardocumentos':
                ...
                try:
                    ...
                    becas = BecaAsignacion.objects.filter(
                        solicitud__inscripcion__persona__personadocumentopersonal__isnull=True,
                        status=True, solicitud__periodo=periodo, tipo=1, cargadocumento=True).order_by('-solicitud_id')
                ...
                try:
                    ...
                    response = HttpResponse(content_type="application/ms-excel")
                    response[
                        'Content-Disposition'] = 'attachment; filename=becados_matriz_siies_caces_' + random.randint(1,
                                                                                                                     10000).__str__() + '.xls'

                    ws.write_merge(0, 0, 0, 16, 'UNIVERSIDAD ESTATAL DE MILAGRO', titulo)
                    ws.write_merge(1, 1, 0, 16, 'DIRECCIÓN DE BIENESTAR UNIVERSITARIO', titulo2)
                    ws.write_merge(2, 2, 0, 16, 'MATRIZ SIIES - CACES', titulo2)
                    ws.write_merge(3, 3, 0, 16, 'PERIODO ' + periodo.nombre, titulo2)

                    row_num = 4
                    columns = [
                        (u"CODIGO_IES", 3000),
                        (u"CODIGO_CARRERA", 3000),
                        (u"CIUDAD_CARRERA", 3000),
                        (u"CARRERA", 15000),
                        (u"TIPO_IDENTIFICACION", 3000),
                        (u"IDENTIFICACIÓN", 5000),
                        (u"NOMBRES COMPLETOS", 10000),
                        (u"CODIGO_BECA", 5000),
                        (u"ANIO", 3000),
                        (u"FECHA_INICIO_PERIODO", 5000),
                        (u"FECHA_FIN_PERIODO", 5000),
                        (u"TIPO_AYUDA", 5000),
                        (u"MOTIVO_BECA", 10000),
                        (u"OTRO_MOTIVO", 5000),
                        (u"MONTO_RECIBIDO", 3000),
                        (u"PORCENTAJE_VALOR_ARANCEL", 3000),
                        (u"PORCENTAJE_MANUTENCION", 3000),
                        (u"TIPO_FINANCIAMIENTO", 5000),
                        (u"PASAPORTE", 5000)
                    ]
                    for col_num in range(len(columns)):
                        ws.write(row_num, col_num, columns[col_num][0], fuentecabecera)
                    ...
                    if periodo.id >= 90 and periodo.id < 119:
                        nueva = BecaAsignacion.objects.filter(status=True, solicitud__periodo=periodo,
                                                              solicitud__becatipo_id__isnull=False,
                                                              estadorevisioncontrato=2, tipo=1)
                        renovacion = BecaAsignacion.objects.filter(status=True, solicitud__periodo=periodo,
                                                                   solicitud__becatipo_id__isnull=False, tipo=2)
                        becas = nueva | renovacion
                        becas = becas.order_by('solicitud__inscripcion__persona__apellido1',
                                               'solicitud__inscripcion__persona__apellido2',
                                               'solicitud__inscripcion__persona__nombres')
                    else:
                        becas = BecaAsignacion.objects.filter(status=True, solicitud__periodo=periodo,
                    ...
                    for s in becas:
                        ...
                        umat = s.solicitud.inscripcion.matricula_periodo_actual(periodo)
                        if umat:
                            umat = umat[0]

                        # MAYO A SEPTIEMBRE 2019 V2
                        if periodo.id == 85 and not umat:
                            # MAYO A SEPTIEMBRE 2019 EN LINEA
                            pmayosep2019online = Periodo.objects.get(pk=82)
                            umat = s.solicitud.inscripcion.matricula_periodo_actual(pmayosep2019online)
                            if umat:
                                umat = umat[0]
                        if umat:
                            ws.write(row_num, 0, "1024", fuentenumeroentero)
                            ws.write(row_num, 1, umat.inscripcion.mi_malla().codigo.upper() if umat else '', fuentenormal)
                            ws.write(row_num, 2, u"MILAGRO", fuentenormal)
                            ws.write(row_num, 3, umat.inscripcion.carrera.nombre, fuentenormal)
                            ws.write(row_num, 4, p.tipo_identificacion_completo(), fuentenormal)
                            ws.write(row_num, 5, p.identificacion(), fuentenormal)
                            ws.write(row_num, 6, p.nombre_completo_inverso(), fuentenormal)
                            ws.write(row_num, 7, str(umat.id) + "-" + str(s.solicitud.becatipo_id) if umat else '',
                                     fuentenormal)
                            ws.write(row_num, 8, periodo.anio if periodo.anio else 2019, fuentenumeroentero)
                            ws.write(row_num, 9, str(periodo.inicio)[:10], fuentefecha)
                            ws.write(row_num, 10, str(periodo.fin)[:10], fuentefecha)
                            ws.write(row_num, 11, 'BECA COMPLETA', fuentenormal)
                            ws.write(row_num, 12,
                                     s.solicitud.becatipo.nombrecaces.upper() if s.solicitud.becatipo.nombrecaces else "",
                                     fuentenormal)
                            ws.write(row_num, 13, "", fuentenormal)
                            ws.write(row_num, 14, s.montobeneficio, fuentenumerodecimal)
                            ws.write(row_num, 15, "", fuentenormal)
                            ws.write(row_num, 16, "", fuentenormal)
                            ws.write(row_num, 17, "RECURSOS FISCALES", fuentenormal)
                            ws.write(row_num, 18, p.pasaporte.__str__(), fuentenormal)
                    wb.save(response)
                    return response
                except Exception as ex:
                    print("Error...")
...
def generate_precandidatos_beca_desviacion_standar_todas_carrerasnew(request, data, periodoactual, periodoanterior):
    ...
    with transaction.atomic():
        try:
            lista_envio_masivo = generar_precandidatos_becas(periodoanterior, periodoactual, usuario_ejecuta_id=request.user.id,enviarcorreo=True)
