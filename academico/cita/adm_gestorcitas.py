...
def view(request):
    ...
    if request.method == 'POST':
            ...
            with transaction.atomic():
                try:
                    ...
                    if form.is_valid():
                        if id != 0:
                            instance = PersonaCitaAgendada.objects.get(pk=id)
                            gestion=GestionServicioCita(cita=instance, observacion=form.cleaned_data['observacion'])
                        elif ids != 0:
                            instance = SubCitaAgendada.objects.get(pk=ids)
                            gestion=GestionServicioCita(subcita=instance, observacion=form.cleaned_data['observacion'])
        ...
        if 'action' in request.GET:
            ...
            # Agendar cita por administrativos
                ...
                try:
                    ...
                    if 'idarea' in request.GET:
                        ...
                        servicios = ServicioConfigurado.objects.filter(status=True, mostrar=True, serviciocita__departamentoservicio_id=id)
                        for s in servicios:
                    ...
                    if usuario.is_superuser:
                        citas_emergentes = PersonaCitaAgendada.objects.filter(
                            Q(fechacita__gte=finicio) & Q(fechacita__lte=ffin) & Q(status=True) & Q(
                                escitaemergente=True) & Q(estado__in=estadoid))

                        citas_planificadas = PersonaCitaAgendada.objects.filter(
                            Q(fechacita__gte=finicio) & Q(fechacita__lte=ffin) & Q(status=True) & Q(
                                escitaemergente=False) & Q(estado__in=estadoid))
                    else:
                        citas_emergentes = PersonaCitaAgendada.objects.filter(
                            Q(fechacita__gte=finicio) & Q(fechacita__lte=ffin) & Q(status=True) & Q(
                                escitaemergente=True) & Q(persona_responsable=persona) & Q(estado__in=estadoid))
                        citas_planificadas = PersonaCitaAgendada.objects.filter(
                            Q(fechacita__gte=finicio) & Q(fechacita__lte=ffin) & Q(status=True) & Q(
                                escitaemergente=False) & Q(persona_responsable=persona) & Q(estado__in=estadoid))

                            # Realiza el conteo de estados en las listas de citas
                    contpendientes = len(citas_emergentes.filter(estado=0)) + len(citas_planificadas.filter(estado=0))
                    contreservados = len(citas_emergentes.filter(estado=1)) + len(citas_planificadas.filter(estado=1))
                    contanulados = len(citas_emergentes.filter(estado=2)) + len(citas_planificadas.filter(estado=2))
                    contfinalizados = len(citas_emergentes.filter(estado=5)) + len(citas_planificadas.filter(estado=5))
                    contcorregir = len(citas_emergentes.filter(estado=3)) + len(citas_planificadas.filter(estado=3))
                    contrechazado = len(citas_emergentes.filter(estado=4)) + len(citas_planificadas.filter(estado=4))
                    conttramite = len(citas_emergentes.filter(estado=6)) + len(citas_planificadas.filter(estado=6))
            ...
            #         # Obtén la instancia de PersonaCitaAgendada
            #         data['cita'] = cita = PersonaCitaAgendada.objects.get(id=int(encrypt(request.GET['id'])))
            #         data['estructura'] = estructura = EstructuraInforme.objects.filter(servicio = cita.servicio.serviciocita, activo=True).order_by('orden')
            #         # Filtra los historiales relacionados con la fecha de cita de la persona
            #         data['citas'] = citas = InformePsicologico.objects.filter(personacita__persona_id=cita.persona.id,status=True).order_by('fecha_creacion')
            #         data['tipoformulario'] = request.GET['tipoformulario']
            #         data['esderivada'] = esderivada = request.GET['derivada']
            #         data['proceso'] = proceso = cita.proceso()
            #
            #         data['asignados'] = asignados = SubCitaAgendada.objects.filter(persona_responsable=persona,
            #                                                                         status=True).values_list('citaprincipal__servicio__serviciocita__id', flat=True)
            #
            #         try:
            #             if esderivada == 'true':
            #                 personaderivada = SubCitaAgendada.objects.get(citaprincipal__persona=cita.persona, status=True)
            #                 data['cabeceraInforme'] = cabeceraInforme = InformePsicologico.objects.filter(
            #                     personacita=personaderivada.citaprincipal).latest('fecha_creacion')
            #             else:
            #                 data['cabeceraInforme'] = cabeceraInforme = InformePsicologico.objects.filter(
            #                 personacita=cita, status=True).latest('fecha_creacion')
            #         except ObjectDoesNotExist as ex:
            #             data['cabeceraInforme'] = cabeceraInforme = None
            #
            #         if cabeceraInforme:
            #             form = InformePsicologicoForm(initial=model_to_dict(cabeceraInforme))
            #             data['form'] = form
            ...
            #         form.fields['institucioneducativa'].queryset = InstitucionesColegio.objects.none()
            #
            #         ultimo_secuencial_procesoinforme = DetalleHistorialPsicologico.objects.filter(informe=cabeceraInforme).aggregate(Max('informe_id'))['informe_id__max']
            #         procesoinforme = DetalleHistorialPsicologico.objects.filter(informe = cabeceraInforme, informe_id = ultimo_secuencial_procesoinforme, status=True)
            #         data['procesoinforme'] = procesoinforme
            #
            #         ultimo_secuencial = DetalleInformePsicologico.objects.filter(informe=cabeceraInforme).aggregate(Max('informe_id'))['informe_id__max']
            #         data['detalleinforme'] = detalleinforme = DetalleInformePsicologico.objects.filter(informe = cabeceraInforme,
            #                                                                                            informe_id = ultimo_secuencial, status=True)
            #
            #         detalleestructura = []
            #         for x in estructura:
            ...
            #         subtituloProcesos = {}
            #
            #         if cita.servicio.serviciocita.id == 5:
            #
            #             for x in proceso:
            #                 matching_objects = [pi for pi in procesoinforme if pi.proceso.id == x.id]
            #                 marcada = False
            ...
            #             messages.error(request, 'Usted no tiene acceso a esta solicitud.')
            #             return redirect(request.path)
            #         data['navactivo'] = 'historialclinico'
            #         template='adm_agendamientocitas/modal/formhistorialcitas.html'
            #         return render(request,template,data)
            #     except Exception as ex:
            #         print('Error on line {}'.format(sys.exc_info()[-1].tb_lineno))
            #         pass
                ...
                try:
                    ...
                    if not persona == cita.persona_responsable and not usuario.is_superuser:
                        messages.error(request, 'Usted no tiene acceso a esta solicitud.')
                        return redirect(request.path)
                    data['proceso'] = proceso = cita.proceso()
                    request.session['viewactivo'] = 1
            ...
            try:
                ...
                responsable, estadocita, perfil, desde, hasta, search, filtro, url_vars = request.GET.get('responsable', ''), \
                                                                                          ...
                                                                                          request.GET.get('s', ''), Q(status=True), ''
                # if es_responsable and not request.user.has_perm('sga.puede_revisar_total_reservas') or responsable:
                if es_responsable and not request.user.has_perm('sga.puede_revisar_total_citas') or responsable:
                    ...
                    # horarios= HorarioServicioCita.objects.filter(status=True, responsableservicio__responsable_id=idpersona)
                    # servicios=[]
                    # for horario in horarios:
                    #     servicios.append(horario.responsableservicio.servicio)
                ...
                if search:
                    s = search.split(' ')
                    if len(s) == 1:
                        filtro = filtro & (Q(persona__nombres__icontains=search) | Q(persona__cedula__icontains=search) | Q(codigo__icontains=search))
                    if len(s) >= 2:
                ...
                data['responsableservicios'] = ResponsableServicioCita.objects.filter(status=True, activo=True).order_by('responsable_id').distinct('responsable_id')
                # CONTADOR
                ...
                if 'exportar_excel' in request.GET:
                    ...
                    columns = [u"N°", u"NOMBRES Y APELLIDOS", u"CÉDULA", u"PERFIL",
                               u"¿ASISTIO?", u"CARRERA", u"MODALIDAD", u"ESTADO", "RESPONSABLE", "SERVICIO"
                               ]
                    row_num = 3
                    ...
                    for list in listado:
                        ...
                        if list.asistio:
                            asistio='SI'
                        elif list.estado == 5 or list.estado == 6:
                            asistio='NO'
                        ws.cell(row=row_num, column=1, value=numero)
                        ws.cell(row=row_num, column=2, value=str(list.persona))
                        ws.cell(row=row_num, column=3, value=str(list.persona.cedula))
                        ws.cell(row=row_num, column=4, value=str(list.perfil))
                        ws.cell(row=row_num, column=5, value=asistio)
                        ws.cell(row=row_num, column=6, value=str(
                            list.perfil.inscripcion.carrera) if list.perfil.es_estudiante() else mensaje)
                        ws.cell(row=row_num, column=7, value=str(
                            list.perfil.inscripcion.modalidad) if list.perfil.es_estudiante() else mensaje)
                        ws.cell(row=row_num, column=8, value=list.get_estado_display())
                        ws.cell(row=row_num, column=9, value=list.persona_responsable)
                        row_num += 1
                    wb.save(response)
                    return response
