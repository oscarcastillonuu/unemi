...
from investigacion.funciones import coordinador_investigacion, tecnico_revisor_grupoinvestigacion, notificar_grupo_investigacion, vicerrector_investigacion_posgrado, experto_investigacion, secuencia_informe_grupoinvestigacion, responsable_coordinacion, FORMATOS_CELDAS_EXCEL, \
    notificar_revision_solicitud_produccion_cientifica
...
def view(request):
    ...
    if request.method == 'POST':
        ...
        if 'action' in request.GET:
            ...
            try:
                search, id, estado, buscarpor, idsp, filtro, url_vars = request.GET.get('s', ''), request.GET.get('id', ''), request.GET.get('estado', ''), request.GET.get('buscarpor', ''), request.GET.get('idsp', ''), Q(status=True), ''
                ...
                articulos = ArticuloInvestigacion.objects.select_related().filter(filtro).order_by('revista__nombre', '-fechapublicacion', 'numero', 'nombre')
