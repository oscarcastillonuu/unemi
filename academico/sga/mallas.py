...
from inno.models import ActaResponsabilidad
from inno.forms import ActaResponsabilidadForm
    ...
    if request.method == 'POST':
        ...
        if action == 'editintegracioncurricular':
            ...
            try:
                ...
                return JsonResponse({"result": "ok", 'idresultado': unidades.id, 'orden': unidades.orden, 'nombre': unidades.descripcion, 'id_resultadoaprendizaje':unidades.contenidoresultadoprogramaanalitico.id})
            except Exception as ex:
                return JsonResponse({"result": "bad", "mensaje": u"Error al guardar los datos."})
            ...
            try:
                ...
                unidades.save(request)
                log(u'Editó unidad de resultado de programa analitico: %s %s' % (unidades.contenidoresultadoprogramaanalitico.programaanaliticoasignatura.asignaturamalla.asignatura.nombre, unidades), request, "add")
                return JsonResponse({"result": "ok", 'idunidades': unidades.id})
            except Exception as ex:
                transaction.set_rollback(True)
                return JsonResponse({"result": "bad", "mensaje": u"Error al guardar los datos."})
        ...
        if 'action' in request.GET:
            ...
            if action == 'programanaliticoposgrado':
                ...
                try:
                    ...
                    data['asignaturamalla'] = asignaturamalla = AsignaturaMalla.objects.filter(malla__carrera__in=miscarreras).get(pk=int(encrypt(request.GET['id'])))
                    data['corequisitos'] = asignaturamalla.asignaturamallacorequisito_set.all().order_by('corequisito__asignatura')
                    puede_modificar_mallas = True
                    if not persona.usuario.is_superuser:
                        if not persona.usuario.has_perm('sga.puede_modificar_mallas'):
                            puede_modificar_mallas = False
