...
export const get: RequestHandler = async ({ request, url }) => {
    ...
    let eCantidadesCelularHogar = [];
    const params = {
        action: 'data',
        model: 'ProveedorServicio:tiposervicio=1',
        q: q
    }
    if (token) {
    ...
    if (res.status >= 400) {
        return {
            status: 400,
            body: eCantidadesCelularHogar.slice(0, 1000)
        }
    }
    if (!res.ok) {
        return {
    ...
    if (response.isSuccess) {
        //console.log(response.value.data.eCantidadesCelularHogar);
        response.data.results.forEach(element => {
            eCantidadesCelularHogar.push({ id: element.id, name: element.name })
        });
