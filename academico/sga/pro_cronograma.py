...
from investigacion.models import ProyectoInvestigacionIntegrante, BitacoraActividadDocente, HistorialBitacoraActividadDocente, DetalleBitacoraDocente, DepartamentoBitacora, PersonaBitacora, UserCriterioRevisor, GrupoInvestigacionIntegrante, ProyectoInvestigacion, GrupoInvestigacion, GrupoInvestigacionIntegrante
...
from inno.models import RegistroClaseTutoriaDocente, SolicitudTutoriaIndividual, HorarioTutoriaAcademica, \
    InformeMensualDocente, HistorialInforme, HistorialInformeMensual, TerminosCondicionesProfesorDistributivo, \
    TerminosCondiciones, SolicitudAperturaClaseVirtual, HorasInformeMensualDocente, AulaPlanificacionSedeVirtualExamen
...
from sga.funcionesxhtml2pdf import conviert_html_to_pdf, conviert_html_to_pdfsaveqrcertificado, conviert_html_to_pdfsave, conviert_html_to_pdfsaveqrinformepracticasmensual, convert_html_to_pdf, conviert_html_to_pdf_name
        ...
        if 'action' in request.POST:
            ...
            if action == 'asistenciaregistrada':
                try:
                    ...
                    return HttpResponse(json.dumps({'resp': False, 'mensaje': ex}))
            ...
            if action == 'conasignaturapreferenciaposgrado':
                ...
                try:
                    # actividaddetalledistributivo = ActividadDetalleDistributivo.objects.get(pk=request.POST['id'])
                    ...
                    if f.is_valid():
                        ...
                        if EvidenciaActividadDetalleDistributivo.objects.filter(criterio=criterio, hasta__month=f.cleaned_data['hasta'].month, hasta__year=f.cleaned_data['hasta'].year, status=True).exclude(criterio__criterioinvestigacionperiodo__criterio__id__in=(58,)).exists():
                        ...
                        # if f.cleaned_data['desde'] > actividaddetalledistributivo.hasta:
                        #     return JsonResponse({"result": "bad", "mensaje": u"Las Fechas deben estar entre %s - %s." % (actividaddetalledistributivo.desde, actividaddetalledistributivo.hasta)})
                        # if f.cleaned_data['hasta'] > actividaddetalledistributivo.hasta:
                        #     return JsonResponse({"result": "bad", "mensaje": u"Las Fechas deben estar entre %s - %s." % (actividaddetalledistributivo.desde, actividaddetalledistributivo.hasta)})
            ...
            if action == 'firmarinformepppinternadorotativo':
                ...
                try:
                    ...
                    try:
                        ...
                        except Exception as ex: raise NameError(f'Documento con inconsistencia en la firma. {ex.__str__()}.')
                ...
                try:
                    ...
                    if f.is_valid():
                        ...
                        if newfile:
                            ...
                            evidenciaaudi = EvidenciaActividadAudi(evidencia=evidenciaactividaddetalledistributivo, archivo=evidenciaactividaddetalledistributivo.archivo)
                            evidenciaaudi.save(request)
                            hostorial = HistorialAprobacionEvidenciaActividad(evidencia=evidenciaactividaddetalledistributivo, estadoaprobacion=1)
                            hostorial.save(request)

                        estadoevidencia = evidenciaactividaddetalledistributivo.get_estadoaprobacion_display()
                        ...
                        _evidencia = EvidenciaActividadDetalleDistributivo.objects.filter(criterio=evidenciaactividaddetalledistributivo.criterio, hasta__month=f.cleaned_data['hasta'].month, hasta__year=f.cleaned_data['hasta'].year).last()

                        historico = HistorialAprobacionEvidenciaActividad.objects.filter(evidencia=_evidencia, status = True).order_by('-id').first()

                        if historico.estadoaprobacion == 3:
                            HistorialAprobacionEvidenciaActividad(evidencia=_evidencia, aprobacionpersona=persona, fechaaprobacion=datetime.now().date(), observacion= '', estadoaprobacion=1).save()

                        evidenciaactividaddetalledistributivo.save(request)
                        log(u'Modifico Evidencia Profesor Distributivo Horas: %s' % evidenciaactividaddetalledistributivo, request, "edit")
                        return JsonResponse({"result": "ok", 'id_evidencia':evidenciaactividaddetalledistributivo.pk})
                    else:
                        raise NameError('Error')
                except Exception as ex:
            ...
            #         return JsonResponse({"result": "bad"})
                ...
                try:
                    ...
                    if int(request.POST['tipopersona']) == 1:
                        ...
                        return download_html_to_pdf('adm_criteriosactividadesdocente/informe_actividad_diaria_coordinador_posgrado.html', {'pagesize': 'A4','data': profesor.informe_actividades_mensual_coordinador_posgrado(periodo, fechainicio, fechafin)})
                except Exception as ex:
                ...
                try:
                    ...
                    if form.is_valid():
                        ...
                        for practica in practicasseleccionadas:
                            if form.cleaned_data['aplicarpractica']:
            ...
            if action == 'addbitacora':
                try:
                    ...
                    cabbitacora = BitacoraActividadDocente.objects.get(pk=int(encrypt(request.POST['id'])))
                    if fechapost >= cabbitacora.fechaini.date() and fechapost <= cabbitacora.fechafin.date():
                        if form.is_valid():
                            detallebitacora = DetalleBitacoraDocente(bitacoradocente_id=int(encrypt(request.POST['id'])),
                                                                     titulo=form.cleaned_data['titulo'],
                            ...
                            listadepartamento = form.cleaned_data['departamento']
                            for depa in listadepartamento:
                                departamentobitacora = DepartamentoBitacora(detallebitacora=detallebitacora,
                                                                            departamento=depa)
                                departamentobitacora.save(request)
                            listadopersona = form.cleaned_data['persona']
                            for lperso in listadopersona:
                                personabitacora = PersonaBitacora(detallebitacora=detallebitacora,
                                                                  persona=lperso)
                                personabitacora.save(request)
                            log(u'Adicionó bitacora de actividades docente %s' % (detallebitacora), request, "add")
                            return JsonResponse({"result": "ok"})
                    else:
                        return JsonResponse({"result": "bad", "mensaje": f"Favor ingresar una fecha que se encuentre en el rango desde {cabbitacora.fechaini.strftime('%d/%m/%Y')} hasta {cabbitacora.fechafin.strftime('%d/%m/%Y')}"})
                except Exception as ex:
                    transaction.set_rollback(True)
                    return JsonResponse({"result": "bad", "mensaje": u"Error al guardar los datos."})
                ...
                try:
                    ...
                    f = BitacoraActividadForm(request.POST)
                    hi = request.POST['hora'] if 'hora' in request.POST else ''
                    hf = request.POST['horafin'] if 'horafin' in request.POST else ''
                    ...
                    if f.is_valid():
                        listadepar = []
                        bitacoradocente.titulo = f.cleaned_data['titulo']
                        bitacoradocente.fecha = f.cleaned_data['fecha']
                        bitacoradocente.horainicio = f.cleaned_data['hora']
                        bitacoradocente.horafin = f.cleaned_data['horafin']
                        bitacoradocente.descripcion = f.cleaned_data['descripcion']
                        bitacoradocente.link = f.cleaned_data['link']
                        if newfile:
                            bitacoradocente.archivo = newfile
                        bitacoradocente.save(request)
                        listadepartamento = f.cleaned_data['departamento']
                        for depa in listadepartamento:
                            listadepar.append(depa.id)
                        ...
                        for depa in listadepartamento:
                            if not DepartamentoBitacora.objects.filter(detallebitacora=bitacoradocente, departamento=depa):
                                departamentobitacora = DepartamentoBitacora(detallebitacora=bitacoradocente,
                                                                            departamento=depa)
                                departamentobitacora.save(request)
                        return JsonResponse({"result": "ok"})
                    else:
            ...
            if action == 'generainformefirmar':
                try:
                    from pdip.models import ContratoDip, ContratoCarrera, ContratoAreaPrograma
                    fechaini = convertir_fecha(request.POST['fini'])
                    fechafin = convertir_fecha(request.POST['ffin'])
            ...
            #         pass
                ...
                try:
                    ...
                    for fecha in fechas_mensuales:
                        ...
                        __tienebitacoraperiodoactual = BitacoraActividadDocente.objects.values('id').filter(criterio=detalledistributivo, fechafin__month=ultimo_dia.month, fechafin__year=ultimo_dia.year, profesor=profesor, status=True).exists()
                        if primer_dia.date() <= datetime.now().date() and not __tienebitacoraperiodoactual:
                            mesbitacora = BitacoraActividadDocente(profesor=profesor, criterio=detalledistributivo, nombre='REGISTRO DE BITÁCORA MES DE: ' + MESES_CHOICES[ultimo_dia.month - 1][1].upper(), fechaini=primer_dia, fechafin=ultimo_dia)
                ...
                try:
                    ...
                    data['totalhorasplanificadas'] = totalhorasplanificadas = mesbitacora.horas_planificadas()
                ...
                try:
                    ...
                    form = BitacoraActividadForm(initial={'fecha': mesbitacora.fechaini.date()})
...
def migrar_evidencia_integrantes_proyecto_investigacion(**kwargs):
    try:
        # Co - director con permiso de subir evidencia
        ...
        if inv := evidencia.criterio.criterioinvestigacionperiodo:
            ...
            proyecto = ProyectoInvestigacion.objects.filter(profesor=evidencia.criterio.distributivo.profesor, estado_id=37, status=True).exclude(cerrado=True).first()

            if inv.criterio.id == 56:
                if integrante := ProyectoInvestigacionIntegrante.objects.filter(tiporegistro__in=[1, 3, 4], persona=evidencia.criterio.distributivo.profesor.persona, funcion=2, status=True).first():
                    if lider := integrante.proyecto.integrantes_proyecto().filter(funcion=1).first():
                        if not DetalleDistributivo.objects.values('id').filter(distributivo__activo=True, criterioinvestigacionperiodo__criterio__id=55, distributivo__profesor__persona=lider.persona, distributivo__periodo=evidencia.criterio.distributivo.periodo, status=True).exists():
                            can_upload_evidence, exclude_, proyecto = True, [1, 2], integrante.proyecto

            if inv.criterio.id == 55 or can_upload_evidence:
                if proyecto:
                    for integrante in proyecto.integrantes_proyecto().exclude(funcion__in=exclude_):
                        _evidencia = None
                    ...
                    messages.add_message(request, messages.WARNING, f'No tiene un proyecto de investigación asociado.')
...
def migrar_evidencia_integrante_grupo_investigacion(**kwargs):
    ...
    if evidencia.criterio.criterioinvestigacionperiodo:
        if evidencia.criterio.criterioinvestigacionperiodo.criterio.pk == CRITERIO_INTEGRANTE_DIRECTOR_GRUPO_INVESTIGACION:
            request, detalledistributivo = kwargs.pop('request'), evidencia.criterio
            periodo = detalledistributivo.distributivo.periodo

            try:
                if director := GrupoInvestigacionIntegrante.objects.filter(persona=detalledistributivo.distributivo.profesor.persona, grupo__vigente=True, grupo__status=True, funcion=1, status=True).first():
