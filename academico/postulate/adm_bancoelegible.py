        ...
        if 'action' in request.GET:
            ...
            if action == 'excel_postulantes__all_banco_habilitados':
                try:
                    ...
                    partidas = Partida.objects.values_list('id', flat=True).filter(status=True, convocatoria__ffin__year__range =[aniolimite, anioact],
                                                                                   carrera__coordinacion__id__in=mis_cordinaciones).order_by('convocatoria__descripcion')
                    ...
                    for det in listado:
                        ...
                        ws.write(row_num, 14, det.nota_final_disertacion, formatoceldacenter)
                        ws.write(row_num, 15, det.nota_final_entrevista, formatoceldacenter)
                        ws.write(row_num, 16, det.nota_final, formatoceldacenter)
                        ws.write(row_num, 17, det.revisado_por.username if det.revisado_por else '', formatoceldacenter)
                        ws.write(row_num, 18, det.fecha_revision, formatoceldafecha)
                        row_num += 1
                    workbook.close()
                    output.seek(0)
            ...
            try:
                ...
                eListaPersonaAplicarPartida= PersonaAplicarPartida.objects.filter(status=True,
                                                                                  partida__convocatoria__ffin__year__range=[aniolimite, anioact],
                                                                                  nota_final__gte=70,
                                                                                  partida__carrera__coordinacion__id__in=mis_cordinaciones,
                                                                                  esganador=False).exclude(persona_id__in=listadodistributivo)
