...
class PeriodoSerializer(Helper_ModelSerializer):
    ...
    class Meta:
        model = Periodo
        fields = ('id','idm','urlmoodle', 'clasificacion', 'urlmoodle2')
        #exclude = ['usuario_creacion', 'usuario_modificacion']
...
class MatriculaSerializer(Helper_ModelSerializer):
    ...
    def get_tiene_matricula_sede_examen(self, obj):
        return MatriculaSedeExamen.objects.values("id").filter(matricula=obj, detallemodeloevaluativo__alternativa__id__in=[20, 14]).exists()

    def get_esta_visible_horario_examen(self, obj):
        return MateriaAsignada.objects.values("id").filter(matricula=obj, visiblehorarioexamen=True).exists()
...
class MatriculaSedeExamenSerializer(Helper_ModelSerializer):
    ...
    def get_terminos_examenes(self, obj):
        ...
        if es_admision or es_pregrado:
            ePeriodoMatricula = obj.matricula.nivel.periodo.periodomatricula_set.filter(status=True, mostrar_terminos_examenes=True).first()
            if ePeriodoMatricula:
                return {'view': True,
                        'text': ePeriodoMatricula.terminos_examenes}
        return {'view': False,
                'text': None}
