<script lang="ts">
	...
	import { getMaterialesPisos as loadDataMateriales } from '$lib/utils/loadDataApi';
	...
	onMount(async () => {
		...
		if (aData.eFichaSocioeconomica) {
			...
			if (eFichaSocioeconomica.materialpiso) {
				selectMaterial = eFichaSocioeconomica.materialpiso['pk'] ?? 0;
			}
		}
		await delay(2000);
		load = false;
	...
	const saveDatosCaracteristicaVivienda = async () => {
		...
		await resetForms('none');
		const $frmMaterialPiso = document.getElementById('frmMaterialPiso');
		const formData = new FormData($frmMaterialPiso);
		formData.append('materialpiso', selectMaterial.toString());
		formData.append('field', 'materialpiso');
...
>
	...
	<ModalBody>
		{#if !load}
			<form action="javascript:;" id="frmMaterialPiso">
				<div class="row g-3">
					<div class="col-12">
						<label for="id_materialpiso" class="form-label fw-bold"
							><span><i class="fe fe-alert-octagon text-warning" /></span> Material Predominante en en el Piso del Hogar </label
						>
						<select
						...
						<div class="valid-feedback" id="id_materialpiso_validate">¡Se ve bien!</div>
					</div>
				</div>
			</form>
		{:else}
