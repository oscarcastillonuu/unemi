<script lang="ts">
	...
	onMount(async () => {
		...
		if (aData.eFichaSocioeconomica) {
			...
			if (eFichaSocioeconomica.horastareahogar) {
				inputHorasTareaHogar = eFichaSocioeconomica.horastareahogar ?? 0;
			}
		}
		await delay(2000);
		load = false;
	...
	const saveDatosActividadExtracurricular = async () => {
		...
		if (inputHorasTareaHogar != null) {
			formData.append('horastareahogar', inputHorasTareaHogar.toString());
		} else {
			formData.append('horastareahogar', '0');
		}
		formData.append('field', 'horastareahogar');
	...
	<ModalBody>
		{#if !load}
			<form action="javascript:;" id="frmActividad">
				<div class="row g-3">
					<div class="col-12">
						<label for="id_horastareahogar" class="form-label fw-bold"
							><span><i class="fe fe-alert-octagon text-warning" /></span> Horas de tareas en el hogar
						</label>
						<input
							type="number"
							class="form-control form-control-sm"
							aria-label=""
							id="id_horastareahogar"
							bind:value={inputHorasTareaHogar}
							on:keyup={() => numeric('id_horastareahogar', 0, 1000, 0)}
						/>

						<div class="valid-feedback" id="id_horastareahogar_validate">¡Se ve bien!</div>
					</div>
				</div>
			</form>
		{:else}
