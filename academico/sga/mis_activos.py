...
from sagest.models import ActivoFijo, SolicitudActivos, PrestamoActivosOperaciones, \
    ...
    Notificacionactivoresponsable, ConstatacionFisica, ActaConstatacion
    ...
    if request.method == 'POST':
        if 'action' in request.POST:
            ...
            if action == 'confmantenimiento':
                ...
                try:
                    id = int(request.POST['id'])
                    prestamo = PrestamoActivosOperaciones.objects.get(pk=id, status=True)
                    ...
                    return JsonResponse({"result": "bad", "mensaje": u'Error al devolver el activo'})
                ...
                try:
                    ...
                    seguimiento.save(request)
                    correccion = CorreccionesSolicitudTraspasoActivos(solicitudtraspaso=rechazasolicitud,
                                                                      observacion=request.POST['observacionrechazo'],
                                                                      estado=18)
                    correccion.save(request)
                    return JsonResponse({"result": "ok"})
                except Exception as ex:
        ...
        if 'action' in request.GET:
            ...
            if action == 'confmantenimiento':
                ...
                try:
                    solicitudtraspaso = SolicitudTraspasoActivos.objects.filter(id=int(request.GET['id'])).order_by(
                        '-id')
                ...
                try:
                    seguimientosolicitud = SeguimientoSolicitudTraspaso.objects.filter(
                        solicitudtraspaso_id=int(request.GET['id'])).order_by('-id')
            ...
            try:
                ...
                if search:
                    ...
                    # ss = search.split(' ')
                    # if len(ss) == 1:
                    #     activos = ActivoFijo.objects.filter((Q(codigogobierno=search) | Q(codigointerno=search)) & Q(responsable=persona) &
                    #                                         Q(statusactivo=1)).order_by('ubicacion', 'descripcion')
                    # else:
                ...
                return render(request, "mis_activos/view.html", data)
            except Exception as ex:
                pass
