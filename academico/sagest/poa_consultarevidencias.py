...
from sagest.models import AccionDocumentoDetalle, AccionDocumentoDetalleRecord, PeriodoPoa, Departamento, \
    AccionDocumento, InformeGenerado, UsuarioConsultaEvidencia, ObjetivoEstrategico
...
def view(request):
    ...
    if UsuarioConsultaEvidencia.objects.filter(userpermiso_id=persona.usuario_id,status=True).exists():
        a = 0
    else:
        ...
        if 'action' in request.GET:
            ...
            if action == 'consultadepartamento':
                try:
                    ...
                    if int(request.GET['idp']) < 4:
                        ids = UsuarioConsultaEvidencia.objects.values_list('unidadorganica').filter(userpermiso_id=persona.usuario_id,status=True)
            ...
            if action == 'consultadepartamentodos':
                ...
                try:
                    ...
                    if int(request.GET['idp']) < 4:
                        if UsuarioConsultaEvidencia.objects.filter(userpermiso_id=persona.usuario_id,
                                                                   status=True,
            ...
            periodo = PeriodoPoa.objects.filter(status=True)
