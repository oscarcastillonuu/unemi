        ...
        if 'action' in request.GET:
            ...
            # ACUERDOS COMPROMISO
                ...
                try:
                    ...
                    tipo, desde, hasta, search, filtros, url_vars = request.GET.get('tipo', ''), request.GET.get('desde', ''), request.GET.get('hasta', ''), request.GET.get('search', ''), Q(status=True), ''
                    if desde:
                        data['desde'] = desde
            ...
            #                              cambiando actividades
                ...
                try:
                    ...
                    columns = [
                        ...
                        (u"CARRERA", 15000),
                        (u"EMPRESA", 10000),
                        (u"REPRESENTANTE EMPRESA", 10000),
                        (u"CORREO EMPRESA", 10000),
                        (u"TELF. EMPRESA", 10000),
                        (u"TIPO INSTITUCIÓN", 10000),
                        (u"SECTOR ECONOMICO", 10000),
                    ...
                    for index,acuerdo in enumerate(listaacuerdo):
                        ...
                        ws.write(row_num, 2, acuerdo.empresa.nombre, font_style2)
                        ws.write(row_num, 3, acuerdo.empresa.representante, font_style2)
                        ws.write(row_num, 4, acuerdo.empresa.email, date_format)
                        ws.write(row_num, 5, acuerdo.empresa.telefonos, date_format)
                        ws.write(row_num, 6, acuerdo.empresa.get_tipoinstitucion_display(), date_format)
                        ws.write(row_num, 7, acuerdo.empresa.get_sectoreconomico_display(), date_format)
                        ws.write(row_num, 8, "SI" if acuerdo.para_practicas else "NO", font_style2)
                        ws.write(row_num, 9, "SI" if acuerdo.para_pasantias else "NO", font_style2)
                        ws.write(row_num, 10, acuerdo.totalvinculados(), font_style2)
                        ws.write(row_num, 11, acuerdo.fechaelaboracion, date_format)
                        ws.write(row_num, 12, acuerdo.fechainicio, date_format)
                        ws.write(row_num, 13, acuerdo.fechafinalizacion, date_format)
                        ws.write(row_num, 14, "SI" if acuerdo.archivo else "NO", font_style2)
                        ws.write(row_num, 15, "SI" if acuerdo.anulado else "NO", font_style2)
                        row_num += 1
                    wb.save(response)
                    return response
                except Exception as ex:
