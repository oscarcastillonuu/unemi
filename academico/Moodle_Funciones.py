        ...
        try:
            ...
            if tarea.archivotareapracticasilabo:
                ...
                if tarea.archivotareapracticasilabo:
                    ...
                    """ % (namea, namea, tarea.archivotareapracticasilabo.url, namea)
                if tarea.detalle:
                    detalletareapractica = """
    ...
    with transaction.atomic(using=cursor_verbose):
        try:
            ...
            if tarea.idtestmoodle == 0:
                ...
                sql = """INSERT INTO mooc_course_modules (course,module,instance,visible,visibleold,visibleoncoursepage,idnumber,groupmode,groupingid,completion,completiongradeitemnumber,completionview,completionexpected,availability,showdescription,added) VALUES('%s','17','%s','1','1','1','','0','0','2','0','0','0',NULL,'1','%s') """ % (
                    cursoid, instanceid, fecha)
                cursor.execute(sql)

                sql = """select id from mooc_course_modules WHERE course=%s AND module=17 and instance='%s' """ % (
                    cursoid, instanceid)
                cursor.execute(sql)
                buscar = cursor.fetchall()
        ...
        try:
            ...
            if foro.rubrica or foro.archivorubrica:
                ...
                if foro.archivorubrica:
                    ...
                    """ % (namea, namea, foro.archivorubrica.url, namea)
                intro = """%s
                    <p><h3 class="section-title" style="color: #3b5998;font-weight: bold;">Rúbrica:</h3></p>
                    <p>%s</p>
                    %s
            ...
            if foro.archivoforo:
                ...
                if foro.archivoforo:
                    ...
                    """ % (namea, namea, foro.archivoforo.url, namea)
                intro = """%s
