    ...
    if request.method == 'POST':
        ...
        if action == 'autorizarevidencia':
            ...
            try:
                ...
                if f.is_valid():
                    ...
                    if PreInscripcionPracticasPP.objects.filter(fechainicio=f.cleaned_data['fechainicio'], fechafin=f.cleaned_data['fechainicio'], status=True).exists():
                        return JsonResponse(
                            {"result": "bad", "mensaje": "Ya existe un registro con las misma fechas indicadas"})
                    if not f.cleaned_data['mensaje']:
                        return JsonResponse({"result": "bad", "mensaje": "Ingrese un mensaje"})
            ...
            try:
                ...
                if 'id' in request.POST:
                    ...
                    preinscripciones=pre.detallepreinscripcionpracticaspp_set.filter(status=True).order_by(
                        'inscripcion__persona__apellido1', 'inscripcion__persona__apellido2',
                        'inscripcion__persona__nombres')
                    ...
                    columns = [
                        ...
                        (u"ETNIA", 10000),
                        (u"ULTIMO NIVEL MATRICULA", 10000),
                    ]
                    row_num = 3
                    ...
                    if pre.preguntas():
                        for p in pre.preguntas():
                            ...
                            col_num = col_num + 1
                    row_num = 4
                    for pi in preinscripciones.distinct():
                        persona_ = pi.inscripcion.persona
                        ...
                        ws.write(row_num, 46, persona_.nacimiento, font_style)
                        ws.write(row_num, 47, persona_.edad(), font_style)
                        if perfil_.tienediscapacidad:
                            ws.write(row_num, 48, perfil_.tipodiscapacidad.nombre if perfil_.tipodiscapacidad else '', font_style)
                            ws.write(row_num, 49, perfil_.porcientodiscapacidad, font_style)
                            ws.write(row_num, 50, perfil_.carnetdiscapacidad, font_style)
                            ws.write(row_num, 51, perfil_.raza.__str__() if perfil_.raza else '', font_style)
                        else:
                            ws.write(row_num, 48, '', font_style)
                            ws.write(row_num, 49, '', font_style)
                            ws.write(row_num, 50, '', font_style)
                            ws.write(row_num, 51, '', font_style)
                        matricula_ = pi.inscripcion.matricula_set.filter(status=True, cerrada=False).order_by('-id').first()
                        if matricula_:
                            ws.write(row_num, 52, matricula_.nivelmalla.__str__(), font_style)
                        row_num += 1
                    wb.save(response)
                    return response
                else:
        ...
        if action == 'gestionar_preins_indmasivosalud':
            ...
            try:
                ...
                if not cartavinculacion.archivo:
        ...
        if action == 'cambiaritinerario':
            ...
            try:
                ...
                for pi in preinscripciones.exclude(archivo='',archivo__isnull=False).distinct():
                    ...
                    ws.write(row_num,11,'%s' % pi.convenioempresa if pi.convenioempresa else pi.otraempresaempleadora,font_style2)
                    ws.write(row_num,12,'%s' % pi.email if pi.email else "",font_style2)
                    ws.write(row_num,13,'%s' % pi.telefonoempresa if pi.telefonoempresa else "",font_style2)
                    ws.write(row_num,14,'%s' % pi.tipoinstitucion if pi.tipoinstitucion else "",font_style2)
                    ws.write(row_num,15,'%s' % pi.pais if pi.pais else "",font_style2)
                    ws.write(row_num,16,'%s' % pi.provincia if pi.provincia else "",font_style2)
                    ws.write(row_num,17,'%s' % pi.canton if pi.canton else "",font_style2)
                    ws.write(row_num,18,'%s' % pi.direccion if pi.direccion else "",font_style2)
                    ws.write(row_num,19,'%s' % pi.fecha_modificacion,font_style2)
                    ws.write(row_num,20,'%s' % pi.get_estado_display(),font_style2)
                    row_num += 1
                wb.save(response)
                return response
            except Exception as ex:
        ...
        if 'action' in request.GET:
            ...
            if action == 'cambiacarrerapractica_actividad':
                ...
                try:
                    ...
                    listaprofesor = [[x.profesor_id, f"{x.profesor.persona.cedula} - {x.profesor.persona.nombre_completo_inverso()} ({x.periodo.nombre}) - hor.({x.horas_docencia_segun_criterio_carrera(listaidcriterio, carrera_id)}) - asig.({x.profesor.contar_practicaspreprofesionales_asignadas_carrera(periodo, carrera_id, preinscripcionid)})"] for x in profesoresdistributivo]
