...
class RequerimientoPlanificacionAutomatizaForm(FormModeloBase):
    ...
    responsable = forms.ModelChoiceField(queryset=Persona.objects.select_related().filter(status=True, ), required=True, label=u'Responsable', widget=forms.Select(attrs={'col': '12'}))
