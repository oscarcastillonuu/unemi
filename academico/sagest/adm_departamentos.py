    ...
    if request.method == 'POST':
        ...
        if action == 'addproductos':
            try:
                ...
                if f.is_valid():
                    factor = ProductoServicioTh(nombre=f.cleaned_data['nombre'], tipo=f.cleaned_data['tipo'])
                    factor.save(request)
        ...
        if 'action' in request.GET:
            ...
            if action == 'adddepartamento':
                try:
                    ...
                    return render(request, "adm_departamentos/adddepartamento.html", data)
                except Exception as ex:
                    pass
            ...
            if action == 'edit':
                try:
                    ...
                    form = DepartamentoForm(initial={'nombre': departamento.nombre,
                                                     'permisogeneral': departamento.permisogeneral,
                                                     'codigoindice': departamento.codigoindice,
                                                     'permisodepartamento': departamento.permisodepartamento,
                                                     'tipoindice': departamento.tipoindice})
                    data['form'] = form
