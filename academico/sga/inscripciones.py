...
from sga.models import Persona, PersonaReligion, PersonaDocumentoPersonal, Inscripcion, ExamenComplexivo, \
    ...
    ComplexivoTematica, PerdidaGratuidad, DetalleModeloEvaluativo, SedeVirtual, Notificacion, Graduado, Egresado, PerfilUsuario
    ...
    if request.method == 'POST':
        ...
        if 'action' in request.GET:
            ...
            try:
                ...
                inscripciones = Inscripcion.objects.select_related().filter(filtros).exclude(carrera__coordinacion__id=9)
                paging = MiPaginador(inscripciones, 16)
                p = 1
                try:
                    paginasesion = 1
                    if 'paginador' in request.session:
