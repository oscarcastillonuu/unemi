...
from sagest.models import ObjetivoEstrategico, PeriodoPoa, Departamento, AccionDocumento, TIPO_ACCION, \
    MedioVerificacion, IndicadorPoa, ObjetivoOperativo, ObjetivoTactico, ProgramaPoa, DistributivoPersona, MetaPoa
...
def view(request):
    ...
    if request.method == 'POST':
        ...
        if action == 'consultaroperativo':
            try:
                operativo = ObjetivoOperativo.objects.get(pk=request.POST['id'])
                return JsonResponse({"result": "ok", 'idoperativo': operativo.id, 'orden': operativo.orden, 'nombre': operativo.descripcion, 'ponderacion': operativo.ponderacion})
            except Exception as ex:
                return JsonResponse({"result": "bad", "mensaje": u"Error al consultar los datos."})
        ...
        if action == 'editoperativos':
            try:
                ...
                operativos.ponderacion = request.POST['idponderacionoperativoedit']
        ...
        if 'action' in request.GET:
            ...
            if action == 'addestrategico':
                try:
                    ...
                    data['iddepa'] = int(request.GET['iddepa'])
            ...
            data['departamento'] = departamentoseleccionado = Departamento.objects.filter(pk__in=ObjetivoEstrategico.objects.values_list('departamento_id').filter(periodopoa=periodopoa, status=True), status=True).order_by('nombre')
            if 'iddepartamentopoa' in request.GET:
                if request.GET['iddepartamentopoa'] == '0':
            ...
            objetivos = ObjetivoEstrategico.objects.filter(departamento_id=departamentopoa,periodopoa=periodopoa,status=True).order_by('orden')
