<script lang="ts">
	...
	onMount(async () => {
		...
		if (aData.eFichaSocioeconomica) {
			...
			if (eFichaSocioeconomica.salubridadvida) {
				selectSalubridad = eFichaSocioeconomica.salubridadvida ?? 0;
			}
		}
		await delay(2000);
		load = false;
	...
	const saveDatosSaludEstudiante = async () => {
		...
		formData.append('salubridadvida', selectSalubridad.toString());
		formData.append('field', 'salubridadvida');
	...
	<ModalBody>
		{#if !load}
			<form action="javascript:;" id="frmEnfermedad">
				<div class="row g-3">
					<div class="col-12">
						<label for="id_salubridadvida" class="form-label fw-bold"
							><span><i class="fe fe-alert-octagon text-warning" /></span> Salubridad de las condiciones
							de vida
						</label>
						<select
							class="form-select form-select-sm"
							aria-label=""
							id="id_salubridadvida"
							bind:value={selectSalubridad}
						>
							<option value={0} selected> ----------- </option>
							{#each eTipos as eTipo}
								{#if selectSalubridad === eTipo.id}
						...
						<div class="valid-feedback" id="id_salubridadvida_validate">¡Se ve bien!</div>
					</div>
				</div>
			</form>
		{:else}
