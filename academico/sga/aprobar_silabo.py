...
from settings import ARCHIVO_TIPO_SYLLABUS, SITE_STORAGE
...
from sga.excelbackground import reporte_recurso_aprendizaje_background, reporte_cumplimiento_background, reporte_cumplimiento_background_v2
            ...
            if action == 'aprobarsilabo':
                ...
                try:
                    ...
                    noti = Notificacion(cuerpo='Generación de reporte de excel en progreso',
                                        ...
                                        en_proceso=True)
                    noti.save(request)
                    reporte_cumplimiento_background_v2(request=request, notiid=noti.pk, periodo=periodo, coordinacion=coordinacion, codcarrera=request.GET['cod_carrera'], finicio=finicio, ffin=ffin, tipo='pdf').start()
                    return JsonResponse({"result": True,
                                         "mensaje": u"El reporte se está realizando. Verifique su apartado de notificaciones después de unos minutos.",
                                         "btn_notificaciones": traerNotificaciones(request, data, persona)})
