<script lang="ts">
	...
	import { getCantidadesCelularesHogar as loadDataCantidades } from '$lib/utils/loadDataApi';
	...
	let selectCantidadCelularHogar = 0;
	...
	onMount(async () => {
		...
		if (aData.eFichaSocioeconomica) {
			...
			if (eFichaSocioeconomica.cantcelulares) {
				selectCantidadCelularHogar = eFichaSocioeconomica.cantcelulares['pk'] ?? 0;
			}
		}
		await delay(2000);
		load = false;
	...
	const saveDatosAccesoTecnologia = async () => {
		...
		formData.append('cantcelulares', selectCantidadCelularHogar.toString());
		formData.append('field', 'cantcelulares');
	...
	<ModalBody>
		{#if !load}
			<form action="javascript:;" id="frmCantidad">
				<div class="row g-3">
					<div class="col-12">
						<label for="id_cantcelulares" class="form-label fw-bold"
						...
						<select
							...
							aria-label=""
							id="id_cantcelulares"
							bind:value={selectCantidadCelularHogar}
						>
							<option value={0} selected> ----------- </option>
							{#each eCantidadades as eCantidad}
								{#if selectCantidadCelularHogar === eCantidad.id}
						...
						<div class="valid-feedback" id="id_cantcelulares_validate">¡Se ve bien!</div>
					</div>
				</div>
			</form>
		{:else}
