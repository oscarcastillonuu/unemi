    ...
    if request.method == 'POST':
        ...
        if 'action' in request.GET:
            ...
            try:
                ...
                becaasignaciones = BecaAsignacion.objects.filter(status=True,
                                                                 solicitud__becaaceptada=2,
                                                                 solicitud__inscripcion__persona__cuentabancariapersona__status=True,
                                                                 solicitud__inscripcion__persona__cuentabancariapersona__archivo__isnull=False,
                                                                 )#cargadocumento=True,
                cuentast = cuentas = CuentaBancariaPersona.objects.filter(status=True,
                                                                          activapago=True,
                                                                          persona_id__in=becaasignaciones.values_list('solicitud__inscripcion__persona_id', flat=True).distinct(),
                ...
                cuentast = cuentast.filter(persona__inscripcion__becasolicitud__periodo=becaperiodo.periodo, persona__inscripcion__becasolicitud__becaaceptada=2).distinct()
