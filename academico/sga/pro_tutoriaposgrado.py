                ...
                try:
                    ...
                    if grupointegrante.calpresidente > 0 and grupointegrante.calsecretaria > 0 and grupointegrante.caldelegado > 0:
                        if not CalificacionTitulacionPosgrado.objects.filter(
                                tematitulacionposgradomatricula=grupointegrante, confirmacalificacionrubricas=False,
                                status=True):
                            valorpuntaje = CalificacionTitulacionPosgrado.objects.filter(
                                tematitulacionposgradomatricula=grupointegrante, status=True)
                            calificacion = round(
                                null_to_numeric(valorpuntaje.aggregate(prom=Avg('puntajerubricas'))['prom']), 2)
