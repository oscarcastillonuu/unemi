    ...
    if request.method == 'POST':
        ...
        if action == 'metodoPago':
            ...
            try:
                ...
                if f.is_valid():
                    ...
                    filtro = SecuenciaMemoActividadPosgrado(secuencia=f.cleaned_data['secuencia'],anioejercicio= secuencia_memo().anioejercicio, tipo =tipo )
                    filtro.save(request)
            ...
            try:
                with transaction.atomic():
                    ...
                    instancia.status = False
                    instancia.save(request)
                    log(u'Elimino la actividad %s del perfil %s'%(instancia.actividad,instancia.perfil),request,'delete')
            ...
            try:
                with transaction.atomic():
                    ...
                    instancia.status = False
                    instancia.save(request)
                    log(u'Elimino informe de actividades y jornada laboral: %s %s' % (instancia,instancia.contrato.persona), request, "delete")
                    res_json = {"error": False}
            except Exception as ex:
                res_json = {'error': True, "message": "Error: {}".format(ex)}
            return JsonResponse(res_json, safe=False)
            ...
            try:
                ...
                if f.is_valid():
                    ...
                    if CampoContratoDip.objects.filter(identificador=(f.cleaned_data['identificador']).upper(),status=True).exists():
            ...
            try:
                ...
                if f.is_valid():
                    ...
                    if CampoContratoDip.objects.filter(identificador=(f.cleaned_data['identificador']).upper(),status=True).exclude(pk=int(request.POST['id'])).exists():
                        return JsonResponse({"result": "bad", "mensaje": "El identificador ya existe."})
            ...
            try:
                if 'id' in request.POST:
                    ...
                    return JsonResponse({"result": "ok", 'codigo':codpartida, 'fechapartida':res.fecha.strftime("%d/%m/%Y")})
            except Exception as ex:
            ...
            try:
                if 'cod' in request.POST:
                    if 'id' in request.POST:
                        res = ContratoDip.objects.values('id').filter(status=True, id =request.POST['id'],codigocontrato=request.POST['cod']).exists()
                        if res:
                            return JsonResponse({"result": "ok"})
                        return JsonResponse({"result": "bad",'mensaje':'El codigo esta en uso'})
                    else:
                        res = ContratoDip.objects.values('id').filter(status=True, codigocontrato=request.POST['cod']).exists()
                        if res:
                            return JsonResponse({"result": "bad"})
                        return JsonResponse({"result": "ok",'mensaje':'El codigo esta en uso'})
                if 'codpar' in request.POST:
                    if 'id' in request.POST:
                        res = ContratoDip.objects.values('id').filter(status=True, id =request.POST['id'],codigopartida=request.POST['codpar']).exists()
                        if res:
                            return JsonResponse({"result": "ok"})
                        return JsonResponse({"result": "bad",'mensaje':'El codigo esta en uso'})
                    else:
                        res = ContratoDip.objects.values('id').filter(status=True, codigopartida=request.POST['codpar']).exists()
                        if res:
                            return JsonResponse({"result": "ok"})
                        return JsonResponse({"result": "bad",'mensaje':'El codigo esta en uso'})
            except Exception as ex:
            ...
            try:
                ...
                if f.is_valid():
                    ...
                    for dep in f.cleaned_data['nombreareaprograma']:
                        if not ContratoAreaPrograma.objects.values('id').filter(status=True,contrato=filtro,gestion=dep).exists():
                            ...
                            areapr = ContratoAreaPrograma.objects.filter(status=True, contrato=filtro,gestion=dep).order_by('-id').first()
                            lis_excluid_area.append(areapr.id)
                    del_arpro = ContratoAreaPrograma.objects.filter(status=True,contrato=filtro).exclude(id__in=lis_excluid_area)
                    for del_ar in del_arpro:
                        del_ar.status=False
                        del_ar.save(request)
                    for carrera in f.cleaned_data['carrera']:
                        if not ContratoCarrera.objects.values('id').filter(status=True,contrato=filtro,carrera=carrera).exists():
                            ...
                            cc = ContratoCarrera.objects.filter(status=True, contrato=filtro, carrera=carrera).order_by('-id').first()
                            lis_excluid.append(cc.id)
                    con_carrera = ContratoCarrera.objects.filter(status=True,contrato=filtro).exclude(id__in=lis_excluid)
                    ...
                    filtro.save(request)
                    historial = HistorialContratoDip(contratodip=filtro, observacion=f.cleaned_data['descripcion'], estado=filtro.estado, persona=persona)
                    historial.save(request)
            ...
            try:
                with transaction.atomic():
                    ...
                    if DetalleCertificacionPresupuestariaDip.objects.values('id').filter(status=True, certificado = certificacion, contratodip=instancia).exists():
                        detalle = DetalleCertificacionPresupuestariaDip.objects.filter(status=True, certificado = certificacion, contratodip=instancia).order_by('-id').first()
            ...
            try:
                ...
                if form.is_valid():
                    if 'archivo' in request.FILES:
                        ...
                        if newfile.size > 100194304:
                            transaction.set_rollback(True)
                            return JsonResponse({"result": 'bad', "mensaje": u"Error, el tamaño del archivo es mayor a 10 Mb."}, safe=False)
                        # if not exte in ['pdf']:
                        #     transaction.set_rollback(True)
                        #     return JsonResponse({"result": 'bad', "mensaje": u"Error, solo archivo .pdf"}, safe=False)
                    contrato = ContratoDip(codigocontrato=form.cleaned_data['codigocontrato'],
                                           ...
                                           validadorgp_id=form.cleaned_data['validadorgp'] if form.cleaned_data['validadorgp'] else None,
                    ...
                    for dep in form.cleaned_data['nombreareaprograma']:
                        if not ContratoAreaPrograma.objects.values('id').filter(status=True,contrato=contrato,gestion=dep).exists():
                            areapr = ContratoAreaPrograma(
                                contrato=contrato,
                                gestion=dep
                            )
                    ...
                    for carrera in form.cleaned_data['carrera']:
                        if not ContratoCarrera.objects.values('id').filter(contrato=contrato, carrera=carrera, status=True).exists():
                ...
                return JsonResponse({"result": "ok"}, safe=False) #return JsonResponse({"result": "ok","id":contrato.id}, safe=False)
            except Exception as ex:
                transaction.set_rollback(True)
                ...
                if variable_valor('VALIDAR_CERTIFICACION_PRESUPUESTARIA_POS'):
                    if 'fecha' in request.POST:
                        if convertir_fecha(request.POST['fecha']) < datetime.now().date():
                            return JsonResponse({"result": True, "mensaje": "*La fecha ingresada debe ser mayor o igual a la fecha actual. %s" % datetime.now().date()}, safe=False)
                if f.is_valid():
            ...
            try:
                ...
                if f.is_valid():
                    eRecursoPresupuestarioPosgrado = RecursoPresupuestarioPosgrado(descripcion=f.cleaned_data['descripcion'])
            ...
            try:
                ...
                if f.is_valid():
                    eDetalleRecursoPresupuestarioPosgrado.desglosemoduloadictar = f.cleaned_data['desglosemoduloadictar']
            ...
            try:
                eDetalleRecursoPresupuestarioPosgrado = DetalleRecursoPresupuestarioPosgrado.objects.get(pk=int(request.POST['id']))
                log(u'Elimino  eDetalleRecursoPresupuestarioPosgrado: %s' % eDetalleRecursoPresupuestarioPosgrado, request, "del")
            ...
            try:
                eItemRecursoPresupuestarioPosgrado = ItemRecursoPresupuestarioPosgrado.objects.get(pk=int(request.POST['id']))
            ...
            try:
                eCabeceraRecursoPresupuestarioPosgrado = CabeceraRecursoPresupuestarioPosgrado.objects.get(pk=int(request.POST['id']))
                eCabeceraRecursoPresupuestarioPosgrado.status = False
                eCabeceraRecursoPresupuestarioPosgrado.save(request)
                log(u'Elimino  programa maestria recurso presupuestario: %s' % eCabeceraRecursoPresupuestarioPosgrado, request, "del")
            ...
            try:
                ...
                transaction.set_rollback(True)
                return JsonResponse({"result": True, "mensaje": f"{ex.__str__()}({sys.exc_info()[-1].tb_lineno})"}, safe=False)
            ...
            try:
                ...
                if not filtro.esta_en_contrato():
                    ...
                    return JsonResponse({"result": False, "mensaje": u"Certificacion eliminada correctamnete!",'error':False})
                transaction.set_rollback(True)
            ...
            try:
                ...
                if HorarioPlanificacionContrato.objects.filter(dia=dia, contrato=eConvocatoria, inicio=inicio, fin=fin, status=True).exists():
            ...
            try:
                ...
                if HorarioPlanificacionContrato.objects.filter(dia=dia, contrato=eConvocatoria, inicio=inicio, fin=fin, status=True).exclude(id=horario.id).exists():
                    raise NameError(f"Ya existe un horario registrado en este dia y fecha.")
                horario.dia = dia
            ...
            try:
                ...
                if not persona.usuario.is_superuser: log("Actualizo permiso para registrar actividades en bitácora: %s"%(registro.__str__()),request,'change')
            ...
            try:
                ...
                for cont in form.cleaned_data['personacontrato']:
                    if GrupoRevisionPagoContrato.objects.filter(status=True,personacontrato=cont).exclude(gruporevision=grupo).exists():
            ...
            try:
                ...
                if form.is_valid():
                    ...
                    log(f"Agrego configuracion requisito contratacion: {eContratacionConfiguracionRequisito}",request, 'add')
                res_json = {"result": True}
            ...
            try:
                ...
                if form.is_valid():
                    ...
                    log(f"Agrego  requisito contratacion: {eRequisitoContratacionConfiguracionRequisito}",request, 'add')
                res_json = {"result": True}
            ...
            try:
                ...
                log(f"Eliminó configuracion de requisitos contratacion: {eContratacionConfiguracionRequisito}", request, 'del')
                res_json = {"error": False}
            except Exception as ex:
                err_ = "{}({})".format(ex, sys.exc_info()[-1].tb_lineno)
                transaction.set_rollback(True)
            ...
            try:
                id = int(encrypt(request.POST['id']))
                eRequisitoContratacionConfiguracionRequisito = RequisitoContratacionConfiguracionRequisito.objects.get(status=True, id=id)
            ...
            try:
                ...
                if not form.is_valid():
                    raise NameError(f'{[{k:v[0]} for k,v in form.errors.items()]}')
                if GrupoRevisionPago.objects.filter(status=True,persona=form.cleaned_data['persona']).exclude(id=grupo.id):
                    raise NameError("Ya existe un grupo con esa persona")
                ...
                for cont in form.cleaned_data['personacontrato']:
                    if GrupoRevisionPagoContrato.objects.filter(status=True,personacontrato=cont).exclude(gruporevision=grupo).exists():
                        raise NameError(f"La persona {cont}, se encuentra ya en grupo de revisión")
                    if GrupoRevisionPagoContrato.objects.filter(status=True,personacontrato=cont,gruporevision=grupo).exists():
                        rev = GrupoRevisionPagoContrato.objects.filter(status=True,personacontrato=cont,gruporevision=grupo).order_by('-id').first()
                    else:
                        rev = GrupoRevisionPagoContrato(
                            gruporevision=grupo,
                            personacontrato=cont
                ...
                grupocontr_borrar = GrupoRevisionPagoContrato.objects.filter(status=True,gruporevision=grupo).exclude(id__in=lis_excl)
            ...
            try:
                id = request.POST.get('id', None)
                eContratacionConfiguracionRequisito = ContratacionConfiguracionRequisito.objects.get(status=True, id=int(encrypt(id)))
                form = ContratacionConfiguracionRequisitoForm(request.POST)
                if form.is_valid():
                    ...
                    eContratacionConfiguracionRequisito.save(request)
                    log(f"Editó eContratacionConfiguracionRequisito de revisión: {eContratacionConfiguracionRequisito}", request, 'change')
                res_json = {"result": True}
            except Exception as ex:
                err_ = "Error: {}. En la linea {}".format(ex, sys.exc_info()[-1].tb_lineno)
                transaction.set_rollback(True)
            ...
            try:
                id = request.POST.get('id', None)
                eRequisitoContratacionConfiguracionRequisito = RequisitoContratacionConfiguracionRequisito.objects.get(status=True, id=int(encrypt(id)))
                form = ContratacionGestionRequisitoForm(request.POST)
                if form.is_valid():
                    ...
                    eRequisitoContratacionConfiguracionRequisito.save(request)
                    log(f"Editó eRequisitoContratacionConfiguracionRequisito de revisión: {eRequisitoContratacionConfiguracionRequisito}", request, 'change')
                res_json = {"result": True}
            except Exception as ex:
                err_ = "Error: {}. En la linea {}".format(ex, sys.exc_info()[-1].tb_lineno)
                transaction.set_rollback(True)
                    ...
                    if newfile:
                        if newfile.size > 6291456:
                            ...
                            if ext == '.pdf':
                                ...
                                _name = remover_caracteres_tildes_unicode( remover_caracteres_especiales_unicode(_name)).lower().replace(' ', '_').replace('-',
                                                                                                                    '_')
        ...
        if 'action' in request.GET:
            ...
            if action == 'addperfil':
                ...
                try:
                    ...
                    if 'idperfil' in request.GET:
                        ...
                        for act in actividades:
                            if not ActividadesContratoPerfil.objects.filter(status=True,perfil_id = idp,actividad_id=act).exists():
                                acti = ActividadesContratoPerfil(
                                    perfil_id=idp,
                                    actividad_id = act
                                )
                                acti.save(request)
                                actividades2.append({'actividad':acti.actividad.id,'perfil':acti.perfil.id,'id':acti.pk,'descripcion':acti.actividad.descripcion})
                                log('Adiciono la actividad %s al cargo %s'%(acti.actividad.descripcion,acti.perfil),request,'add')
                    if 'idcontrato' in request.GET:
                        ...
                        contrato = ContratoDip.objects.get(status=True,pk = idcon)
                        for act in actividades:
                            if not ActividadesContratoPerfil.objects.filter(status=True,contrato = contrato,perfil=contrato.cargo,actividad_id=act).exists():
                                acti = ActividadesContratoPerfil(
                                    contrato_id = idcon,
                                    actividad_id = act
                                )
                                acti.save(request)
                                actividades2.append({'actividad': acti.actividad.id, 'contrato': acti.contrato.id, 'id': acti.pk, 'descripcion': acti.actividad.descripcion})
                                log('Adiciono la actividad %s al contrato %s' % (acti.actividad.descripcion, acti.contrato), request, 'add')
                ...
                try:
                    if not(HistorialPagoMes.objects.values('id').filter(status=True,contrato_id=int(request.GET['id']),fecha_pago__month=int(request.GET['desde']),fecha_pago__year=int(request.GET['anio'])).exists()):
                        historial = HistorialPagoMes(
                            contrato_id=int(request.GET['id']),
                            fecha_pago=date(year=int(request.GET['anio']), month=int(request.GET['desde']), day=date.today().day),
                            cancelado=False
                        )
                        historial.save(request)
                        log('Agrega historial de pago %s' % (historial), request, 'add')
                    else:
                        historial = HistorialPagoMes.objects.get(status=True,contrato_id=int(request.GET['id']),fecha_pago__month=int(request.GET['desde']),fecha_pago__year=int(request.GET['anio']))
                    ids = request.GET['id']
                    desde = request.GET['desde']
                    contratopersona = ContratoDip.objects.filter(pk=int(ids))[0]
                    cumplehoras = TrabajadorDiaJornada.objects.filter(persona = contratopersona.persona, mes=int(desde),status=True)

                    if desde == '':
                        return JsonResponse({'result': False, 'mensaje': 'Debe ingresar una fecha válida'})
                    if MemoActividadPosgrado.objects.filter(status=True,contrato =contratopersona,mes = int(desde) ).exists():
                        return JsonResponse({'result':False,'mensaje':'Ya existe el registro con el mes de %s'%(MESES_CHOICES[int(desde)-1][1])})
                    if PlantillaInformes.objects.values('id').filter(tipo= 0,vigente=True,anio=2022,status=True).exists():
                    ...
                    #desde = date(desde)
                    secpos = SecuenciaMemoActividadPosgrado.objects.filter(tipo = PlantillaContratoDip.objects.last(),status=True,
                    ...
                    filename_contrato = os.path.join(direccion_contrato, nombre_contrato)
                    memos = MemoActividadPosgrado(secuencia=memo, secuenciamemo=secpos, contrato=contratopersona,mes=int(desde),historialpago=historial)
                    # guarda la direccion
                    cantidad_parrafo = 0
                    document = Document(nombre_plantilla)
                    parrafo = document.paragraphs
                    ...
                    fecha = str(date.today().day) +" de "+str(MESES_CHOICES[int(date.today().month)-1][1]) +" del "+str(date.today().year)
                ...
                try:
                    ...
                    cumplehoras = TrabajadorDiaJornada.objects.filter(persona=contratopersona.persona, mes=int(desde),
                                                                      status=True)
                    if InformeActividadJornada.objects.values('id').filter(mes = int(desde),status=True,contrato=contratopersona).exists():
                        return JsonResponse({'result':False,'mensaje':'Ya existe el registro con el mes de %s'%(MESES_CHOICES[int(desde)-1][1])})
                    ...
                    secpos = SecuenciaMemoActividadPosgrado.objects.filter(tipo=PlantillaContratoDip.objects.last(), status=True,
                                                                  anioejercicio__anioejercicio=datetime.now().year)[0]
                    informe = InformeActividadJornada(mes=int(desde),contrato=contratopersona,secuenciageneral=secpos,secuencia=secinforme,historialpago=historial)
                    #data = extraervalores(contratopersona.persona.id,request.GET['desde'],request.GET['anio'],True if 'h' in request.GET else False)
                    data['bitacora'] = BitacoraActividadDiaria.objects.filter(status=True,persona =contratopersona.persona,fecha__month = int(desde) )
                ...
                try:
                    ...
                    fechas = TrabajadorDiaJornada.objects.values_list('fecha').filter(persona=contratopersona.persona, anio=2022, mes=int(desde), status=True).order_by('fecha')
                    ...
                    if InformeTecnico.objects.values('id').filter(mes = int(desde),status=True,contrato=contratopersona).exists():
                        return JsonResponse({'result':False,'mensaje':'Ya existe el registro con el mes de %s'%(MESES_CHOICES[int(desde)-1][1])})
                    if not InformeActividadJornada.objects.values('id').filter(mes = int(desde),status=True,contrato=contratopersona).exists():
                        return JsonResponse({'result':False,'mensaje':'No ha generado el reporte de asistencia y actividades'})
                    if not(HistorialPagoMes.objects.values('id').filter(status=True,contrato_id=int(request.GET['id']),fecha_pago__month=int(request.GET['desde']),fecha_pago__year=int(request.GET['anio'])).exists()):
                        historial = HistorialPagoMes(
                            contrato_id=int(request.GET['id']),
                            fecha_pago=date(year=int(request.GET['anio']), month=int(request.GET['desde']), day=date.today().day),
                        ...
                        historial = HistorialPagoMes.objects.get(status=True,contrato_id=int(request.GET['id']),fecha_pago__month=int(request.GET['desde']),fecha_pago__year=int(request.GET['anio']))
                    data['info'] = InformeActividadJornada.objects.filter(mes=int(desde), status=True, contrato=contratopersona).last()
                    ...
                    secpos = SecuenciaMemoActividadPosgrado.objects.filter(tipo=PlantillaContratoDip.objects.last(), status=True,
                                                                  anioejercicio__anioejercicio=datetime.now().year)[0]
                    informe = InformeTecnico(mes=int(desde),contrato=contratopersona,secuenciageneral=secpos,secuencia=secinforme,historialpago=historial)
                    # data = extraervalores(contratopersona.persona.id,request.GET['desde'],request.GET['anio'],True if 'h' in request.GET else False)
                ...
                try:
                    ...
                    cumplehoras = TrabajadorDiaJornada.objects.filter(persona=contratopersona.persona, mes=int(desde),
                                                                      status=True)
                    if not InformeTecnico.objects.values('id').filter(mes = int(desde),status=True,contrato=contratopersona).exists():
                        return JsonResponse({'result':False,'mensaje':'No ha generado el Informe'})
                    if not InformeActividadJornada.objects.values('id').filter(mes = int(desde),status=True,contrato=contratopersona).exists():
                        return JsonResponse({'result':False,'mensaje':'No ha generado el reporte de asistencia y actividades'})
                    if ActaPago.objects.values('id').filter(mes = int(desde),status=True,contrato=contratopersona).exists():
                        return JsonResponse({'result':False,'mensaje':'Ya existe el registro con el mes de %s'%(MESES_CHOICES[int(desde)-1][1])})
                    secinforme = contratopersona.secuencia_inftecnico()
                    if not(HistorialPagoMes.objects.values('id').filter(status=True,contrato_id=int(request.GET['id']),fecha_pago__month=int(request.GET['desde']),fecha_pago__year=int(request.GET['anio'])).exists()):
                        historial = HistorialPagoMes(
                    ...
                    secpos = SecuenciaMemoActividadPosgrado.objects.filter(tipo=PlantillaContratoDip.objects.last(), status=True,
                                                                  anioejercicio__anioejercicio=datetime.now().year)[0]
                    informe = ActaPago(mes=int(desde),contrato=contratopersona,secuenciageneral=secpos,secuencia=secinforme,historialpago=historial)
                    # data = extraervalores(contratopersona.persona.id,request.GET['desde'],request.GET['anio'],True if 'h' in request.GET else False)
                    data['inftecnicod'] = contratopersona.informetecnico_set.filter(status=True, mes=int(desde))[0]
                    data['actividadjornada'] = contratopersona.informeactividadjornada_set.filter(status=True, mes=int(desde))[0]
                    data['fehca'] = date.today()
                    data['mes'] =MESES_CHOICES[datetime.today().month-1]
                    data['secuencia'] = '0'+str(informe) if informe.secuencia>9 else '00'+str(informe)
                ...
                try:
                    ...
                    form = ContratoDipForm(initial={
                        ...
                        'nombreareaprograma':filtro.contratoareaprograma_set.values_list('gestion_id',flat=True).filter(status=True),
                ...
                try:
                    ...
                    eRecursoPresupuestarioPosgrado =RecursoPresupuestarioPosgrado.objects.filter(status=True).order_by('-id')
                ...
                try:
                    ...
                    data['recursopresupuestario'] = RecursoPresupuestarioPosgrado.objects.filter(status=True).order_by('-id')
                ...
                try:
                    ...
                    data['VALIDAR_CERTIFICACION_PRESUPUESTARIA_POS'] = variable_valor('VALIDAR_CERTIFICACION_PRESUPUESTARIA_POS')
                ...
                try:
                    data['memos'] = memo = MemoActividadPosgrado.objects.filter(status=True,contrato=int(request.GET['idcon']))
                ...
                try:
                    data['bitacora'] = bitacora = InformeActividadJornada.objects.filter(status = True,contrato=int(encrypt(request.GET['idcon'])))
                ...
                try:
                    data['contrato'] = contratopersona= ContratoDip.objects.get(status=True,id=encrypt(request.GET['idcon']))
                    data['historial'] = HistorialPagoMes.objects.filter(status=True,contrato = contratopersona).order_by('fecha_pago')
                    data['actapago'] = ActaPago.objects.filter(status=True, contrato=contratopersona)
                    data['tecnico'] = InformeTecnico.objects.filter(status=True, contrato=contratopersona)
                ...
                try:
                    data['contrato'] = contratopersona= ContratoDip.objects.get(status=True,id=encrypt(request.GET['idcon']))
                    data['historial'] =historial =  HistorialPagoMes.objects.get(status=True, contrato=contratopersona,id=encrypt(request.GET['idhist']))
                    data['actapago'] = ActaPago.objects.filter(status=True, contrato=contratopersona,fecha_creacion__year=historial.fecha_pago.year,mes=historial.fecha_pago.month).first()
                    data['tecnico'] = InformeTecnico.objects.filter(status=True, contrato=contratopersona,fecha_creacion__year=historial.fecha_pago.year,mes=historial.fecha_pago.month).first()
                    data['bitacora'] = InformeActividadJornada.objects.filter(status=True, contrato=contratopersona,fecha_creacion__year=historial.fecha_pago.year,mes=historial.fecha_pago.month).first()
                    data['memos'] = MemoActividadPosgrado.objects.filter(status=True, contrato=contratopersona,fecha_creacion__year=historial.fecha_pago.year,mes=historial.fecha_pago.month).first()
                    if ContratoDip.objects.filter(status=True,id=encrypt(request.GET['idcon']),persona = persona).exists():
                        data['trabaja'] = True
                ...
                try:
                    ...
                    data['semana'] = [[1, 'Lunes'], [2, 'Martes'], [3, 'Miercoles'], [4, 'Jueves'], [5, 'Viernes'], [6, 'Sabado'], [6, 'Domingo']]
                ...
                try:
                    ...
                    ePersona_id = ContratoDip.objects.filter(status=True, validadorgp__isnull=False).values_list( 'validadorgp_id', flat=True).distinct()
                    if search:
                        data['search'] = search
                        ss = search.split(' ')
                        if len(ss) == 1:
                ...
                try:
                    ...
                    filtro,url_vars, search = Q(status=True), '&action=configuracionrequisitocontratacion', request.GET.get('s',None)
                ...
                try:
                    ...
                    ids_sub = filtro.gruporevisionpagocontrato_set.values_list('personacontrato_id', flat=True).filter(status=True)
                ...
                try:
                    ...
                    data['id'] = id = request.GET.get('id')
                    filtro, url_vars, search = Q(status=True, departamento_id=int(encrypt(id))), f'&action=gestposgrado&id={id}', request.GET.get('s', None)
                ...
                try:
                    ...
                    if id == 0:
                        raise NameError("Parametro no encontrado")
                    filtro, url_vars, search = Q(status=True,contratacionconfiguracionrequisito_id = id), f'&action=gestposgrado&id={id}', request.GET.get('s', None)
                    eContratacionConfiguracionRequisito = ContratacionConfiguracionRequisito.objects.get(pk = id)
                    eRequisitoContratacionConfiguracionRequisito = RequisitoContratacionConfiguracionRequisito.objects.filter(filtro)
...
def secuencia_memo():
    ...
    if PlantillaContratoDip.objects.values('id').filter(status=True).last():
        ...
        if SecuenciaMemoActividadPosgrado.objects.values('id').filter(status=True,anioejercicio__anioejercicio=anio, tipo=contrato).exists():
            secuencia = SecuenciaMemoActividadPosgrado.objects.filter(status=True,anioejercicio__anioejercicio=anio, tipo=contrato)[0]
        else:
            secuencia = SecuenciaMemoActividadPosgrado(anioejercicio=anio_ejercicio(), tipo=contrato, secuencia=secuencia_choices[0])
            secuencia.save()
    else:
        if SecuenciaMemoActividadPosgrado.objects.values('id').filter(status=True,anioejercicio__anioejercicio=anio).exists():
