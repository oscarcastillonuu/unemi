            ...
            if action == 'insripcioncursoposgrado':
                try:
                    ...
                    if CapEventoPeriodoIpec.objects.filter(fechainicioinscripcion__lte=hoy, fechafininscripcion__gte=hoy , publicarinscripcion=True, status=True).exists():
                        cursos = CapEventoPeriodoIpec.objects.filter(fechainicioinscripcion__lte=hoy, fechafininscripcion__gte=hoy, publicarinscripcion=True, status=True)
                        for cur in cursos:
                ...
                try:
                    ...
                    evento = CapEventoPeriodoIpec.objects.filter(fechainicioinscripcion__lte=hoy, fechafininscripcion__gte=hoy , publicarinscripcion=True, status=True).order_by('fechainicio')
                ...
                try:
                    ...
                    cursosrecomendados = CapEventoPeriodoIpec.objects.filter(status=True,fechainicioinscripcion__lte=hoy, fechafininscripcion__gte=hoy , publicarinscripcion=True,enfoque=evento.enfoque).exclude(id=evento.id)
                    if cursosrecomendados.count() < 4:
                        cursosrecomendados = CapEventoPeriodoIpec.objects.filter(status=True,fechainicioinscripcion__lte=hoy, fechafininscripcion__gte=hoy , publicarinscripcion=True).exclude(id=evento.id)[:4]
                    else:
