...
def view(request):
    ...
    if request.method == 'POST':
        ...
        elif action == 'verificarubicacion':
            try:
                ...
                print("Chequeo de localización ejecutada...")

                if unemi_poly.contains(user_current_location):
                    print("Ok")
                    return JsonResponse({"result": "ok", "titulo": "Ubicación correcta!!!", "mensaje": u"Usted se encuentra dentro de la Universidad", "showSwal": True})
                else:
                    print("Error")
                    return JsonResponse({"result": "bad", "titulo": "Atención!!!", "mensaje": u"Usted se encuentra fuera de la Universidad", "showSwal": "True", "swalType": "error"})

            except Exception as ex:
                msg = ex.__str__()
                transaction.set_rollback(True)
                return JsonResponse({"result": "bad", "titulo": "Error", "mensaje": u"Error al consultar los datos. [%s]" % msg, "showSwal": "True", "swalType": "error"})
