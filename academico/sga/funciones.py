    ...
    if cache.has_key(f"variable_global_{variable.upper()}"):
        ...
        if eVariablesGlobal:
            if eVariablesGlobal['tipodato'] == 1:
                ...
                valor = eVariablesGlobal['valor'].split(',')
...
def actualiza_asistencia(materiaasignada_id):
    ...
    try:
        materiaasignada = MateriaAsignada.objects.db_manager("sga_select").values("sinasistencia", "matricula__inscripcion__coordinacion_id", "materia__sinasistencia", "fechaasignacion", "materia__fechafinasistencias").get(pk=materiaasignada_id)
    except ObjectDoesNotExist:
        materiaasignada = None
    if materiaasignada:
        vAsistenciafinal = 0
        if materiaasignada['sinasistencia'] or materiaasignada['materia__sinasistencia']:
        ...
        MateriaAsignada.objects.db_manager("default").filter(pk=materiaasignada_id).update(asistenciafinal=vAsistenciafinal)
...
def porciento_asistencia(materiaasignada_id, sinasistencia, fechaasignacion, materia_fechafinasistencias, coordinacion_id):
    ...
    if coordinacion_id == 7:
        asistencias = AsistenciaLeccion.objects.db_manager("sga_select").values("id", "asistio").filter(materiaasignada_id=materiaasignada_id, status=True, leccion__clase__status=True, leccion__clase__activo=True, leccion__status=True).distinct()
    else:
    ...
    totalregistros = 0
    real = 0
    for Fasistencia in asistencias:
        totalregistros += 1
        if Fasistencia['asistio']:
            real += 1
    if totalregistros > 0:
        return null_to_decimal(((real * 100) / totalregistros), 0)
    else:
        return 100
        ...
        if conn.search(busqueda, '(objectclass=person)'):
            ...
            conn.add(busqueda, ['inetOrgPerson'],
                     {'givenName': nombres, 'sn': apellidos, 'uid': usuario_aux, 'cn': nombre_completo, 'mail': mail,
                      'userPassword': clave})
            conn.add(busqueda, ['inetOrgPerson'],
                     {'givenName': 'CARLOS ERNESTO', 'sn': 'LOCKE MORAN', 'uid': 'clockem', 'cn': 'LOCKE MORAN CARLOS ERNESTO', 'mail': 'clockem@unemi.edu.ec',
                      'userPassword': 'Locke19842411'})
        conn.unbind()
...
def actualiza_usuario_revisa_actividad(request, profesor, criterio, distributivo, accion):
    try:
        ...
        if criterio.criterio.id.__str__() in variable_valor('USER_CRITERIOS_IDS'):
            director_carrera = distributivo.carrera.coordinadorcarrera_set.filter(sede=distributivo.coordinacion.sede, tipo=3, periodo=distributivo.periodo, status=True).first() #Director(a) de carrera
            listadopersonas = criterio.usercriteriorevisor_set.filter(status=True)
            if accion == 'add':
                if revisor := listadopersonas.filter(tiporevisor=1, rol=1, persona=director_carrera.persona).first(): #1 'EVIDENCIA')
                    if not UserCriterioRevisorIntegrantes.objects.values('id').filter(usuariorevisor=revisor, profesor_id=profesor.id, status=True).exists():
                        ...
                        user.save(request)
                if revisor := listadopersonas.filter(tiporevisor=2, rol=1, persona=director_carrera.persona).first(): #2 'BITACORA'
                    if not UserCriterioRevisorIntegrantes.objects.values('id').filter(usuariorevisor=revisor, profesor_id=profesor.id, status=True).exists():
                        user = UserCriterioRevisorIntegrantes(usuariorevisor=revisor, profesor_id=profesor.id, status=True)
                        user.save(request)
            if accion == 'del':
