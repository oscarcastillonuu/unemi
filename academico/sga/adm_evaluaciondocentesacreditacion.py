...
from sga.models import TablaPonderacionInstrumento, Profesor, Persona, \
    ...
    RubricaModalidad, TipoProfesor, RubricaTipoProfesor, CronogramaProcesoEvaluativoAcreditacion, RubricaCriterioVinculacion, TablaPonderacionConfiguracion, ProfesorMateria, RubricaModalidadTipoProfesor, Administrativo, RespuestaRubrica
from bd.models import CronogramaCoordinacion
from bd.forms import CronogramaCoordinacionForm
from sga.templatetags.sga_extras import encrypt, cincoacien
        ...
        if 'action' in request.GET:
            ...
            if action == 'listadistributivoxls':
                ...
                try:
                    ...
                    data['ponderaciones'] = TablaPonderacionInstrumento.objects.all()
                ...
                try:
                    ...
                    codigocarrera = request.GET["carr"]
                    if codigocarrera == '':
                        codigocarrera = 0
                    if int(codigocarrera) > 0:
                        listado = listado.filter(carrera_id=int(codigocarrera))
                    ...
                    for per in listado:
                        ...
                        ws.write(a, 22, cincoacien(per[18]), decimal_style)
                    wb.save(response)
                    return response
                except Exception as ex:
                    pass
                    ...
                    for lista in listadoprofesores:
                        ...
                        if lista.profesordistributivohoras_set.filter(periodo=proceso.periodo, status=True):
                            ...
                            if distri.coordinacion:
                                campo3 = distri.coordinacion.nombre
                            if distri.carrera:
                                campo4 = distri.carrera.nombre
                        if distri.coordinacion:
                            campo3 = distri.coordinacion.nombre
                        if distri.carrera:
                            campo4 = distri.carrera.nombre
                ...
                try:
                    ...
                    for lista in listadoprofesores:
                        ...
                        campo4 = ''
                        if lista.coordinacion:
                            campo3 = lista.coordinacion.nombre
                        if lista.carrera:
                            campo4 = lista.carrera.nombre
                        ws.write(row_num, 0, campo1, font_style2)
                        ws.write(row_num, 1, campo2, font_style2)
                        ws.write(row_num, 2, campo3, font_style2)
                        ws.write(row_num, 3, campo4, font_style2)
            ...
            if action == 'docentesrelacionados':
                try:
                    ...
                    for lis in listadistributivo:
                        ...
                        lista.append([lis.id, str(lis.profesor.persona.apellido1 + ' ' + lis.profesor.persona.apellido2 + ' ' + lis.profesor.persona.nombres),cont, nombretablaponderativa])
                    data = {"results": "ok", 'listadoprofesores': lista}
                    return JsonResponse(data)
                except Exception as ex:
                    pass
