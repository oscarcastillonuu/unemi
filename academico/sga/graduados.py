...
import openpyxl
from sga.excelbackground import reporte_tituloinsigniamasivo_background
        ...
        if 'action' in request.GET:
            ...
            if action == 'add':
                ...
                try:
                    ...
                    ws.write_merge(0, 0, 0, 6, 'UNIVERSIDAD ESTATAL DE MILAGRO', title)
                    response = HttpResponse(content_type="application/ms-excel")
                    response['Content-Disposition'] = 'attachment; filename=graduados' + random.randint(1,
                                                                                                        10000).__str__() + '.xls'
                    columns = [
                        (u"FACULTAD", 6000),
                        (u"CODIGO CARRERA", 6000),
                        (u"CARRERA", 6000),
                        (u"TIPO IDENTIFICACIÓN", 6000),
                        (u"CEDULA", 6000),
                        (u"ALUMNO", 6000),
                        (u"NOMBRES", 6000),
                        (u"APELLIDOS", 6000),
                        (u"TITULO", 6000),
                        (u"FECHA EGRESADO", 6000),
                        (u"FECHA GRADUACIÓN", 6000),
                        (u"FECHA REFRENDACIÓN", 6000),
                        (u"NOTA EGRESO", 6000),
                        (u"NOTA TRAB. TITULACION", 6000),
                        (u"NOTA DE GRADO", 6000),
                        (u"PAIS", 6000),
                        (u"PROVINCIA", 6000),
                        (u"CANTON", 6000),
                        (u"PARROQUIA", 6000),
                    ...
                    sql = """
                                    SELECT 
                                            co.nombre as Facultad, 
                                                        malla.codigo codigo_carrera,
                                                        ca.nombre as carrera,
                                            p.id,
                                            (p.apellido1||' '||p.apellido2||' '||p.nombres) as alumno, 
                                            p.nombres nombres, (p.apellido1||' '||p.apellido2) apellidos,
                                            CASE WHEN sex.id=1 THEN malla.tituloobtenidomujer ELSE malla.tituloobtenidohombre END titulo_obtener, 
                                            eg.fechaegreso fecha_egreso,
                                            gr.fechagraduado, gr.fecharefrendacion,
                                            gr.notafinal,gr.promediotitulacion, gr.notagraduacion,
                                            pa.nombre as pais, 
                    ...
                    for r in results:
                        i = 0
                        persona = Persona.objects.get(pk=r[3])
                        campo1 = r[0]
                        campo2 = r[1]
                        campo3 = r[2]
                        campo4 = persona.tipo_identificacion_completo()
                        ...
                        campo8 = r[6]
                        campo9 = r[7]
                        campo10 = r[8] if r[8] else ''
                        campo11 = r[9] if r[9] else ''
                        campo12 = r[10] if r[10] else ''
                        campo13 = r[11]
                        campo14 = r[12]
                        campo15 = r[13]
                        campo16 = r[14]
                        ...
                        campo26 = r[24]
                        campo27 = r[25]
                        campo28 = r[26] if r[26] else ''
                        campo29 = r[27] if r[27] else ''
                        campo30 = r[28]
                        campo31 = r[29]
                        ...
                        ws.write(row_num, 27, campo28, date_format)
                        ws.write(row_num, 28, campo29, date_format)
                ...
                try:
                    ...
                    if user.is_superuser:
                        ...
                        listado = TipoActaFirma.objects.filter(tipoacta__tipo=6, firmado=True, status=True, orden=1)
                    else:
                        listado = persona.tipoactafirma_set.filter(tipoacta__tipo=6, firmado=True, status=True)
            ...
            ids = None
            inscripcionid = None
            graduados = Graduado.objects.filter(inscripcion__carrera__in=miscarreras).order_by('inscripcion__carrera__nombre', 'inscripcion__persona__apellido1', 'inscripcion__persona__apellido2')
...
def generartituloeinsigniaposgrado(request, graduados, data, vistaprevia = False, procesomasivo=False, IS_DEBUG=False):
    ...
    for graduado in graduados:
        ...
        try:
            ...
            if graduado.fecharefrendacion >= datetime(2022, 10, 25, 0, 0, 0).date() and not (graduado.fecharefrendacion == datetime(2023, 2, 13, 0, 0, 0).date()):
                ...
                if DistributivoPersonaHistorial.objects.filter(persona__id=26999, denominacionpuesto=502, status=True).exists():
                    firmatres = DistributivoPersonaHistorial.objects.filter(persona__id=26999, denominacionpuesto=502, status=True)[0]
                    firmatres_tipocargo = '(E)'
            # Tercera firma
            data['firmatres_rubrica'] = FirmaPersona.objects.filter(status=True, persona=firmatres.persona, tipofirma=1).first().firma
            ...
            data['imgfirmadirector'] = firma_departamento.personadepartamento.firmapersona_set.filter(status=True).order_by('-tipofirma').first()
