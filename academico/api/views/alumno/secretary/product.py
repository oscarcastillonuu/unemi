...
from sga.models import PerfilUsuario, Carrera, Matricula, Graduado, MateriaAsignada, InscripcionMalla, AsignaturaMalla
                ...
                try:
                    ...
                    for eAsignatura in eAsignaturas:
                        ...
                        if eSolicitudes.values("id").exists():
                            ...
                            nota = eSolicitud.solicitud_asignatura_record(eAsignatura)
                            if asigho:
...
def apto_para_homologacion(inscripcion):
    ...
    if Graduado.objects.filter(status=True, inscripcion=inscripcion).exists():
        ...
        if MateriaAsignada.objects.filter(status=True, matricula=eMatricula).exists():
            eMateriaAsi = MateriaAsignada.objects.filter(status=True, matricula=eMatricula).order_by('materia__inicio').first()
            if fechaactual > eMateriaAsi.materia.inicio:
                apto = '2'
    ...
    try:
        ...
        for eProducto in eProductos:
            ...
            eProducts.append({
                ...
                "homologa": homologa,
                "carrera": eInscripcion.carrera.__str__()
                # "reqmallaculminada": eCertificado.mallaculminada,
                # "idsinencryptar": eCertificado.pk,
                #"cost": (Decimal(eCertificado.costo).quantize(Decimal('.01')) if eCertificado.costo else Decimal(0).quantize(Decimal('.01'))).__float__(),
                # "validity": {
