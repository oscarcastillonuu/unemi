            ...
            try:
                ...
                if f.is_valid():
                    ...
                    if rubro.epunemi and rubro.idrubroepunemi > 0:
                        ...
                        cursor.execute(sql)
                        cursor.close()
                    log(u'Modifico valor del rubro: %s - %s' % (rubro, rubro.persona), request, "edit")
                    return JsonResponse({"result": "ok"})
                else:
                     raise NameError('Error')
            except:
                    ...
                    for mat in matriculados:
                        ...
                        valorcancelado = null_to_decimal(
                            Rubro.objects.filter(status=True, matricula=mat, cancelado=True).aggregate(
                                valor=Sum('valortotal'))['valor'], 2)
                        ws.write(fila, 6, str(valor))
                        ws.write(fila, 7, str(valoradeudado))
                        ws.write(fila, 8, str(valorcancelado))
                        fila = fila + 1
                    workbook.close()
                    ruta = ruta + filename
