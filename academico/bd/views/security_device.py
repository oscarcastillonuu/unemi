    ...
    if request.method == 'POST':
        ...
        if action == 'validaCode':
            try:
                ...
                if not UserAccessSecurityCode.objects.values("id").filter(pk=idc).exists():
                    raise NameError(u"Parametro de código incorrecto")
                eUserAccessSecurityCode = UserAccessSecurityCode.objects.get(pk=idc)
            ...
            try:
                ...
                eUserAccessSecurityDevices = eUserAccessSecurityDevices.filter(ip_public=ip_public, type=type,
                ...
                return JsonResponse({"result": 'ok', "mensaje": u"Se adicionó correctamente dispositivo"})
            except Exception as ex:
                transaction.set_rollback(True)
                return JsonResponse({"result": 'bad', "mensaje": f"Ocurrio un error: {ex.__str__()}"})
