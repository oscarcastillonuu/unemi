<script lang="ts">
	...
	function validateForm(data: { [inputName: string]: any }): void {
        ...
        if (!isRequiredFieldValid(data.extracto.name)) {
			errors['extracto'] = { ...errors['extracto'], required: true };
		} else {
			errors['extracto'] = { ...errors['extracto'], required: false };
		}
	}
	...
	const save = async (formData) => {
		...
		formData.append('action', 'adddocfinaltitulacionposgrado');
...
<form id="FormAddDocEnsayo" on:submit|preventDefault={onSubmit}>
	<div class="row">
		<div class="col-12 mb-4" id="fieldset_archivo">
			<FormGroup>
				<Label class="mb-3" for="id_propuesta">Documento Propuesta de titulación (Completo) <span class="text-danger">*</span></Label>
        ...
        <div class="col-12 mb-4" id="fieldset_archivo">
			<FormGroup>
				<Label class="mb-3" for="id_extracto">Doc. revisión de plagio (Desde introducción hasta conclusión) <span class="text-danger">*</span></Label>
