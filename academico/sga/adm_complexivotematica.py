...
from inno.forms import FirmaGrupoTitulacionForm
from inno.models import RequisitoMateriaUnidadIntegracionCurricular, RequisitoIngresoUnidadIntegracionCurricular, GrupoTitulacionIC, MateriaGrupoTitulacion, MateriaTitulacionNota, FirmaGrupoTitulacion, GrupoFirma, MateriaTitulacionFirma
...
def adicionoagrupofirma(request, lmate):
    ...
    if not GrupoTitulacionIC.objects.values('id').filter(materia=lmate).exists():
        ...
        grupo = GrupoTitulacionIC.objects.filter(materia=lmate, tiporubrica=tiporubrica)[0]
        if not grupo.grupofirma_set.values('id').filter(status=True).exists():
            if tiporubrica in [3, 4]:
                grupofirma = GrupoFirma(grupo_id=grupo.id, activo=True)
                grupofirma.save(request)
            ...
            grupofirma = GrupoFirma.objects.filter(grupo_id=grupo.id, activo=True)[0]
            if not grupofirma.firmagrupotitulacion_set.values('id').filter(status=True).exists():
                if lmate.profesor_materia_principal():
                    firmagrupo = FirmaGrupoTitulacion(grupofirma_id=grupofirma.id,
                                                      profesor_id=lmate.profesor_materia_principal().profesor.id)
