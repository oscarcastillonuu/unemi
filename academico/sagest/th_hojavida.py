...
from sga.models import PersonaDatosFamiliares, DeclaracionBienes, CuentaBancariaPersona, Titulacion, Capacitacion, \
    ...
    Externo, PerfilUsuario, PersonaDetalleMaternidad
    ...
    if request.method == 'POST':
        ...
        if action == 'validarLenguaMaterna':
            ...
            try:
                ...
                if f.is_valid():
                    ...
                    if DeclaracionBienes.objects.filter(persona=persona, fecha=f.cleaned_data['fecha'],tipodeclaracion=f.cleaned_data['tipodeclaracion']).exists():
        ...
        # DECLARACIONES
            ...
            try:
                ...
                if f.is_valid():
                    if 'archivo' not in request.FILES:
                        return JsonResponse({'result': 'bad', 'mensaje': u'Debe subir la declaración en formato pdf'})
                    if DeclaracionBienes.objects.filter(persona=persona, fecha=f.cleaned_data['fecha']).exists():
                        return JsonResponse({'result': 'bad', 'mensaje': u'Ya tiene una declaración en la misma fecha'})
                    newfile = request.FILES['archivo']
                    newfile._name = generar_nombre("declaracion_", newfile._name)
                    fechaproxima = None
        ...
        # SITUACION LABORAL
            ...
            try:
                ...
                # if os.path.exists(SITE_STORAGE + archivo_borrar):
                #     os.remove(SITE_STORAGE + archivo_borrar)
                # cuerpo = f"Informe mensual de posgrado generado y firmado por {solicitud.contrato.persona}"
                # notificacion(
                #     "Solicitud de %s para revisión del informe mensual de posgrado" % solicitud.contrato.persona,
                #     cuerpo, solicitud.contrato.gestion.responsable, None, f'/adm_solicitudpago',
                #     solicitud.id,
        ...
        if action == 'subirrequisitosperiodotthh':
            with transaction.atomic():
                try:
        ...
        if 'action' in request.GET:
                ...
                try:
                    ...
                    if not request.GET.get('fi', '') or not request.GET.get('ff', ''):
                        return HttpResponseRedirect(
                            f"/th_hojavida?info=Seleccione la fecha inicio/fin de la actividad.")

                    fi, ff = convertir_fecha_hora_invertida(
                        request.GET['fi'] + " 00:01"), convertir_fecha_hora_invertida(request.GET['ff'] + " 23:59")
                    actividadesbitacora = BitacoraActividadDiaria.objects.filter(
                        Q(fecha__gte=fi, fecha__lte=ff, persona=persona, status=True)).order_by('fecha')
                    contrato = ContratoDip.objects.filter(
                        Q(persona=persona, fechainicio__lte=fi, fechainicio__lt=ff, fechafin__gt=fi, fechafin__gte=ff,
                    ...
                    if contrato:
                        ...
                        data['carreras'] = ContratoCarrera.objects.filter(contrato=contrato, status=True)
                        data['areasprogramas'] = ContratoAreaPrograma.objects.filter(contrato=contrato, status=True)
                        nombres = unicodedata.normalize('NFD', u"%s" % persona.un_nombre_dos_apellidos()).encode(
                            'ascii', 'ignore').decode("utf-8").upper()
                        return conviert_html_to_pdf_name(
                            'th_hojavida/informe_actividad_mensual_administrativo_posgrado.html', {"data": data},
                            f"INFORME ADMINISTRATIVO - {nombres} {hoy.hour}{hoy.minute}{hoy.second}")
                    else:
                        return HttpResponseRedirect(
                            f"/th_hojavida?info=Estimad{'o' if persona.es_mujer() else 'a'} {persona}, no se encontró un contrato vigente.")
                except Exception as ex:
                ...
                try:
                    if 'id' in request.GET:
                        ...
                        if request.user.user_permissions.filter(
                                codename='puede_aprobar_justificacion_marcada_director').exists():
                            data['es_director_th'] = True
                        elif request.user.user_permissions.filter(
                                codename='puede_cerrar_justificacion_marcada_analista').exists():
            ...
            # DATOS FINANCIEROS
                ...
                try:
                    ...
                    data['declaraciones'] = persona.declaracionesbienes()
                    request.session['viewactivoth'] = ['informacionpersonal', action]
