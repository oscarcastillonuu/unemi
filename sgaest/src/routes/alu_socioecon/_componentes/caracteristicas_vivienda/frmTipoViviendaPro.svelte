<script lang="ts">
	...
	import { getTiposViviendasPro as loadDataTiposViviendas } from '$lib/utils/loadDataApi';
	...
	onMount(async () => {
		...
		eTipos = await loadDataTiposViviendas();
		await delay(1000);
		...
		if (aData.eFichaSocioeconomica) {
			...
			if (eFichaSocioeconomica.tipoviviendapro) {
				selectTipoVivienda = eFichaSocioeconomica.tipoviviendapro['pk'] ?? 0;
			}
		}
		await delay(2000);
		load = false;
	...
	const saveDatosCaracteristicaVivienda = async () => {
		...
		await resetForms('none');
		const $frmTipoVivienda = document.getElementById('frmTipoVivienda');
		const formData = new FormData($frmTipoVivienda);
		formData.append('tipoviviendapro', selectTipoVivienda.toString());
		formData.append('field', 'tipoviviendapro');
...
>
	...
	<ModalBody>
		{#if !load}
			<form action="javascript:;" id="frmTipoVivienda">
				<div class="row g-3">
					<div class="col-12">
						<label for="id_tipoviviendapro" class="form-label fw-bold"
							><span><i class="fe fe-alert-octagon text-warning" /></span> ¿Su vivienda es propia o arrendada
							o cedida por familiares?
						</label>
						<select
							class="form-select form-select-sm"
							aria-label=""
							id="id_tipoviviendapro"
							bind:value={selectTipoVivienda}
						>
							<option value={0} selected> ----------- </option>
							{#each eTipos as eTipo}
								{#if selectTipoVivienda === eTipo.id}
						...
						<div class="valid-feedback" id="id_tipoviviendapro_validate">¡Se ve bien!</div>
					</div>
				</div>
			</form>
		{:else}
