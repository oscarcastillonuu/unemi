...
from sga.models import Canton, Provincia, TIPO_PERSONA, Parroquia, Pais, Sexo, ProgramasInvestigacion, Coordinacion, TIPO_PROYECTO_ARTICULO, ProyectosInvestigacion, Modalidad, Titulo, RevistaInvestigacion, Persona
...
class ConvocatoriaProyectoForm(forms.Form):
    ...
    archivoresolucion = ExtFileField(label=u'Resolución OCAS', required=False, help_text=u'Tamaño Maximo permitido 4Mb, en formato pdf', ext_whitelist=(".pdf",), max_upload_size=4194304, widget=forms.FileInput(attrs={'formwidth': '50%'}))
    ...
    archivoconvocatoria = ExtFileField(label=u'Bases Convocatoria', required=False, help_text=u'Tamaño Maximo permitido 4Mb, en formato xls', ext_whitelist=(".pdf",), max_upload_size=4194304, widget=forms.FileInput(attrs={'formwidth': '100%'}))
...
class RegistroPropuestaProyectoInvestigacionForm(FormModeloBase):
    categoria = forms.ModelChoiceField(label=u"Tipo", queryset=Categoria.objects.filter(status=True, vigente=True).order_by('numero'), required=False, widget=forms.Select(attrs={'col': '12', 'separator2': True, 'separatortitle': 'Datos Generales'}))
...
class ExternoForm(FormModeloBase):
    ...
    funcionproyecto = forms.ChoiceField(label=u"Rol en el Proyecto", required=False, choices=FUNCION_INTEGRANTE, widget=forms.Select(attrs={'col': '6', 'disabled': 'disabled'}))
...
class SolicitudRegArticuloPubForm(FormModeloBase):
    ...
    numerorevista = forms.CharField(label=u"Número de la Revista", max_length=250, widget=forms.TextInput(attrs={'col': '12', 'autocomplete': 'off'}), required=False)
    volumenrevista = forms.CharField(label=u"Volumen de la Revista", max_length=250, widget=forms.TextInput(attrs={'col': '12', 'autocomplete': 'off'}), required=False)
    paginaarticulorevista = forms.CharField(label=u"Páginas del artículo en la revista", max_length=250, widget=forms.TextInput(attrs={'col': '12', 'autocomplete': 'off'}), required=False)
...
class PeriodoEvaluacionInvestigacionForm(FormModeloBase):
    descripcion = forms.CharField(label=u"Descripción", max_length=150, widget=forms.TextInput(attrs={'col': '12', 'autocomplete': 'off'}), required=False)
    instructivo = ExtFileField(label=u'Instructivo', required=False, help_text=u'Tamaño Maximo permitido 4Mb, en formato pdf', ext_whitelist=(".pdf",), max_upload_size=4194304, widget=forms.FileInput(attrs={'col': '12'}))
    # archivocga = ExtFileField(label=u'Resolución CGA', required=False, help_text=u'Tamaño Maximo permitido 4Mb, en formato pdf', ext_whitelist=(".pdf",), max_upload_size=4194304, widget=forms.FileInput(attrs={'col': '6'}))
    inicio = forms.DateField(label=u"Inicio", initial=datetime.now().date(), widget=DateTimeInput(format='%d-%m-%Y', attrs={'class': 'selectorfecha', 'col': '6', 'separator2': True, 'separatortitle': 'Evaluación'}), required=False)
    fin = forms.DateField(label=u"Fin", initial=datetime.now().date(), widget=DateTimeInput(format='%d-%m-%Y', attrs={'class': 'selectorfecha', 'col': '6'}), required=False)
    # inicioevalint = forms.DateField(label=u"Inicio", initial=datetime.now().date(), widget=DateTimeInput(format='%d-%m-%Y', attrs={'class': 'selectorfecha', 'col': '6', 'separator2': True, 'separatortitle': 'Evaluación Interna'}), required=False)
    # finevalint = forms.DateField(label=u"Fin", initial=datetime.now().date(), widget=DateTimeInput(format='%d-%m-%Y', attrs={'class': 'selectorfecha', 'col': '6'}), required=False)
    # inicioevalext = forms.DateField(label=u"Inicio", initial=datetime.now().date(), widget=DateTimeInput(format='%d-%m-%Y', attrs={'class': 'selectorfecha', 'col': '6', 'separator2': True, 'separatortitle': 'Evaluación Externa'}), required=False)
    # finevalext = forms.DateField(label=u"Fin", initial=datetime.now().date(), widget=DateTimeInput(format='%d-%m-%Y', attrs={'class': 'selectorfecha', 'col': '6'}), required=False)
