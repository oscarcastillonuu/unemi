            ...
            try:
                ...
                if f.is_valid():
                    if not ArticuloInvestigacion.objects.filter(nombre=f.cleaned_data['nombre'], status=True).exclude(pk=int(encrypt(request.POST['id']))).exists():
                        ...
                        articulo.categoria = int(request.POST['categoriaarticuloid'])
                        articulo.observacion = f.cleaned_data['observacion'] if int(request.POST['categoriaarticuloid']) != int(request.POST['categoriabaseid']) else ''
                        articulo.save(request)
