...
def view(request):
    ...
    if request.method == 'POST':
        ...
        if action == 'addestructurainforme':
            with transaction.atomic():
                try:
                    idservicio = int(encrypt(request.POST['id']))
                    form = EstructuraInformeForm(request.POST)
                    if form.is_valid() and form.validador(idservicio):
                        estructura = EstructuraInforme(servicio_id=idservicio,
                                                          titulo=form.cleaned_data['titulo'],
                                                          tipoinforme=form.cleaned_data['tipoinforme'],
                                                          orden=form.cleaned_data['orden'],
                                                          segmentacion=form.cleaned_data['segmentacion'],
                        ...
                        diccionario = {'id': encrypt(estructura.id),
                                       'idservicio': estructura.servicio.id,
                                       'titulo': estructura.titulo,
                                       'tipoinforme': estructura.get_tipoinforme_display(),
                                       'orden': estructura.orden,
                                       'activo': estructura.activo,
                                       }
                        log(u'Agrego Estructura a Servicio: %s' % estructura, request, "add")
                        return JsonResponse({'result': True, 'data_return': True, 'mensaje': u'Guardado con exito',
                                             'data': diccionario})
                    else:
                        transaction.set_rollback(True)
                        x = [{k: v[0]} for k, v in form.errors.items()][0]
                        err = ''
        ...
        #  NOTICIAS
            ...
            with transaction.atomic():
                try:
                    form = NoticiaVinculaForm(request.POST, request.FILES)
            ...
            try:
                with transaction.atomic():
                    ...
                    idpadre = encrypt_id(request.POST['idpadre'])
                    if form.is_valid():
                        if ResponsableGrupoServicio.objects.filter(responsable=int(request.POST['responsable'])).exists():
                ...
                with transaction.atomic():
                    ...
                    if not form.is_valid():
                        return JsonResponse({'result': 'bad', "form": [{k: v[0]} for k, v in form.errors.items()],
                                             "mensaje": "Error en el formulario"})
                    persona = Persona.objects.get(id=int(request.POST['responsable']))
                    if ResponsableGrupoServicio.objects.filter(responsable=persona).exclude(
        ...
        if 'action' in request.GET:
            ...
            # CARD INFORMATIVO VISTA
                ...
                try:
                    ...
                    data['card'] = card = CardInformativo.objects.last()
            ...
            # if action == 'alertainformacion':
            #     try:
            #
            #         data['title'] = u'Card Informativo'
            #         return render(request, 'adm_agendamientocitas/cardinformativowebsite.html', data)
            #
            #     except Exception as ex:
            #         return render({'result': False, 'mensaje': '{}'.format(ex)})
            ...
            if action == 'estructurainforme':
                try:
                    ...
                    data['listado'] = EstructuraInforme.objects.filter(servicio=servicio, activo=True).order_by('orden')
                    #data['listado'] = servicio.estructura()
                    data['form'] = form
                    #data['estructura'] = EstructuraInforme.objects.filter(status=True)
            ...
            # SITIO WEB
                ...
                try:
                    ...
                    if len(SECCION) > len(titulos):
                        for s in SECCION:
                            titulo = TituloWebSiteServicio.objects.filter(departamentoservicio__id = iddpservicio, status=True, seccion=s[0])
                            if not titulo.exists():
                                title = ''
                                subtitle = ''
                                if s[0] == 1:
                                    title = castTituloSubtituloSitioWeb(iddpservicio)
                                ...
                                titulo = TituloWebSiteServicio(departamentoservicio=departamento, seccion=s[0], titulo=title, subtitulo=subtitle)
                                titulo.save(request)
                    data['secciones'] = SECCION
                    data['s_activa'] = int(seccionactiva)
                    data['titulos'] = TituloWebSiteServicio.objects.filter(departamentoservicio__id=iddpservicio, status=True).order_by('seccion')
            ...
            # CARD INFORMATIVO
                ...
                try:
                    ...
                    if ultimo_registro:
                        form = CardInformativoForm(initial={
                        ...
                        if ultimo_registro.imagen:
                            form.fields['imagen'].initial = ultimo_registro.imagen.url
                    else:
                    ...
                    data['idpadre'] = encrypt_id(request.GET['id'])
                    visible_fields = form.visible_fields()
                    total_fields: int = len(visible_fields)
                    lista = [(1, 'Información Básica', visible_fields[:2]),
...
def consultaPersonaDepartamento(request):
    ...
    personasresponsables_id = ResponsableGrupoServicio.objects.filter(departamentoservicio__id = departamentoid, status=True,
                                                   responsable__nombres__icontains=q).values_list('responsable__id', flat=True)
