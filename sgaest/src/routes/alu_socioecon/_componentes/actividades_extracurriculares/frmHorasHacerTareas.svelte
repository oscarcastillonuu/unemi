<script lang="ts">
	...
	let inputHorasHacerTareaHogar = 0;
	...
	onMount(async () => {
		...
		if (aData.eFichaSocioeconomica) {
			...
			if (eFichaSocioeconomica.horashacertareas) {
				inputHorasHacerTareaHogar = eFichaSocioeconomica.horashacertareas ?? 0;
			}
		}
		await delay(2000);
		load = false;
	...
	const saveDatosActividadExtracurricular = async () => {
		...
		if (inputHorasHacerTareaHogar != null) {
			formData.append('horashacertareas', inputHorasHacerTareaHogar.toString());
		} else {
			formData.append('horashacertareas', '0');
		}
		formData.append('field', 'horashacertareas');
	...
	<ModalBody>
		{#if !load}
			<form action="javascript:;" id="frmActividad">
				<div class="row g-3">
					<div class="col-12">
						<label for="id_horashacertareas" class="form-label fw-bold"
							><span><i class="fe fe-alert-octagon text-warning" /></span> ¿Cuanto tiempo promedio se
							dedica para hacer sus tareas?
						</label>
						<input
							type="number"
							class="form-control form-control-sm"
							aria-label=""
							id="id_horashacertareas"
							bind:value={inputHorasHacerTareaHogar}
							on:keyup={() => numeric('id_horashacertareas', 0, 1000, 0)}
						/>

						<div class="valid-feedback" id="id_horashacertareas_validate">¡Se ve bien!</div>
					</div>
				</div>
			</form>
		{:else}
