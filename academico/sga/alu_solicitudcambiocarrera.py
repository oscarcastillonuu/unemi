            ...
            if action == 'add':
                try:
                    if AperturaPeriodoCambioCarrera.objects.filter(status=True, publico=True).exists():
                        if not inscripcion.solicitudcambiocarrera_set.exists() or not SolicitudCambioCarrera.objects.filter(status=True, inscripcion=inscripcion, estados__in=[0,7,4]).exists():
