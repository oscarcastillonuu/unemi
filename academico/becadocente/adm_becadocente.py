...
from investigacion.funciones import coordinador_investigacion, vicerrector_investigacion_posgrado, rector_institucion, analista_investigacion, experto_investigacion, secuencia_informe_factibilidad, director_juridico, experto_juridico, secuencia_resolucion_comite, salto_linea_nombre_firma_encontrado, secuencia_solicitud_certificacion
...
from sga.funciones import MiPaginador, log, validar_archivo, generar_nombre, cuenta_email_disponible_para_envio, fechaletra_corta, remover_atributo_style_html, moneda_a_letras, remover_caracteres, elimina_tildes, remover_caracteres_tildes_unicode, remover_caracteres_especiales_unicode, null_to_decimal
    ...
    if request.method == 'POST':
        ...
        if action == 'addconvocatoria':
            try:
                ...
                if f.is_valid():
                    if not Convocatoria.objects.filter(status=True, descripcion=f.cleaned_data['descripcion'].strip().upper()).exists():
                        ...
                        # Obtengo estado EN REVISIÓN
                        estado = obtener_estado_solicitud(9, 1)
                        ...
                        # Guardo los documento asociados a la convocatoria
                        ...
                        recorrido = RecorridoRegistro(
                            ...
                            persona=persona,
                            observacion='CONVOCATORIA REGISTRADA',
                            estado=estado
                        )
                        recorrido.save(request)

                        log(u'%s agregó convocatoria de becas para docentes: %s' % (persona, convocatoria), request, "add")
            ...
            try:
                ...
                if f.is_valid():
                    if not Convocatoria.objects.filter(status=True, descripcion=f.cleaned_data['descripcion'].strip().upper()).exclude(pk=int(encrypt(request.POST['id']))).exists():
                        # Consultar la convocatoria
                        convocatoria = Convocatoria.objects.get(pk=int(encrypt(request.POST['id'])))

                        # Validar que la convocatoria no haya sido REVISADA por el EXPERTO DE INVESTIGACIÓN
                        if convocatoria.revisada():
                            return JsonResponse({"result": "bad", "titulo": "Atención!!!", "mensaje": u"No se puede guardar el registro debido a que ya tiene estado REVISADO", "showSwal": "True", "swalType": "warning"})
                        ...
                        ultimoestado = convocatoria.estado.valor
                        guardarecorrido = False

                        # Si el último estado asignado fue de RECHAZO
                        if ultimoestado in [3, 5, 7, 9]:
                            # Obtengo estado EN REVISIÓN
                            estado = obtener_estado_solicitud(9, 1)
                        ...
                        log(u'%s editó convocatoria de becas para docentes: %s' % (persona, convocatoria), request, "add")
        ...
        if 'action' in request.GET:
            ...
            try:
                convocatorias = Convocatoria.objects.filter(status=True).order_by('-iniciopos', '-finpos')
                es_personaljuridico = persona.pertenece_a_departamento(122)
