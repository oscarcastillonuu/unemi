            ...
            if 's' in request.GET:
                ...
                if len(ss) == 1:
                    usuarioevidencia = UsuarioEvidencia.objects.filter(
                        ...
                        Q(userpermiso__persona__apellido2__icontains=search),status=True).order_by('unidadorganica__departamento__nombre','userpermiso__persona__apellido1')
                else:
                    usuarioevidencia = UsuarioEvidencia.objects.filter(
                        Q(userpermiso__persona__apellido1__icontains=ss[0]) &
                        Q(userpermiso__persona__apellido2__icontains=ss[1]), status=True).order_by(
                        'unidadorganica__departamento__nombre', 'userpermiso__persona__apellido1')

            else:
                usuarioevidencia = UsuarioEvidencia.objects.filter(status=True).order_by('unidadorganica__departamento__nombre','userpermiso__persona__apellido1')
