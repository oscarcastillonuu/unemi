<script lang="ts">
	...
	import { getMaterialesParedes as loadDataMateriales } from '$lib/utils/loadDataApi';
	...
	onMount(async () => {
		...
		eMateriales = await loadDataMateriales();
		await delay(1000);
		...
		if (aData.eFichaSocioeconomica) {
			...
			if (eFichaSocioeconomica.materialpared) {
				selectMaterial = eFichaSocioeconomica.materialpared['pk'] ?? 0;
			}
		}
		await delay(2000);
		load = false;
	...
	const saveDatosCaracteristicaVivienda = async () => {
		...
		await resetForms('none');
		const $frmMaterialPared = document.getElementById('frmMaterialPared');
		const formData = new FormData($frmMaterialPared);
		formData.append('materialpared', selectMaterial.toString());
		formData.append('field', 'materialpared');
...
>
	...
	<ModalBody>
		{#if !load}
			<form action="javascript:;" id="frmMaterialPared">
				<div class="row g-3">
					<div class="col-12">
						<label for="id_materialpared" class="form-label fw-bold"
							><span><i class="fe fe-alert-octagon text-warning" /></span> Material Predominante en las paredes  </label
						>
						<select
							class="form-select form-select-sm"
							aria-label=""
							id="id_materialpared"
							bind:value={selectMaterial}
						>
							<option value={0} selected> ----------- </option>
							{#each eMateriales as eMaterial}
								{#if selectMaterial === eMaterial.id}
									<option value={eMaterial.id} selected>
										{eMaterial.name}
									</option>
								{:else}
									<option value={eMaterial.id}>
										{eMaterial.name}
									</option>
								{/if}
							{/each}
						</select>
						<div class="valid-feedback" id="id_materialpared_validate">¡Se ve bien!</div>
					</div>
				</div>
			</form>
		{:else}
