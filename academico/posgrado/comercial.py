...
from sga.funciones_templatepdf import contratoformapagoprograma, pagareaspirantemae
...
from sga.models import miinstitucion, Persona, CUENTAS_CORREOS, Carrera, \
    ...
    VariablesGlobales, Notificacion, Reporte, Canton, PersonaSituacionLaboral
                        ...
                        if lead.contrato_set.filter(status=True).last().id and lead.contrato_set.filter(status=True).last().tablaamortizacion_set.all():
                            ...
                            for t in tablaamortizacion:
                                amortizacion = TablaAmortizacion(
                                    ...
                                    nombre=des if t[0] != '' else desmatricula,
                                    valor=t[3],
        ...
        if 'action' in request.GET:
            ...
            if action == 'detalleprospecto':
                ...
                try:
                    ...
                    ws.merge_range('A1:S1', 'Reporte general de ventas', formatotitulo_filtros)
                    if desde and hasta:
                        ws.merge_range('A2:S2', 'Desde el ' + desde + ' Hasta el ' + hasta, formatotitulo_filtros)
                ...
                try:
                    ...
                    ws.write(1, 16, 'LEAD DE ZONA', formatoceldacab)
                    ws.write(1, 17, '¿ACTIVO?', formatoceldacab)
                    ...
                    if estado != "":
                        ...
                        if estado == 1:
                            ...
                            filtro = filtro & Q(inscripcion__matricula__isnull=False)
                        elif estado == 10:
                    ...
                    for lead in leads:
                        ...
                        ws.write('R%s' % filas_recorridas, str('SI' if lead.status else 'NO'), formatoceldaleft)
            ...
            if action == 'detallerubrosins':
                ...
                try:
                    from posgrado.forms import ConvenioForm
                    from posgrado.models import Convenio
                    data['title'] = u'Editar convenio de información'
                    data['id'] = request.GET['id']
