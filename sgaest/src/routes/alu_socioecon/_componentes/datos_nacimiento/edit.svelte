<script lang="ts">
	...
	const saveDatosNacimiento = async () => {
		...
		formData.append('action', 'saveDatosNacimiento');
		...
		if (errors.length > 0) {
			...
			if (!res.isSuccess) {
				...
				dispatch('actionRun', { action: 'saveDatosNacimiento' });
			}
		}
	};
...
{#if ePersona}
	...
	>
		<ModalHeader toggle={mToggle} class="bg-primary text-white">
			<span class="text-white">Actualizar datos de nacimiento</span>
		</ModalHeader>
		<ModalBody>
			{#if !load}
		...
		<ModalFooter>
			...
			{#if !load}
				<Button color="primary" class="rounded-3 btn-sm" on:click={saveDatosNacimiento}
					>Guardar</Button
				>
			{/if}
		</ModalFooter>
