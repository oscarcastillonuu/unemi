<script lang="ts">
	...
	onMount(async () => {
		...
		if (aData.eFichaSocioeconomica) {
			...
			if (eFichaSocioeconomica.horastrabajodomestico) {
				inputHorasTrabajoHogar = eFichaSocioeconomica.horastrabajodomestico ?? 0;
			}
		}
		await delay(2000);
		load = false;
	...
	const saveDatosActividadExtracurricular = async () => {
		...
		if (inputHorasTrabajoHogar != null) {
			formData.append('horastrabajodomestico', inputHorasTrabajoHogar.toString());
		} else {
			formData.append('horastrabajodomestico', '0');
		}
		formData.append('field', 'horastrabajodomestico');
	...
	<ModalBody>
		{#if !load}
			<form action="javascript:;" id="frmActividad">
				<div class="row g-3">
					<div class="col-12">
						<label for="id_horastrabajodomestico" class="form-label fw-bold"
							><span><i class="fe fe-alert-octagon text-warning" /> Horas de trabajo doméstico</span
							>
						</label>
						<input
							type="number"
							...
							aria-label=""
							id="id_horastrabajodomestico"
							bind:value={inputHorasTrabajoHogar}
							on:keyup={() => numeric('id_horastrabajodomestico', 0, 1000, 0)}
						/>

						<div class="valid-feedback" id="id_horastrabajodomestico_validate">¡Se ve bien!</div>
					</div>
				</div>
			</form>
		{:else}
