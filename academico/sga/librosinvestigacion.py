...
from investigacion.funciones import coordinador_investigacion, notificar_revision_solicitud_produccion_cientifica
    ...
    if request.method == 'POST':
        ...
        if 'action' in request.GET:
            ...
            if action == 'aprobar':
                ...
                try:
                    ...
                    if 'id' in request.GET:
                        tipobus = 1
                        ids = request.GET['id']
                        capituloslibros = CapituloLibroInvestigacion.objects.select_related().filter(pk=int(encrypt(ids)), status=True).order_by('titulocapitulo')
                    elif 's' in request.GET:
                        search = request.GET['s']
                        if search.isdigit():
                            if tipobus != 4:
                                capituloslibros = CapituloLibroInvestigacion.objects.select_related().filter(pk=search, status=True)
                            else:
                                anio = int(search)
                                capituloslibros = CapituloLibroInvestigacion.objects.select_related().filter(fechapublicacion__year=anio, status=True)#.order_by('titulocapitulo')
                        else:
                            if tipobus == 1:
                                capituloslibros = CapituloLibroInvestigacion.objects.select_related().filter(titulocapitulo__icontains=search, status=True)
                            elif tipobus == 2:
                                capituloslibros = CapituloLibroInvestigacion.objects.select_related().filter(titulolibro__icontains=search, status=True)
                            else:
                                ss = search.split(' ')
                                if len(ss) == 1:
                        ...
                        capituloslibros = CapituloLibroInvestigacion.objects.select_related().filter(Q(participantecapitulolibros__profesor__persona__id__in=lista_ids_personas) |
                                                                                                     Q(participantecapitulolibros__administrativo__persona__id__in=lista_ids_personas),
                                                                                                      participantecapitulolibros__status=True, status=True)


                    else:
                        tipobus = 1
                        capituloslibros = CapituloLibroInvestigacion.objects.select_related().filter(status=True)#.order_by('titulocapitulo')
                    ...
                    data['aniosevidencias'] = CapituloLibroInvestigacion.objects.filter(status=True, fechapublicacion__isnull=False).annotate(anio=ExtractYear('fechapublicacion')).values_list('anio', flat=True).order_by('-anio').distinct()
                    ...
                    data['participantes'] = Persona.objects.values('id', 'cedula', 'nombres', 'apellido1', 'apellido2').filter(Q(profesor__participantecapitulolibros__isnull=False, profesor__participantecapitulolibros__status=True)
                                                                                                                               |Q(administrativo__participantecapitulolibros__isnull=False, administrativo__participantecapitulolibros__status=True)
                                                                                                                               ).distinct().order_by('apellido1', 'apellido2', 'nombres')
            ...
            if 'id' in request.GET:
                data['tipobus'] = 2
                ids = request.GET['id']
                librosinvestigacion = LibroInvestigacion.objects.select_related().filter(pk=int(encrypt(ids)), status=True).order_by('nombrelibro')
            elif 's' in request.GET:
                search = request.GET['s']
                if search.isdigit():
                    if int(request.GET['tipobus']) == 4:
                        data['tipobus'] = 4
                        anio = int(search)
                        librosinvestigacion = LibroInvestigacion.objects.select_related().filter(fechapublicacion__year=anio, status=True)
                    else:
                        data['tipobus'] = 2
                        librosinvestigacion = LibroInvestigacion.objects.select_related().filter(pk=search, status=True)
                else:
                    tipobus = int(request.GET['tipobus'])
                    data['tipobus'] = tipobus
                    if tipobus == 1:
                        librosinvestigacion = LibroInvestigacion.objects.select_related().filter(revista__nombre__icontains=search, revista__status=True, status=True)
                    if tipobus == 2:
                        librosinvestigacion = LibroInvestigacion.objects.select_related().filter(nombrelibro__icontains=search, status=True)
                    if tipobus == 3:
                        if ' ' in search:
                            s = search.split(" ")
                            ...
                            librosinvestigaciondoc = LibroInvestigacion.objects.select_related().filter(pk__in=participanteslibrosdoc, status=True)
                            ...
                            librosinvestigacionadm = LibroInvestigacion.objects.select_related().filter( pk__in=participanteslibrosadm, status=True)
                ...
                librosinvestigacion = LibroInvestigacion.objects.select_related().filter(pk__in=participanteslibros, status=True)
            else:
                data['tipobus'] = 2
                librosinvestigacion = LibroInvestigacion.objects.select_related().filter(status=True).order_by('nombrelibro')
            ...
            data['aniosevidencias'] = LibroInvestigacion.objects.filter(status=True, fechapublicacion__isnull=False).annotate(anio=ExtractYear('fechapublicacion')).values_list('anio', flat=True).order_by('-anio').distinct()
            data['participantes'] = Persona.objects.values('id', 'cedula', 'nombres', 'apellido1', 'apellido2').filter(Q(profesor__participantelibros__isnull=False, profesor__participantelibros__status=True)
                                                                                                                          |Q(administrativo__participantelibros__isnull=False, administrativo__participantelibros__status=True)
                                                                                                                          ).distinct().order_by('apellido1', 'apellido2', 'nombres')
