<script lang="ts">
	...
	onMount(async () => {
		if (aData !== undefined) {
			...
			//console.log('num_materias_maxima:', num_materias_maxima);

			const [res, errors] = await apiPOST(fetch, 'alumno/matricula/pregrado', {
	...
	const openAsignatura = (asignatura) => {
		if (asignatura.validarequisitograduacion) {
			...
			eRequisitos.forEach(function (requisito, index) {
				if (requisito.enlineamatriculacion) {
					...
					eMaterias.forEach(function (_materia, index) {
						console.log('nivelmalla_id:', _materia.nivelmalla_id);
						console.log('ultimoNivel:', ultimoNivel.idm);
						console.log('validarequisitograduacion:', _materia.validarequisitograduacion);
						if (
					...
					console.log(contadorMateriaSeleccionadaUltimoNivel);
		...
		if (asignatura.asignaturapracticas){
			const mensaje = {
					title: '',
					html: `<b> ACTA DE COMPROMISO Y RESPONSABILIDAD PARA MATRICULACIÓN EN ASIGNATURAS DE PRÁCTICAS PREPROFESIONALES (Prácticas Laborales y/o Prácticas de Servicio Comunitario).</b>
												...
												<p style="text-align: justify; font-size: 16px; margin-top: 15px">Al matricularse en la asignatura ${asignatura.asignatura} que incluye prácticas preprofesionales se
							...
							<ol style="padding-left: 20px;font-size: 14px!important;">
									...
									<li style="text-align: justify">
										Los estudiantes que mantengan rubros pendientes deberán cancelar dentro de la tercera y cuarta semana,
										de lo contrario reprobarán las asignaturas.
									</li>
									<li style="text-align: justify">
	...
	const selectSubject = (_materia) => {
		...
		let fnSelect = function () {
			loading.setLoading(true, 'Cargando, espere por favor...');
			if (_materia.teoriapractica == 0 || _materia.tipomateria == 3) {
				eMaterias.push({
					id: _materia.id,
					...
					validaconflictohorarioalu: _materia.validaconflictohorarioalu
				});
		...
		loading.setLoading(true, 'Cargando, espere por favor...');
		console.log(_materia.validaconflictohorarioalu);
		if (
			ePeriodoMatricula.valida_conflicto_horario &&
			eMalla.modalidad != 3 &&
			eMaterias.length > 0 &&
			_materia.tipomateria != 3 &&
			_materia.validaconflictohorarioalu
		) {
			//console.log(eMaterias);
			if (eMaterias.length > 0) {
				let materia_aux = {
	...
	const calcula_gratuidad = () => {
		...
		if (ePeriodoMatricula.valida_gratuidad && eMaterias.length > 0) {
			...
			/*//console.log("total_asignaturas_aperturadas", total_asignaturas_aperturadas);
			//console.log("porciento_perdida_parcial_gratuidad", porciento_perdida_parcial_gratuidad);
			//console.log("porcentaje_seleccionadas", porcentaje_seleccionadas);*/
							...
							if (response.value.data.valorpagar > 0) {
								const mensaje = {
									...
									text: `${ePersona.es_mujer ? 'Estimada' : 'Estimado'} ${
										ePersona.nombre_completo
									}, le informamos que el proceso de matriculación ha finalizado y se han generados rubros. Consulte los valores en el Módulo "Mis Finanzas"`,
									type: 'warning',
									icon: 'warning',
									showCancelButton: false,
									allowOutsideClick: false,
...
{#if !load}
				...
				<div class="row align-items-center">
					<div class="col-xl-12 col-lg-12 col-md-12 col-12">
						<h4
							...
							style="width:100%; text-align:left; border-bottom: 1px solid #1c3247; line-height:0.1em; margin:10px 0 20px;"
						>
							<span
								style="padding:0 10px; background: #1c3247 ; padding: 5px 10px; color: #FFFFFF; border-radius: 5px"
								>{eNM.nombre}</span
							>
						</h4>
						{#if asignaturasmalla.length > 0}
								...
								{#each asignaturasmalla as am}
									{#if eNM.idm == am.nivelmalla_id}
										<div class="col mb-4 cardSubjects valueYesSearch ida_{am.asignatura_id}">
											<div
												class="card rounded-3 h-100 border border-2 shadow-none card-dashed-hover"
											>
	...
	<Modal
		...
		backdrop="static"
	>
		<ModalHeader toggle={mToggleAsignatura} class="bg-primary text-white">
			<h4 class="text-white">
				...
				<div class="row">
					<div class="col-12 text-center">
						<div class="alert alert-warning">
							...
							<p class="alert-body">MATERIA NO ESTÁ ABIERTA, CONTACTAR Al BALCÓN DE SERVICIOS</p>
						</div>
					</div>
				</div>
			{:else}
							...
							>
								<div class="card-header">
									...
									<div class="text-muted fs-6">
										{#if m.tipomateria === 1}
											<span
												class="badge rounded-pill bg-info bg-info"
												id={`Tooltip_tipomateria_${m.id}`}>{m.tipomateria_display}</span
											>
											<Tooltip target={`Tooltip_tipomateria_${m.id}`} placement="top"
												>Tipo de impartición de clase</Tooltip
											>
										{:else if m.tipomateria === 2}
											...
											<Tooltip target={`Tooltip_tipomateria_${m.id}`} placement="top"
												>Tipo de impartición de clase</Tooltip
											>
										{:else if m.tipomateria === 3}
											<span class="badge rounded-pill bg-success" id={`Tooltip_tipomateria_${m.id}`}
								...
								<div class="card-body">
									<div class="text-center">
										<span class="badge rounded-pill fs-3 bg-primary" id={`Tooltip_paralelo_${m.id}`}
											>{m.paralelo}</span
										>
										<Tooltip target={`Tooltip_paralelo_${m.id}`} placement="top">Paralelo</Tooltip>
										{#if m.tipomateria != 3}
											<h5 class="card-title mt-2 mb-0 fs-6">{m.session}</h5>
										{/if}
									...
									{/if}
									{#if ePeriodoMatricula.ver_horario_materia && m.tipomateria != 3}
		...
		<ModalFooter>
			<Button color="warning" class="rounded-3 btn-sm" on:click={mToggleAsignatura}>Cerrar</Button>
		</ModalFooter>
	</Modal>
	<Modal
		...
		fade={false}
	>
		<ModalHeader toggle={mToggleAsignaturaPractica} class="bg-primary text-white">
			<h4 class="text-white">
				...
				<div class="row">
					<div class="col-12 text-center">
						<div class="alert alert-warning">
							<h4 class="alert-heading">Importante!</h4>
							<p class="alert-body">
								LA MATERIA NO EXISTE PRÁCTICAS PROGRAMADAS, CONTACTAR Al BALCÓN DE SERVICIOS
							</p>
						</div>
					</div>
				</div>
		...
		<ModalFooter>
			<Button color="warning" class="rounded-3 btn-sm" on:click={mToggleAsignaturaPractica}
				>Cerrar</Button
			>
		</ModalFooter>
	</Modal>
	...
	<Modal
		...
		fade={false}
	>
		<ModalHeader toggle={mToggleConfirmarMatricula} class="bg-primary text-white">
			<h4 class="text-white">Confirmar matrícula</h4>
		</ModalHeader>
		<ModalBody>
			...
			<p>
				...
				<b class="fs-5">¿Está {ePersona.es_mujer ? 'sgura' : 'seguro'} de matricularse?</b>
			</p>
		</ModalBody>
		<ModalFooter>
			<Button color="success" class="rounded-3 btn-sm" on:click={() => confirmarMatricula()}
				>Confirmar</Button
			>
			<Button color="warning" class="rounded-3 btn-sm" on:click={mToggleConfirmarMatricula}
				>Cerrar</Button
			>
		</ModalFooter>
	</Modal>
	...
	<Modal
		...
		fade={false}
	>
		<ModalHeader toggle={() => closeConfirmarCaso()} class="bg-primary text-white">
			<h4 class="text-white">
		...
		<ModalFooter>
			<Button color="success" class="rounded-3 btn-sm" on:click={() => confirmarCaso()}
				>Confirmar</Button
			>
			<Button color="warning" class="rounded-3 btn-sm" on:click={() => closeConfirmarCaso()}
				>Cerrar</Button
			>
		</ModalFooter>
	</Modal>
...
<style>
	...
	/*.nav-item > .nav-link .active {
		background-color: rgba(61, 3, 3, 0.08) !important;
