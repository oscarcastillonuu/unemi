...
from sagest.forms import ActivoFijoForm, InformeBajaForm, DetalleInformeBajaForm, \
    ...
    InformeBajaFormAF, PeriodoConstatacionAFForm, ConstatacionFisicaForm, DescargarBajasAniosForm, GestionarBajaActivoForm
...
from sagest.models import ActivoFijo, ArchivoActivoFijo, HdDetalle_Incidente, MantenimientosActivosGarantia, \
    ...
    ESTADO_ACTA_CONSTATACION, ActivoTecnologico
...
from sga.excelbackground import descarga_activos_bajas_background, reporte_activos_constatados_background, reporte_activos_constatados_openxl_background, reporte_historico_estado_activos_por_constatacion_background, generar_acta_constatacion_background
...
from sga.funcionesxhtml2pdf import conviert_html_to_pdf, conviert_html_to_pdfsaveqr_generico, conviert_html_to_pdfsave_generic_lotes
    ...
    if request.method == 'POST':
        ...
        # CONSTATACION DE ACTIVOS FIJOS
            ...
            with transaction.atomic():
                try:
                    ...
                    if not detalle_c:
                        ...
                        detalle_c.save(request)
                        log(u'Agrego detalle de constatación: %s' % (detalle_c), request, "add")
                    detalle_c.encontrado = eval(request.POST['val'].capitalize())
                    detalle_c.enuso = eval(request.POST['val'].capitalize())
                    detalle_c.estadoactual = activo.estado
                    detalle_c.ubicacionbienes = activo.ubicacion
                    detalle_c.usuariobienes = activo.responsable
                    detalle_c.perteneceusuario = True
                    if not eval(request.POST['val'].capitalize()):
                        detalle_c.requieretraspaso = False
                        detalle_c.requieredarbaja = False
                        # detalle_c.perteneceusuario = False
            ...
            try:
                ...
                if form.cleaned_data['estadoactual'] and activo.estado != form.cleaned_data['estadoactual']:
        ...
        # INFORME DE BAJA
            ...
            try:
                ...
                # VALIDACIONES
                if firmadirector and not director.personadepartamento.id == persona.id:
                ...
                x, y, numPage = obtener_posicion_x_y_saltolinea(archivo_url, palabras, True, True)
                if not x or not y:
            ...
            try:
                ...
                documentos = DocumentoFirmaInformeBaja.objects.filter(status=True, firmadirector=False, responsablefirma__personadepartamento=persona)
            ...
            try:
                ...
                for name in nombres_hojas:
                    ...
                    for index, row in df.iterrows():
                        ...
                        if activo:
                            ubicacion = Ubicacion.objects.filter(nombre=ubicacion).first()
                            ...
                            estadoactual = EstadoProducto.objects.filter(nombre=estado).first()
        ...
        if 'action' in request.GET:
            ...
            if action == 'editdirector':
                ...
                try:
                    ...
                    ws.write(0, 12, 'Costo', formatoceldagris)
                    ws.write(0, 13, 'Reportado por', formatoceldagris)
                    ws.write(0, 14, 'Fecha que se reporto', formatoceldagris)
                    cont = 1
                    for activo in activos:
                        ...
                        ws.write(cont, 12, str(activo.costo))
                        ws.write(cont, 13, str(reportado_por))
                        ws.write(cont, 14, str(fecha))
                        cont += 1
                    workbook.close()
                    output.seek(0)
            ...
            #CONSTATACION DE ACTIVOS FIJOS
                ...
                try:
                    ...
                    base_query = Q(status=True, codigoconstatacion__periodo_id=idp)
                    filtro = base_query
            ...
            # INFORME BAJA
                ...
                try:
                    data['val_extra'] = request.GET.get('idex', '')
                    director = PersonaDepartamentoFirmas.objects.filter(status=True, tipopersonadepartamento_id=1, departamentofirma_id=2, actualidad=True).last()
                    if not director:
                        raise NameError('Usted no es el director actual configurado para firmar documentos.')
                    documentos = DocumentoFirmaInformeBaja.objects.filter(status=True, firmadirector=False, responsablefirma=director)
                    if not documentos:
                        raise NameError('No tienen documentos pendiente a su nombre por ser firmados.')
                    data['info_mensaje'] = f'Esta por firmar {len(documentos)} informes de baja que se encuentran a su nombre como director.'
                    template = get_template("formfirmaelectronicamasiva.html")
                    return JsonResponse({"result": True, 'data': template.render(data)})
                except Exception as ex:
                ...
                try:
                    ...
                    search, url_vars, estado = request.GET.get('s', ''), f"&action={action}", request.GET.get('estado', '')
                ...
                try:
                    ...
                    noti = Notificacion(cuerpo='Reporte de constatación de activos en progreso',
                                        ...
                                        en_proceso=True)
                    noti.save(request)
                    reporte_activos_constatados_openxl_background(request=request, data=data, notif=noti.pk).start()
            ...
            estado, clase, cuenta, search, filtro, url_vars = request.GET.get('estado', ''), \
                                                              request.GET.get('clase',''), \
                                                              request.GET.get('cuenta', ''), request.GET.get('s', ''), Q(status=True), ''
...
def crear_constataciones(activo, persona, request):
    with transaction.atomic():
        try:
            hoy = datetime.now()
            periodo = PeriodoConstatacionAF.objects.filter(status=True, activo=True).first()
            if periodo:
...
def generar_informebaja(request, informebaja):
    try:
        ...
        valido = conviert_html_to_pdfsaveqr_generico(request, dir,
                                                     {'pagesize': 'A4',
                                                      'informe': informebaja.activofijo, 'perso': perso, 'director': director,
                                                      'bajaactivo': informebaja, 'detalle': detalle, 'hoy': datetime.now().date()
                                                      }, directory, nombre_archivo)
        if not valido[0]:
            raise NameError('Error al generar el informe')
        archivo_url = '/media/reportes/informesbaja/' + nombre_archivo
        ruta_archivo = os.path.join(SITE_STORAGE, 'media', 'reportes','informesbaja', nombre_archivo)
