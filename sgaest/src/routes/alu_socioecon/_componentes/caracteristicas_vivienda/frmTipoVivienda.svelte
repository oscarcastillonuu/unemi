<script lang="ts">
	...
	import { getTiposViviendas as loadDataTiposViviendas } from '$lib/utils/loadDataApi';
	...
	onMount(async () => {
		...
		if (aData.eFichaSocioeconomica) {
			...
			if (eFichaSocioeconomica.tipovivienda) {
				selectTipoVivienda = eFichaSocioeconomica.tipovivienda['pk'] ?? 0;
			}
		}
		await delay(2000);
		load = false;
	...
	const saveDatosCaracteristicaVivienda = async () => {
		...
		formData.append('tipovivienda', selectTipoVivienda.toString());
		formData.append('field', 'tipovivienda');
	...
	<ModalBody>
		{#if !load}
			<form action="javascript:;" id="frmTipoVivienda">
				<div class="row g-3">
					<div class="col-12">
						<label for="id_tipovivienda" class="form-label fw-bold"
							><span><i class="fe fe-alert-octagon text-warning" /></span> ¿Cuál es el tipo de vivienda? </label
						>
						<select
						...
						<div class="valid-feedback" id="id_tipovivienda_validate">¡Se ve bien!</div>
					</div>
				</div>
			</form>
		{:else}
