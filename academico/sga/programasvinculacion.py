...
from sga.forms import ProgramasInvestigacionForm, EvidenciaForm, ProyectoInvestigacionForm, \
    ...
    CambioCarreraVinculacionForm,RegistrarHorasVinculacionForm, ParticipanteProfesorVinculacionForm
...
from inno.models import TecnicoAsociadoProyectoVinculacion
    ...
    if request.method == 'POST':
        ...
        if action == 'aprobacion_informe':
            ...
            try:
                ...
                if f.is_valid():
                    ...
                    if es_informe_lider:
                        response, msj = migrar_evidencia_proyecto_vinculacion(request, informe, int(informe.aprobacion) - 1)
                        if not response: messages.warning(request, msj)
        ...
        if action == 'firmainformevinculacion':
            try:
                ...
                if not x or not y: return JsonResponse({'result': 'bad', 'mensaje': "No se encontró la palabra %s en el archivo. Por favor verifique que este nombre se encuentre en el apartado de firmas." % words})
                ...
                if c.aprobacion == 3:
                    ...
                    val, _ = migrar_evidencia_proyecto_vinculacion(request, c, 4)
                    if not val: messages.warning(request, _)
                    log(u'%s firmo documento: %s' % (persona, nombrefile_), request, "add")
                return JsonResponse({'result': 'ok', 'id': c.pk, 'proyecto': c.proyecto.pk, 'para_revision': False})
            except Exception as ex:
            ...
            try:
                f = SubirInformeForm(request.POST, request.FILES)
                if f.is_valid():
                    ...
                    if informe.aprobacion == 3:
                        owner = informe.profesor.persona
                        es_lider = owner == informe.proyecto.lider()  # Si el dueño de la evidencia es lider del  proyecto
                        ...
                        if criterio := DetalleDistributivo.objects.filter(criteriodocenciaperiodo__criterio=c, distributivo__profesor__persona=owner, distributivo__periodo=periodo, status=True).first():
                            if informe.proyecto.evidenciaactividaddetalledistributivo_set.filter(criterio=criterio, desde=informe.fecha_inicio, hasta=informe.fecha_fin, status=True).exists():
                                val, _ = migrar_evidencia_proyecto_vinculacion(request, informe, 4)
                                if not val: return JsonResponse({'result': 'bad', 'mensaje': _})

                    notificacion = Notificacion(titulo="Informe mensual de avance validado",
                                                cuerpo=f"{persona.nombre_completo_inverso()} ha cargado su informe mensual de avance validado del proyecto de Servicio Comunitario",
            ...
            try:
                participante = ParticipantesMatrices.objects.get(pk=request.POST['id'])
                f = RegistrarHorasVinculacionForm(request.POST)
                if f.is_valid():
                    participante.registrohorasdesde = f.cleaned_data['fechainicio']
                    participante.registrohorashasta = f.cleaned_data['fechafinalizacion']
                    participante.horas = f.cleaned_data['horas']
                    participante.estado = f.cleaned_data['estado']
                    participante.save(request)
        ...
        if 'action' in request.GET:
            ...
            #
              ...
              try:
                  ...
                  data['configuraciones'] = configuracion_lider = ConfiguracionInformeVinculacion.objects.filter(status=True, proyecto= proyecto, aprobacion__in=[2,3], informecondensado=True).order_by('-fecha_inicio')
